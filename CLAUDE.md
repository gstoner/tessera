...
>
> **Standalone compiler track (S-series) — S0/S1 + S2–S15 Python reference surface landed (2026-05-10).** Tessera is being lifted into a standalone model compiler that is *runtime-independent of PyTorch, JAX, and Flax*. They are reference vocabularies only — never imported by the runtime. S0 locks the scope decisions (data pipeline, training step, custom-op API, AOT export are all in-scope). S1 ships the standalone primitive contract registry at `python/tessera/compiler/primitive_coverage.py` (**362 entries, 0 planned, 362 partial**) with a guarded dashboard at `docs/audit/standalone_primitive_coverage.md` and a current-state audit at `docs/audit/primitive_coverage_state.md`. S2–S15 each have a Python reference module shipped (`rng.py`, `state/`, `control.py`, `sharding.py`, `losses.py`, `optim.py`, `quantization.py`, `data.py`, `aot.py`, `custom.py`, `memory.py`); contract-axis hardening (math/shape/dtype/batching/sharding/backend) is the next quality gate. See `docs/audit/execution_roadmap.md`.
>
...
| **S-series** (standalone compiler track) | 🟢 S0/S1 + S2–S15 Python reference surface landed | **S0** locks scope (data pipeline, training step, custom-op API, AOT export all in-scope; PyTorch/JAX/Flax reference vocabularies only). **S1** registry at `python/tessera/compiler/primitive_coverage.py` — 362 entries × 12 contract axes × 53 categories. Consults `tessera.autodiff.vjp._VJPS` and `autodiff.jvp._JVPS` so registered (V/J)VPs auto-flip to `complete`. Guards: `tests/unit/test_standalone_compiler_roadmap.py` + snapshot drift gate. **S2–S15 Python reference shipped (2026-05-10):** S2 reductions/stability/numeric helpers/comparisons (35 ops + VJPs); S3 `state/tree.py` pytrees + 8-collection state taxonomy; S4 `rng.py` RNGKey + 12 samplers + replay metadata; S5 `control.py` scan/cond/while/fori/vmap/pmap/jvp/vjp/remat/autocast; S6 `sharding.py` shard_map + 6 collectives; S7 `nn.functional` LinearGeneral/Conv1d/ConvTranspose/LoRA/GroupNorm/InstanceNorm/Weight/Spectral norms + pooling + RNN cells + ALiBi/NTK + GQA/MQA/MLA wrappers; S9 `quantization.py` int8/int4 + fake_quantize + observer + grad scaler; S10 `optim.py` 9 optimizers + 7 schedules + 7 grad transforms; S11 `losses.py` 21 losses; S12 `checkpoint.py` save/load/sharded/migration; S13 `custom.py` custom_primitive/call/vjp/jvp/batching/lowering; S14 `aot.py` AOT export/load + StableHLO/GGUF/safetensors + cache; S15 `data.py` Dataset combinators + ShardedDataset + 5 tokenizers. **Autodiff registry:** 119 VJPs + 25 JVPs (was 22+14 pre-session). **Op catalog:** 201 OpSpecs (was ~145). **Contract-axis hardening (math/shape/dtype/batching/sharding/backend) remains the next quality gate per `docs/audit/primitive_coverage_state.md`.** |
| RubinCPX | ✅ Built | `tessera.target.cpx` dialect, 4 passes, `tessera-cpx-opt` driver, `TESSERA_BUILD_RUBINCPX_BACKEND` CMake option |

**Total active tests: 2,078 passing under `-m "not slow"` in `tests/unit/` (3 pre-existing `test_debug_env.py::TestDiffCommand` failures, unrelated); Apple Phase 8 lit fixtures 4/4 passing; Phase 2 + Phase 7 KV-cache lit fixtures 2/2 passing; S-series roadmap guard 26/26 passing in `tests/unit/test_standalone_compiler_roadmap.py`; autodiff/lowering hardening guard 4/4 passing in `tests/unit/test_autodiff_lowering_gap_hardening.py`.**
...
**Autodiff registry status:** 119 VJPs + 25 JVPs registered (was 22 + 14
pre-S-series). 165 entries with `vjp=complete`, 39 with `jvp=complete` —
the registry computes these automatically by consulting `_VJPS`/`_JVPS`.

**Largest remaining contract-axis gaps** (across all 362 entries):
batching rule 362, sharding rule 362, backend kernel 362, transpose rule
312, JVP 287, math semantics 339, shape rule 339, dtype/layout 339. Per
...
*Last updated: 2026-05-10 — Phases 1–6 complete; Phase 7 lit-clean; Phase 8 Apple operational (CPU 8.2 via Accelerate; GPU 8.3 → 8.4.7 via MPS + custom MSL). KV-cache Decision-#21 lowering shipped on x86 + Apple CPU/GPU + Metalium. Apple GPU runtime exports **26 C ABI symbols** across 9 kernel concepts × {f32, f16, bf16}. Pipelines: `tessera-lower-to-{rocm,metalium,apple_cpu,apple_cpu-runtime,apple_gpu,apple_gpu-runtime}`. Build pin: **LLVM/MLIR 21**. **Standalone compiler track (S-series) S0/S1 + S2–S15 Python reference surface landed** — `primitive_coverage.py` (362 entries × 12 axes × 53 categories), `state/`, `rng.py`, `control.py`, `sharding.py`, `nn/functional.py` extensions, `losses.py`, `optim.py`, `quantization.py`, `data.py`, `aot.py`, `custom.py`, `memory.py`. Autodiff: 119 VJPs + 25 JVPs. Tests: 2,078 passing. Contract-axis hardening (math/shape/dtype/batching/sharding/backend) is the next quality gate per Decision #25. See `docs/audit/primitive_coverage_state.md` and `docs/apple_gpu_overview.md`.*
