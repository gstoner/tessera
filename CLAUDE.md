...
> constraint on the code.** As of 2026-08-30 this section carries **20**
> recorded amendments: six `Adopted 2026-08-02` entries, four letter-suffixed
> sub-decisions, and ten dated banners. Decision #5 has been amended twice —
> fail-open AST inference was recorded as a correction, then superseded when
> W2.2 made inference IR-derived and fail-closed. That is the protocol
> working, not a defect. It was previously headed "Do Not Revisit," which was
> never true of its own contents.
>
> **The 2026-08-30 sweep re-read every decision against the compiler
> direction (MLIR/LLVM core; prune the Python bootstrap backend path;
> contract-carrying Target IR; measured three-tier arbiter). Six changed:**
>
> | # | What was wrong | Severity |
> |---|---|---|
> | 1 | Named a retired target as the only execution path and gated GPU work behind `>= SM_90`, which excludes the live sm_120 lane | **actively harmful** |
> | 11 | Autotune cache key carries no toolchain/delegate version, so a toolkit upgrade silently invalidates measurements without invalidating the cache | latent stale-baseline bug |
> | 12 | Benchmark rows cannot say which lowering route produced a number, so three competing tiers are not comparable | measurement gap |
> | 19 | Justification was doubly false: "hardware-free" (its own example names AMD hardware) and "makes backends lit-testable" (NVVM/ROCDL are equally host-free). Real value is **contract carriage** | wrong reason for a right rule |
> | 26a | Its "revisit on architectural grounds" trigger arrived — and is answered by up-levelling the Apple dialect, *without* AIR | strengthened, not reversed |
> | 28/31 | Read literally, #31 says to delete every Tier-3 candidate #28 exists to keep | **unresolved conflict** |
>
> The last is the one that mattered most: #31 and #28 had no stated boundary,
> so the bootstrap prune had no principled stopping point. They are now
> reconciled in both places — #31 governs lowering *paths*, #28 governs
> implementation *selection*.

1. **CPU-first, then GPU — historical ordering only. Corrected 2026-08-30: do not apply this as a gate.** As written it said "x86 AMX is the only real execution path" and "GPU ops gated behind `target_profile.isa >= SM_90`". Every clause is now false or harmful: **AMX is a retired target** (superseded by ACE — x86 native proof means AVX-512); **gfx1151 and sm_120 both execute broadly**; and the `>= SM_90` comparison reads as excluding **sm_120**, which is the live NVIDIA lane. Someone applying this literally would gate off working hardware. What survives is only the sequencing fact that CPU came first. For what actually executes, read `docs/audit/generated/runtime_execution_matrix.md` (Decision #26) — never this line.
...
11. **Bayesian autotuner warm-starts from SQLite cache.** Key = `hash(device_class + kernel_id + config)`. v2 schema adds Optuna trial IDs.
    ...
    **Amended 2026-08-30 — the key must include the toolchain and delegate identity.** Measured: `autotune_v2` keys on `{op, shape, dtype, arch, layout, numeric_policy, movement}` and **nothing versioned**. Under Decision #28 a cached entry is a *measurement*, and a measurement is only valid for the code that produced it: cuBLAS 12 and 13 differ numerically and in speed, a CUDA/ROCm toolkit upgrade changes generated code, and a Tier-3 delegate is a versioned artifact. Without a version in the key, an upgrade silently invalidates every stored measurement **without invalidating the cache** — the same stale-baseline failure the Krylov ratchet hit on 2026-08-30, latent here instead of in a JSON file. Add the toolchain/library version (and, for a delegate, its ABI hash) to the key, so a stale entry misses rather than lies.
...
14. **MFMA shapes live in a lookup table.** `MFMAFullCoveragePass` reads `mfma_table.inc` (generated by `scripts/generate_mfma_table.py`). Do not hardcode shapes in pass logic.
    ...
    **Corrected 2026-09-25 (code read): no `MFMAFullCoveragePass` exists in `src/`, and nothing includes `mfma_table.inc`** — the table is generated by `scripts/generate_mfma_table.py` and sync-gated by `tests/unit/test_phase_ghi_lane2.py` (its header cites a `test_mfma_table_in_sync.py` that does not exist), but it has no consumer (a Decision #29 unconsumed declaration, kept because CDNA is a planned target). MFMA is **CDNA-only** (gfx90a/942/950, hardware-gated); the fleet's RDNA parts (gfx1151, gfx1201) use **WMMA**, whose shapes are table-driven in Python `_WMMA_VARIANTS` (`rocm_target` → `rocm_mma` / `mma_selector` / `rocm_fragment`). The rule survives — shapes come from a table, not pass logic — but name the WMMA table for fleet work and wire or delete the MFMA one when a CDNA part lands.
...
19. **Backends expose a toolchain-free, contract-carrying Target IR before hardware-specific lowering.** Each backend defines an ODS dialect of target ops (`tessera_rocm.mfma`, `tessera_apple.cpu.accelerate_gemm`, `tessera_apple.gpu.metal_kernel`) between Tile IR and final hardware emission. Validated by `test_target_ir_contract.py`.
    ...
    **`tessera_x86` now exists — x86 complies (built 2026-08-02, W0.10).** It was previously the one backend with no Target IR dialect at all: `TileToX86Pass` lowered Tile IR to 21 `func::CallOp`s into a hand-written C shim, and the Python emitter named a `tessera_x86.func` op no dialect defined. **No carve-out was granted.** The dialect lives at `src/compiler/codegen/tessera_x86_backend/include/TesseraX86/IR/`, is registered in `tessera-opt`, and splits into value-carrying ops (`amx_tile_load`/`amx_tile_zero`/`amx_dpbf16ps`/`amx_dpbusd`/`amx_tile_store` over a real `!tessera_x86.tile` type, so the verifier rejects a tile dot-product whose operands never came from a tile load) and directives (`avx512_gemm_microkernel`, `pack_b_panel`, `elementwise`, `kernel`, `kv_cache_read`, `unsupported`). `abi_call` **models the C-shim boundary instead of hiding it**, so Decision #28's arbiter can tell compiler-generated work from delegated work. Fixtures: `tests/tessera-ir/phase2/x86_target_ir{,_invalid}.mlir` — including a negative case, since a dialect that only ever accepts proves nothing. **Remaining follow-on is `x86vector.*` (AVX-512) lowering only; the AMX half is not optional but closed** — **confirmed 2026-08-30 (was "expected to be superseded"): AMX is a dead end and is not a Tessera target.** It was Intel-only and has been superseded by **ACE (AI Compute Extensions)**, the matrix-instruction spec agreed jointly by AMD and Intel. The AMX ops stay as the IR-level contract without an `amx.*` lowering, and when a matrix lane is next needed here the target is ACE, not AMX. Read every "AMX not applicable (no fleet hardware)" row in `docs/audit/backend/x86/todo.md` as **closed by direction**, not parked pending a purchase — do not scope AMX enablement, benchmarking, or box acquisition. Corollary that has already bitten once: **never gate a test on `hardware_amx` to mean "x86 hardware"**, because it will skip on the only box that can run it (AVX-512 runs on the primary box; no fleet machine has AMX, and none will).
...
28. **The forward compiler direction is the three-tier / measured-arbiter model — leads set the ceiling, the generic framework raises the floor.** (North star, 2026-07-02.) Kernels come from three tiers: **(1)** a generic synthesizer (arch-agnostic region-IR + F4 oracle + synth→compile→cache→launch loop), **(2)** a per-arch codegen plugin (`KernelEmitter`/`TargetPlugin`: MSL / PTX / AMDGCN / C-LLVM), **(3)** hand-tuned kernels. A **measured, accuracy-budgeted arbiter** picks the fastest *in-budget* candidate per `(op, shape-bucket, dtype, target)`. **ROCm and CUDA are the lead performance targets: shared infra must never cap their ceiling** — hand-emitted `wgmma`/`mma.sync`/MFMA/WMMA stay first-class arbiter candidates, displaced only when a compiled kernel is both faster and in accuracy budget. The synthesizer/plugin interface is **symbolic-dim-aware from day one** (`static | bucket | dynamic` policy; first impls bucket-specialize) so dynamic shapes never force an API break. **Reconciled with Decision #31 on 2026-08-30.** #31 allows one production lowering per boundary; this decision keeps three tiers competing for the same op. They do not conflict once the axes are named: **#31 governs lowering *paths* (how IR descends a level), #28 governs implementation *selection* (which kernel runs for one op at one level).** A Tier-3 delegate reached through a declared, verified, arbitrated Target IR op is one lowering path with several measured implementations behind it. Cite #31 against a *second lowering authority*, never against an arbiter candidate — see #31's own banner. Full model: [`docs/audit/compiler/COMPILER_THEORY_OF_OPERATION.md`](docs/audit/compiler/COMPILER_THEORY_OF_OPERATION.md); execution: [`docs/audit/compiler/COMPILER_REFACTOR_PLAN.md`](docs/audit/compiler/COMPILER_REFACTOR_PLAN.md). These are *direction*; MASTER_AUDIT + generated dashboards stay the primary status evidence.

29. **A declaration must have a consumer.** (Adopted 2026-08-02, W0.8.) If the compiler declares metadata — an ODS type or attribute, a `primitive_coverage` axis, a coverage claim — a **named pass or code path must consume it**, or the declaration is deleted. A declaration with no consumer is worse than a missing one: it reads as a closed contract in review and in the dashboards while carrying nothing. **Sequencing corollary for operator expansion (added 2026-08-30).** Decision #19's amendment invites Apple (~12 ops) and x86 (~8) to grow their Target IR. This decision is what keeps that honest: **add each op only when its producer and its consumer land with it.** Landing an op family ahead of the passes that emit and read it manufactures exactly the unconsumed declaration described here — twenty ops that read in review as a closed contract and carry nothing. One op proven end-to-end beats twelve declared. Drift-gated by `tests/unit/test_governance_declarations.py`. Derived from seven independent instances (`manifold` reaching no backend; `MultivectorSpec.grades` ignored by `geometric_product`; a closed `batching_rule` axis whose `vmap` is a Python for-loop; a closed `shape_rule` axis whose inference is a five-case if-chain; nine declared `!tile.*` types alongside 70 `Variadic<AnyType>`; `numeric_policy` with no carrier below Graph IR; `TilingInterface` unused by `fusion_core.py`).

29a. **A declared debt is exempt from deletion, not from being marked.** (Adopted 2026-09-19.) #29 offers two remedies — consume it or delete it — and a third state exists in practice where deleting destroys value: a model that is *built, tested, and recorded as pending wiring*, where the wiring is genuinely wanted. **Worked case:** `rocm_tiling.split_k_required` has a producer, a serializer, a unit test, and `ROCM_PATTERNS_FROM_AMD_ECOSYSTEM.md` naming the wiring as future work. The gfx1201 router-gate shape (M≤16, K=2048, N=256 — 16 tiles on 32 CUs, weight read dominating A by 16×) then showed split-K is genuinely needed, so applying #29's delete remedy would have discarded work about to become load-bearing. This is #31's ordering caveat in #29's clothing: **do not collapse a declaration before the surviving path can carry what it carried.**

    **The same case is why the exemption must stay narrow, because it caused #29's stated harm anyway.** The declaration read as "we model split-K" and cost a session's worth of checking to learn it never runs — and the model that exists is **wrong**: `split_k_required = k > 4096` returns `False` at K=2048, the shape that most needs it, because the real trigger is occupancy (tiles < CUs), not K magnitude. So an unwired model is not merely inert. **It can be wrong, and its wrongness is invisible precisely because nothing runs it** — the one failure mode a live path cannot have. Never cite this exemption to mean an unwired model is harmless.

    **Four conditions, all required.** (1) The declaration is marked **at the site** as unwired, naming the queue item that owns the wiring — a reader must not have to grep to learn it never runs. (2) That owning item exists in a queue with a gate, not in a comment. (3) Its test asserts the model's *behaviour*, not merely that the field is set: an unwired model's only defence against silent wrongness is a test that would fail if the model were wrong. (4) It stays counted in the declaration audit, so the exemption is visible rather than quiet. **Absent any one of these it is an ordinary #29 unconsumed declaration and the remedy is deletion.** This exemption does not touch #29's sequencing corollary: landing an op family ahead of its producer and consumer is still forbidden, because a declared op is a contract offered to callers, where an unwired internal model is not.
...
31. **One implementation per boundary.** (Adopted 2026-08-02, W0.8.) Each IR level boundary has exactly **one production lowering**. A second implementation is either (a) a **declared oracle** with a differential test against the production path, or (b) deleted. Same rule for frontends and AD engines. Drift-gated by `tests/unit/test_governance_declarations.py`. **Ordering caveat:** do not collapse a duplication before the surviving path can carry what the deleted one carried — that is the documented way this fails; see the W0→W1→W2→W3 ordering in `INTEGRATED_COMPILER_PLAN.md`.
    ...
    **Reconciled with Decision #28 on 2026-08-30 — read the two together or you will delete the arbiter.** #28 keeps three tiers of kernels (synthesized / per-arch emitted / hand-tuned) *deliberately competing* for the same op. Read literally, every Tier-3 candidate is a second implementation this decision says to delete, and the delegation contract (`tessera_nvidia.kernel_call` / `inline_ptx`) sits exactly on the ambiguity. The line:
...
extras. `profiler_timing` and `target_perf.apply_corpus` implement this since
2026-09-26 (kernel-side clock + a witness valid in the same sample); SSD
admission and the recorders still carry the old rule, and sm_120 still lacks
its kernel-side witness (`%globaltimer`).
