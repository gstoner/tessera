...
> **Phases 1–6 complete. Phase 7 lit-clean. Phase 8 — Apple M-Series CPU (Phase 8.2) and GPU (Phases 8.3 → 8.4.7) operational. apple_gpu ships 26 runtime symbols across 9 kernel concepts × {f32, f16, bf16}, including 4 fused chains (matmul→softmax, matmul→gelu, matmul→rmsnorm, matmul→softmax→matmul); **plus a MetalPerformanceShadersGraph (MPSGraph) lane (2026-05-29) — Tier-1 activations/norms (silu/silu_mul/relu/sigmoid/tanh/softplus/exp/log/sqrt/rsqrt/neg/abs, layer_norm, rmsnorm, log_softmax, softmax; no N≤256 limit) on `@jit(target="apple_gpu")`, which also completes the f16/bf16 + large-N fused-chain matrix by composing GPU matmul with an MPSGraph epilogue (see `tests/unit/test_apple_gpu_mpsgraph_lane.py` + the full Llama-decoder-layer proof `tests/unit/test_apple_gpu_llama_decoder_layer.py`)**; 9/9 native EBM primitives (incl. `ebm_partition_exact` via stable logsumexp); RAII-hardened Metal buffer pool — every dispatcher acquires via `TS_METAL_BUF_ACQUIRE` macros so early-return paths are release-safe by construction, locked by [tests/unit/test_apple_gpu_buffer_pool.py](tests/unit/test_apple_gpu_buffer_pool.py); `@clifford_jit(target="apple_gpu")` does AST → `CliffordIRProgram` lowering at decoration time (SSA `%tN` refs + inline `#int:N` / `#float:V` literals so `ga.grade_projection(a, 2)` lowers cleanly). See [docs/apple_gpu_overview.md](docs/apple_gpu_overview.md) and [docs/status/ga_ebm_milestone.md](docs/status/ga_ebm_milestone.md).**
>
> **Standalone compiler track (S-series) — S0/S1 + S2–S15 Python reference surface landed, reasoning-model attention/RL coverage shipped, multi-axis contract hardening in place, and the generated support-table audit now gates drift.** Tessera is being lifted into a standalone model compiler that is *runtime-independent of PyTorch, JAX, and Flax*. They are reference vocabularies only — never imported by the runtime. S0 locks the scope decisions (data pipeline, training step, custom-op API, AOT export are all in-scope). S1 ships the standalone primitive contract registry at `python/tessera/compiler/primitive_coverage.py`; treat generated docs (`docs/audit/generated/support_table.md` and `docs/audit/standalone_primitive_coverage.md`) as the count/status truth rather than copying numeric snapshots by hand. `docs/audit/coverage/COVERAGE_AUDIT.md` is now a historical narrative snapshot. S2–S15 each have a Python reference module shipped (`rng.py`, `state/`, `control.py`, `sharding.py`, `losses.py`, `optim.py`, `quantization.py`, `data.py`, `aot.py`, `custom.py`, `memory.py`, **`rl.py`** for PPO/GRPO/CISPO). **Reasoning-model attention family** is registered with VJP+JVP and has a dedicated C++ pass `src/transforms/lib/AttentionFamilyPasses.cpp`. **Long-tail sharding-rule pass** introduced `_SHARDING_RULE_BY_CATEGORY` classifier; backend_kernel remains the leading axis pending Phase G/H/I runtime validation. See `docs/audit/roadmap/ROADMAP_AUDIT.md`.
>
> This file is the authoritative working reference. Read it before touching any code.
> Last updated: May 2026 (MLIR 22 / LLVM 22 build pin — Homebrew `llvm` 22.1.6).
...
The **x86 AMX/AVX512 backend** is the broadest fully wired execution path. Apple CPU/GPU also have real runtime lanes for the explicitly documented Phase 8 surface, including the GA/EBM and selected tensor/complex fast paths. Other accelerator backends primarily produce IR or target artifacts until a hardware-gated proof row says otherwise.
...
**Total active tests: 5,769 passing under `-m "not slow"` in `tests/unit/` (0 failures; 19 skipped) as of 2026-05-29 (~4:46 wall); heavy SuperBench / benchmark contract tests marked `slow` and excluded from the default sweep. Long-tail VJP/JVP closure (2026-05-10) added 25 VJPs and 23 JVPs for collectives (`all_reduce`/`all_gather`/`all_to_all`/`reduce_scatter`), recurrent cells (`simple_rnn_cell`/`gru_cell`/`bidirectional_scan`), quantization STE (`fp4`/`fp6`/`nvfp4` quant+dequant, `int4` dequant), spectral (`stft`/`istft`/`dct`/`spectral_conv`/`spectral_filter`), sparse matmul (`spmm_coo`/`spmm_csr`/`sddmm`/`bsmm`), and linalg (`cholesky`/`qr`/`svd`/`tri_solve`). **Spectral solver pass-body landing (2026-05-10)** added 6 missing JVPs (`fft`/`ifft`/`rfft`/`irfft`/`stft`/`istft`) bringing the spectral family to vjp+jvp+lowering complete, and shipped C++ pass bodies for all 6 spectral passes guarded by 26 Python tests in `tests/unit/test_spectral_solver_passes.py`. **MLIR verifier sprint V1–V7b + V3a/V3b/V3c + V4a/V4b (2026-05-22)** added shape/element-type verifiers on 7 Graph IR ops (`TransposeOp`/`LayerNormOp`/`MoeDispatchOp`/`ReshapeOp`/`CastOp`/`SoftmaxOp`/`RopeOp`/`DropoutOp`), `LayoutLegalityPass` skeleton + producer/consumer rule, target-aware `head_dim`/`tile_q`/`tile_kv` ceilings on `FlashAttnOp` + `ScaledDotProductOp`, `SymbolicDimEqualityPass` (`--tessera-symdim-equality` — bindings + per-op contracts + SSA flow + sum-of-products affine + interprocedural via `func.call` + scf.for/scf.if region propagation), and `DialectExtension` eager-load for `tessera.attn` — 64 structural guards in `tests/unit/test_mlir_verifier_sprint.py` + 12 new lit fixtures across phase2/phase3. **Sprint M (2026-05-22) Apple GPU MLA E2E proof** lands `tests/unit/test_apple_gpu_mla_e2e.py` — first model-shaped end-to-end on Apple Silicon (3 projections + per-head fused matmul→softmax→matmul + output projection) numerically validated against numpy at fp32 rtol=1e-4 across 3 shape parametrizations. **Phase G/H/I hardware-gated frontier (2026-05-22):** `docs/audit/backend/BACKEND_AUDIT.md` is the single honest answer to "what's hardware-blocked vs. still hardware-free?" — registry truth is 0 entries at `backend_kernel = complete` across all 432 (273 partial + 159 planned), gated on real NVIDIA/ROCm/Metalium hardware. Apple Phase 8 lit fixtures 4/4 passing; Phase 2 + Phase 7 KV-cache lit fixtures 2/2 passing; S-series roadmap guard 30/30 passing; full Tessera-IR lit sweep 61 PASS / 19 UNSUPPORTED / 18 XFAIL / 0 FAIL across 98 fixtures (live 2026-06-01, MLIR 22.1.6).**
...
| Module | Purpose |
|--------|---------|
| `__init__.py` | Top-level exports: `jit`, `kernel`, `Region`, `domain`, `dist`, `array`, `index_launch`, `constraint`, `ops`, `Tensor`, `f16`, `mut_f32`, `dtype` (Sprint A0 canonical-dtype module). **`train`** (the agent-native MoE training stack) is **lazily bound** via a PEP 562 module `__getattr__` — `import tessera; tessera.train.X` resolves on first access without import-time cost or cycle risk; the agent-native firewall + lazy export are locked by `tests/unit/test_train_agent_native_firewall.py` + `test_train_top_level_export.py`. |
...
| `compiler/evaluator.py` | **Evaluator program (2026-06-12)** — execution-derived scoring engine; see `docs/audit/compiler/EVALUATOR_PLAN.md` §9.5. `Rung` ladder (8 rungs) + `verdict_for`/`evaluate` (reads the *runtime* provenance signal `execution_kind`/`runtime_status`, not compile intent; a silent numpy fallback can never earn an execution rung). Oracles: vertical (`evaluate`), horizontal/PolyJuice (`horizontal_equivalence` — fused≡unfused on the same backend), metamorphic (`metamorphic_equivalence` — algebraic invariants), **DESIL cross-path** (`cross_path_equivalence` — same program across independent compilers, e.g. Metal vs Accelerate). `safe_input` (legal-by-construction inputs); `nvidia_emission_verdict` (rung 2.5). Guards: `tests/unit/test_evaluator.py`, `test_evaluator_e2.py`. |
| `compiler/conformance_evaluator.py` | **Evaluator program** — corroborates conformance `complete` cells by *independently re-deriving* each on the real backend (matmul/softmax/matmul_softmax/flash_attn → rung 7), "derive validates declare". Coverage gate: every executable complete cell must have a builder. Guard: `tests/unit/test_conformance_evaluator.py`. |
| `compiler/ptx_emit.py` | **Evaluator program / NVIDIA rung 2.5** — emits documented WGMMA PTX assembler text (`wgmma.mma_async...` + fence/commit/wait protocol) for sm_90 bf16 matmul; structural validator (rung 2.5, verifiable host-free) + `ptxas_assemble` (rung 3, Linux-CI, skip-clean). Wired into `@jit` via `matmul_pipeline.emit_nvidia_ptx`. Honest: skeleton, not a complete assemblable kernel. Guard: `tests/unit/test_ptx_emit.py`. |
| `compiler/flywheel.py` | **Evaluator program / Pillar 2 (perf)** — deterministic `(candidate, outcome)` autotuning records: measured Metal latency + analytical roofline on one row, **device-keyed** (per-chip calibration via `sysctl`, `peak_for_device`). `record_matmul`/`sweep_matmul`/`efficiency_trend`; persist (`save_corpus`/`load_corpus`) + distill (`distill_dispatch` → O(1) decision-tree). Guard: `tests/unit/test_flywheel.py`. |
| `compiler/flywheel_autotune.py` | **Evaluator program** — flywheel↔`autotune_v2` bridge: `gemm_workload_for`/`measured_tflops` (FLOP-accounting cross-check), device-grounded `autotuner_for`, `best_record`/`pick_best`, and `autotune_matmul` (autotune the measurable Apple knob — dtype). Guard: `tests/unit/test_flywheel_autotune.py`. |
| `compiler/compiler_grader.py` | **Evaluator program / Pillar 3 (scored env)** — TensorBench-style internal grader: a task = a named compiler-correctness check graded over **hidden inputs** (fresh RNG), passing only when a real oracle agrees. Anti-cheat structural (no silent-fallback credit, no vacuous pass). `task`/`grade`/`grade_all`; 4 seed tasks compose the four oracles. Guard: `tests/unit/test_compiler_grader.py`. |
| `compiler/attention_tasks.py` | **Evaluator program / LongCA** — structure-keyed attention task family: grades attention by (mask × seqlen) — `{full,causal}×{s64,s128,s256}` — not as one op; registers into the grader on import. Guard: `tests/unit/test_attention_tasks.py`. |
| `compiler/magellan.py` | **Evaluator program / Pillar 3** — Magellan-style gated heuristic evolution: `search`/`evolve` optimizers whose fitness is **perf gated behind correctness** (`INF` ⇒ rejected); `dtype_policy_fitness` (measured latency behind an f16-vs-f32 gate). Guard: `tests/unit/test_magellan.py`. |
| `compiler/alphaevolve.py` | **Evaluator program / Pillar 3** — AlphaEvolve-style evaluator-driven search where the **full grader is the gate**; `select` rejects a faster-but-failing reward-hack candidate (the Sakana invariant, proven). Guard: `tests/unit/test_alphaevolve.py`. |
...
| `dflash.py` + `dflash_{reference,io,serve}.py` | **DFlash speculative decoding (block-diffusion draft)** — rides the `attn_bias` substrate. `dflash.py`: `DFlashConfig`/`DFlashWeights`, `target_feature_projection`, decoder layer, `dflash_draft_forward`(`_cached`), `DraftKVCache`/`RotatingDraftKVCache`, samplers, `dflash_linear_verify`/`dflash_speculative_verify` (distribution-preserving rejection), `dflash_step`/`dflash_generate`(`_cached`), position-weighted training loss, `HiddenStateTap`, `DFlashDraft` (`nn.Module`), `apple_gpu_attention_fn`. `dflash_reference.ReferenceDecoderLM` (stateful KV-cached numpy target + `rollback` — greedy-AR ground truth). `dflash_io` (dependency-free safetensors + HF state-dict ↔ `DFlashWeights`). `dflash_serve` (`dflash_generate_text` + `DFlashScheduler`). **Proven: greedy spec-decode == greedy AR vs the `z-lab/dflash` MLX reference.** Attention core runs on Apple GPU `metal_runtime`. See [docs/dflash.md](docs/dflash.md). |
| `models/{__init__,diffusion_gemma,block_diffusion,block_diffusion_runtime,moe_routing,sampler,decode,staging}.py` | **`tessera.models` — production model graphs (experimental).** DiffusionGemma: a Gemma-4-calibrated block-diffusion MoE text model as a **shape-only Graph + config-aware verifier** — `DiffusionGemmaConfig`, `build_text_block`/`verify_text_block`/`verify_config`, param-budget estimator; reference MoE top-k routing/packing (`route_top_k`/`moe_forward`), entropy-bound sampler, block-diffusion step graph + `BlockDiffusionDecoder` decode loop, quant/vision staging manifests. **Native block-diffusion execution** (`block_diffusion_runtime`): GQA canvas denoiser (canvas queries over `[encoder_KV ++ canvas_KV]`, bidirectional) + grouped-SwiGLU MoE FFN via `ops.flash_attn` + `moe_swiglu_block`; `backend="apple_gpu"` runs the whole step on Metal (`execute_block_diffusion_step`, `NativeBlockDiffusionDecoder`). Compiler sees the step as one Graph-IR op **`tessera.diffusion_block_step`** (verifier: bidirectional canvas / GQA / head_dim≤256; Tile→Apple tags `metal_runtime`). Adds differentiable `tessera.ops.softcap` (Gemma logit soft-cap) with VJP+JVP. |
...
| `tpp/` | ✅ Tensor Parallel Primitives — dialect, 7 passes, and `tpp-space-time` pipeline alias registered in `tessera-opt`; 4/4 TPP lit fixtures pass. Python frontmatter is wired for status/pipeline metadata, while embedded-MLIR Python execution remains a subprocess/front-end integration gap. |
...
| Path | Purpose |
|------|---------|
| `tools/tessera-opt/tessera-opt.cpp` | MLIR opt-style driver — all dialects + passes registered. **Builds against MLIR 22.1.6 (2026-05-31; was MLIR 21 / `llvm@21`).** `cmake --build build --target tessera-opt` produces a working 69 MB binary that registers **5 dialects** (tessera / tessera.neighbors / tessera.solver / tessera_apple / **tpp**) + 70+ Tessera passes + 6 named lowering pipelines (x86 / gpu / apple_cpu / apple_cpu-runtime / apple_gpu / apple_gpu-runtime) + **TPP's 7 passes + `tpp-space-time` pipeline alias**. Phase 7 Neighbors lit fixtures (4 tests) all pass; TPP lit fixtures (4 tests) all pass; full tessera-ir suite 34/72 PASS, 19 UNSUPPORTED, 19 XFAIL, 0 FAIL. Smoke-tested by `tests/unit/test_tessera_opt_build.py` (9 tests, skipped when binary missing). |
| `tools/tessera-translate/tessera-translate.cpp` | **`tessera-translate-mlir` C++ binary (2026-05-18).** MLIR-native translation driver registering **4 flags**: `--mlir-to-llvmir`, `--import-llvm`, `--serialize-spirv`, `--deserialize-spirv`. Carries the union of standard MLIR dialects (incl. SPIR-V) + Tessera dialects (core / neighbors / TPP / Apple). End-to-end smoke verified: LLVM-dialect MLIR → real LLVM IR text; SPIR-V module → `.spv` binary → MLIR text round-trip. Built via `cmake --build build --target tessera-translate-mlir`. |
...
| `python/tessera/cli/translate.py` | `tessera-translate` console-script entry — StableHLO / GGUF / SafeTensors / info exports plus the `mlir` passthrough to `tessera-translate-mlir`. |
...
21. **Unsupported lowering must emit a stable diagnostic.** When a backend cannot lower an op (e.g., KV-cache on a target without it), emit a diagnostic that names the op and the target — never silently no-op or fall through. See the KV-cache → target lowering for the canonical pattern. **Per-target coverage matrix:** `docs/audit/coverage/COVERAGE_AUDIT.md` (audited 2026-05-09 — Apple CPU/GPU and ROCm honor #21 with named diagnostics; NVIDIA/x86/TPU/Cerebras simply don't encounter KV-cache ops in tested paths today and need explicit handling when they light up).
...
27. **Ground every Metal / Apple GPU API claim in a real source before declaring it possible or "blocked."** When working the Apple GPU backend (`apple_gpu_runtime.mm`, Tile→Apple lowering, MTL4 / MPSGraph / MSL / packaged kernels), do not assert an API exists, doesn't exist, or is "blocked" from memory or from an empty web fetch. Authoritative sources, in reliability order: **(1) on-machine SDK headers** — `xcrun --show-sdk-path` → `…/System/Library/Frameworks/{Metal,MetalPerformanceShaders,MetalPerformanceShadersGraph,MetalPerformancePrimitives}.framework/Headers/`; these are the exact interfaces the runtime links (e.g. `grep -n "<symbol>" "$(xcrun --show-sdk-path)"/System/Library/Frameworks/Metal.framework/Headers/*.h`). **(2) User-provided doc dumps** (Read them when handed one). **(3) The `apple-metal-docs-urls` memory file** for canonical developer.apple.com URLs. **WebFetch caveat:** developer.apple.com is a JS-rendered SPA — `WebFetch` returns only the page *title*, not the API body (verified 2026-06-02), so it is NOT a reliable Metal-doc source; use the SDK headers or a doc dump instead. **Anti-pattern:** writing a "blocked / no API path" conclusion into an audit doc from absence of evidence in one source. Distinguish runtime-serializable shader libraries / pipeline data sets (`MTL4Compiler.pipelineDataSetSerializer`, `newDynamicLibraryWithURL:`, `MTLBinaryArchive`/`MTLDynamicLibrary serialize`) from authoring a `.mtlpackage` ML package on disk (an *offline* Apple toolchain step — no runtime ABI). The 2026-06-02 `.mtlpackage` "blocked" wording was overstated because the SDK headers were not consulted first.
...
**Open work:** build `tessera-opt` against MLIR 22 (Homebrew `llvm` 22.1.6 — see Local toolchain), run lit tests, fix any pass-body bugs the tests expose.
...
ODS sets `usePropertiesForAttributes = 0` to keep the dialect header-only against MLIR's properties machinery (still required under MLIR 22). `tessera-opt` links the backend behind a `TESSERA_HAVE_APPLE_BACKEND` guard so non-Apple builds are unaffected.
...
End-to-end verified on this Mac (LLVM/MLIR 22.1.6, Accelerate active): single GEMM bitwise-matches numpy; multi-op tiny decode bitwise-matches the numpy reference; rank-3 batched GEMM bitwise-matches numpy; fp16 matmul matches an f32-converted reference at fp16 tolerance.
...
- ✅ TPP solver (`src/solvers/tpp/`) — dialect, type/attr printing, 7 passes, and `tpp-space-time` pipeline alias are wired into `tessera-opt`; 4/4 TPP lit fixtures pass. Remaining gap is embedded-MLIR Python execution beyond the frontmatter/subprocess path.
...
See Architecture Decisions #23 and #24, and `docs/audit/roadmap/ROADMAP_AUDIT.md`.
...
**What's shipped today (per `docs/audit/coverage/COVERAGE_AUDIT.md`):**
...
`docs/audit/coverage/COVERAGE_AUDIT.md` for the per-group breakdown.
...
Everything Tessera needs for local build / lint / typecheck / lit / unit-test is
on Homebrew on this Mac — **`python3` (3.14.5), `ninja`, `cmake` 4.3, `pytest`,
`mypy`, `ruff`, `black`, `isort`, `flake8`, `lit`** are all under
`/opt/homebrew/bin/`; **LLVM/MLIR 22.1.6** is `/opt/homebrew/opt/llvm/` (`brew
install llvm`). You can run the Python flow directly with `python3 -m pytest`
etc. without activating a venv — `numpy` 2.4, `scipy` 1.17, `torch` 2.12,
`transformers` 5.9, `ml_dtypes` 0.5 are all installed at
`/opt/homebrew/lib/python3.14/site-packages/`.

The (older) llvm@21 keg is still installed for any rebuild compatibility need,
but the canonical build pin is now `llvm` (22.1.6). When pointing CMake at LLVM
explicitly, use `/opt/homebrew/opt/llvm/lib/cmake/{llvm,mlir}` (NOT the `llvm@21`
path that older `build/` caches reference).

## C++ Build

```bash
# CPU + Apple backend against Homebrew LLVM/MLIR 22 (Ninja). This is the local
# canonical configure — `tessera-opt` builds and links MLIR 22.1.6.
...
# Re-verify a C++ pass change end-to-end (G3 example): rebuild, run the lit
# fixture with FileCheck, run the drift test.
...
PYTHONPATH=python python3 -m pytest tests/unit/test_apple_gpu_tile_pass_status_matches_envelope.py -q

# CPU-only legacy (Makefiles, older cache pointing at llvm@21)
...
*Last updated: 2026-06-13 — **Speculative decoding (`attn_bias` substrate + DFlash) + DiffusionGemma model graph landed (PRs #67–#71).** **`attn_bias` substrate:** `flash_attn` gains an optional additive `attn_bias` operand — `O = softmax(scale·Q·Kᵀ + attn_bias)·V`, shape `(B,Sq,Sk)` broadcastable from `(1,Sq,Sk)`, end-to-end: `FlashAttnOp` ODS gains `Optional<TensorType>:$attn_bias` + `AttrSizedOperandSegments` + verifier (rank-3 / `Sq` / broadcastable batch); Tile→Apple routes the bias to MPSGraph symbols `tessera_apple_gpu_flash_attn_bias_{f32,f16,bf16}` (transpose K → matmul → scale → +bias → softmax → PV; causal+bias applies both; broadcast bias falls back to reference); `ops.flash_attn(Q,K,V,attn_bias=…)` eager + `@jit(target="apple_gpu")`→`metal_runtime` + CPU reference + VJP returns `dbias`. Metal-vs-numpy ≈3e-7 (fp32). General feature — any structured additive mask now flows through `flash_attn`. **`tessera.dflash` (+ `_reference`/`_io`/`_serve`):** DFlash ([z-lab/dflash](https://github.com/z-lab/dflash)) block-diffusion speculative-decoding draft — predicts a whole `block_size−1` block in one parallel forward via mask tokens + block-structured attention conditioned on target hidden features. **Proven: greedy spec-decode output == greedy autoregressive decode** vs the MLX reference (the guarantee: speculation changes only speed, never output). Attention core on Apple GPU `metal_runtime`. External gates (open): real-checkpoint numerical parity (HF network) + a fully-jitted GPU draft (needs a GPU gather/embedding op). See [docs/dflash.md](docs/dflash.md). **`tessera.models` — DiffusionGemma:** a Gemma-4-calibrated block-diffusion MoE text model as a shape-only Graph + config verifier (`DiffusionGemmaConfig`, `build_text_block`/`verify_*`, param-budget estimator) + reference MoE routing/sampler/decode/staging layers; **the whole block-diffusion step executes on Apple GPU `metal_runtime`** (`block_diffusion_runtime`: GQA canvas denoiser over `[encoder_KV ++ canvas_KV]` + grouped-SwiGLU MoE FFN via `ops.flash_attn` + `moe_swiglu_block`; `execute_block_diffusion_step`, `NativeBlockDiffusionDecoder`), validated vs numpy at head_dim=256. Compiler sees one Graph-IR op **`tessera.diffusion_block_step`** (verifier: bidirectional canvas / GQA / head_dim≤256; Tile→Apple `metal_runtime`). New differentiable **`tessera.ops.softcap`** (Gemma logit soft-cap) with VJP+JVP; new `tessera.ops.bmm`/`fake_quantize` catalog rows; new `nn.functional`/`nn` layers (`block_diffusion_attention`, `mask_token_block`, `linear_general`, `lora_linear`, `conv_transpose`, pooling, `gru_cell`/`simple_rnn_cell`/`bidirectional_scan`, `NativeSparseAttention`, `MixtureOfRecursions`). Apple C ABI now **264 symbols across 112 kernel families** (count truth: `docs/audit/generated/runtime_abi.md`). Registry **445 entries × 12 axes**. New guards: `test_flash_attn_bias_semantics.py`, `test_block_diffusion_attention.py`, `test_dflash_*` (draft/cached/reference/module-serve/train-io/target-tap/apple-gpu-e2e), `test_diffusion_gemma_*` (graph/moe/sampler/decode/runtime/block-step/staging); lit `phase8/apple_gpu_flash_attn_bias.mlir`, `diffusion_block_step{,_invalid}.mlir`. — **Evaluator program — substantially shipped.** A generative, execution-derived, **backend-rung-aware** scoring engine that *derives* conformance/benchmark/autotune surfaces from one honest harness (closing the "registry models reality" gap): `compiler/evaluator.py` (8-rung verdict + provenance gate; four oracles — vertical, horizontal/PolyJuice, metamorphic, **DESIL cross-path**), `conformance_evaluator.py` (re-derives complete cells at rung 7), `ptx_emit.py` (NVIDIA WGMMA PTX emission, rung 2.5), `flywheel.py` + `flywheel_autotune.py` (deterministic autotuning records + per-chip-calibrated roofline + `autotune_v2` bridge), and the scored environment — `compiler_grader.py` (TensorBench-style, hidden inputs, anti-cheat), `attention_tasks.py` (LongCA mask×seqlen), `magellan.py` + `alphaevolve.py` (gated search; reward-hack rejection proven). NVIDIA/ROCm sit truthfully at rung 1–2.5 (WGMMA PTX *emitted*; rung-3 `ptxas` + complete kernel + silicon hardware-gated, need a Linux/CUDA runner). Plan + landed-status: `docs/audit/compiler/EVALUATOR_PLAN.md` §9.5; ~80 guard tests across `tests/unit/test_{evaluator,evaluator_e2,conformance_evaluator,ptx_emit,flywheel,flywheel_autotune,compiler_grader,attention_tasks,magellan,alphaevolve}.py`. — **Toolchain bump + compiler-layer gaps G1–G3 landed.** **Toolchain:** moved off the venv to the Homebrew env on `python3` 3.14.5 (numpy 2.4 / scipy 1.17 / torch 2.12 / transformers 5.9 / ml_dtypes 0.5, all under `/opt/homebrew/lib/python3.14/site-packages/`); local tooling = ninja 1.13 / cmake 4.3 / lit 18.1 / pytest 9.0 / mypy 2.1 / ruff 0.15 / black 26.5 / isort 8.0 / flake8 7.3 (all `/opt/homebrew/bin/`); **MLIR/LLVM bumped 21 → 22.1.6** at `/opt/homebrew/opt/llvm/` (`brew install llvm`). Canonical configure: `cmake -S . -B build -G Ninja -DLLVM_DIR=/opt/homebrew/opt/llvm/lib/cmake/llvm -DMLIR_DIR=/opt/homebrew/opt/llvm/lib/cmake/mlir -DTESSERA_CPU_ONLY=ON -DTESSERA_BUILD_APPLE_BACKEND=ON`; `ninja -C build tessera-opt` builds clean (32 warnings, all pre-existing `rewriter.create<OpTy>` deprecation noise — no errors). `tessera-opt --version` reports `Homebrew LLVM 22.1.6`. **G1 (Schedule→Tile metadata):** `_lower_schedule_ops` lowers `schedule.mesh.define`/`layout`/`artifact` to `tile.mesh.define`/`tile.layout`/`tile.artifact` + wraps `schedule.mesh.region` as `tile.mesh.region(mesh, axis)`; `_flatten_tile_ops` filters `TILE_METADATA_OPS` so they don't leak as compute. **G2 (C++ `LowerScheduleToTargetPass`):** honest downgrade — was empty pseudocode + unregistered + redundant with the Python spine; now fails loudly (`emitError`+`signalPassFailure`) instead of silent-no-op-success, with honest `getArgument`/`getDescription`; added `stubbed` row in `docs/audit/compiler/COMPILER_AUDIT.md`; **compile-verified under MLIR 22**. **G3 (Tile→Apple aligns with runtime envelope):** `TileToApple.cpp`'s GPU pass tags `status = "metal_runtime"` for the full envelope via `isAppleGpuRuntimeOp` mirroring `driver._APPLE_GPU_{MPS,MSL,MPSGRAPH}_OPS` (27 ops) and `artifact_only` otherwise (was blanket `artifact_only`); lit fixture `apple_gpu_lowering.mlir` updated + 3 phase8 Apple fixtures pass; **single-source enforcer** `test_apple_gpu_tile_pass_status_matches_envelope.py` runs the real `tessera-opt` pass over every envelope op and fails if C++ and Python diverge; **all compile + lit + drift-verified under MLIR 22**. Plan + acceptance criteria: `docs/audit/compiler/COMPILER_AUDIT.md`. Open: G4 (single-source runtime execution matrix), G5 (C ABI artifact compile/load/launch — now genuinely buildable here). Affected sweep: 242 IR/spine/apple tests green under MLIR 22. — Phases 1–6 complete; Phase 7 lit-clean; Phase 8 Apple operational (CPU 8.2 via Accelerate; GPU 8.3 → 8.4.7 via MPS + custom MSL). **Tier-2 batched matmul + numpy-pin relaxation (2026-05-29):** the `numpy<2.0.0` pin is **relaxed** (now `numpy>=1.21.0` in `pyproject.toml` + `requirements.txt`) — Tessera runs under numpy 1.x **and** 2.x; the 4 numpy-stub-sensitive mypy sites (savez ×2, nditer `op_flags` ×2 in `aot.py`/`checkpoint.py`/`autodiff/vjp.py`/`ebm/energy.py`) were made portable via `Any`-typing (no version-specific `# type: ignore`), so mypy is clean under both. **Batched matmul (`bmm`) — the Tier-2 keystone — landed**: MPSGraph-backed `tessera_apple_gpu_bmm_{f32,f16}` (bf16 host-upcast) with **batch broadcasting** (shared/`[1,K,N]` B operand — the projection + GQA KV-sharing shape) in `apple_gpu_runtime.mm` + stub parity; `runtime.py::_apple_gpu_dispatch_bmm` routes rank-3+ `tessera.matmul`/`tessera.batched_gemm` (incl. rank-4 fold-to-batch) on `@jit(target="apple_gpu")` → `execution_mode="metal_runtime"`; `tessera.batched_gemm` added to the driver/runtime MPS envelopes. Roadmap in [docs/apple_gpu_tier2_tier3_plan.md](docs/apple_gpu_tier2_tier3_plan.md); guards in `tests/unit/test_apple_gpu_bmm.py` (10, incl. per-head attention scores + broadcast). **Tier-2 projections + batched MHA (2026-05-29):** `tessera.linear_general` (last-axis `x@W(+bias)`) and `tessera.qkv_projection` (`x@W` → split-3, multi-output) now route through the matmul/bmm lane (`_APPLE_GPU_PROJECTION_OPS` gating) → `metal_runtime` (`tests/unit/test_apple_gpu_projections.py`, 9). A **full multi-head-attention block composes from the Tier-2 ops** (`qkv_projection → bmm(Q,Kᵀ)·scale → softmax → bmm(_,V) → linear_general`, batch = B·H, **no per-head loop, head_dim unbounded** — no flash_attn ≤256 limit), validated vs numpy in `tests/unit/test_apple_gpu_batched_mha.py` (6, incl. head_dim=320). GQA/MQA Phase-1 (repeat-KV → flash_attn) is already in `nn.functional`. **Tier-2/3 follow-ons (2026-05-29):** (a) **Tier-3 reductions** — `tessera_apple_gpu_mpsgraph_{reduce,argreduce,scan}_f32` give sum/mean/amax/amin/prod/var/std/argmax/argmin/cumsum/cumprod on GPU; `runtime.py` normalizes arbitrary axis/keepdims/ddof by folding reduced axes to last (`tests/unit/test_apple_gpu_reductions.py`, 51). (b) **Native GQA/MQA** — `flash_attn_gqa_f32` reads KV group `h//(H/G)` with **no repeated KV** (Ph2 bandwidth win); `_apple_gpu_dispatch_gqa` (`tests/unit/test_apple_gpu_gqa.py`, 8). (c) **Fused batched attention** — `tessera_apple_gpu_mpsgraph_bsmm_{f32,f16}` does `softmax((Q@Kᵀ)·scale)@V` per batch in one MPSGraph dispatch; `_apple_gpu_dispatch_batched_attention` (`tests/unit/test_apple_gpu_fused_attention.py`, 8). **Apple GPU MPSGraph lane — compile-time + hardening follow-ups (2026-05-29):** landed the deferred items on the MPSGraph lane. (1) **C++ MLIR lowering path:** registered `silu`/`tanh`/`softplus`/`rmsnorm`/`log_softmax` as first-class `tessera` dialect ops in `TesseraOps.td` (+ `POINTWISE_BUILD_ADJOINT` entries in `AdjointInterface.cpp`) and added three lowering passes — `tessera-unary-to-apple_gpu` / `tessera-silu-mul-to-apple_gpu` / `tessera-rowop-to-apple_gpu` (`src/compiler/codegen/Tessera_Apple_Backend/lib/Target/Apple/Lowering/{Unary,SiluMul,RowOp}ToAppleGPU.cpp`) wired into the `tessera-lower-to-apple_gpu-runtime` pipeline (now 17 passes; the unweighted norms emit null gamma/beta). Lit: `tests/tessera-ir/phase8/apple_gpu_tier1_lowering.mlir` (full tessera-ir lit sweep 61 PASS / 19 UNSUPPORTED / 18 XFAIL / 0 FAIL across 98). (2) **MSL gelu NaN fix:** the hand-written MSL gelu kernels (`gelu_f32`/`_f16` + fused `matmul_gelu_f32`) clamp the tanh argument to `[-30,30]`, eliminating NaN for large activations (|x|≳16). (3) **MPSGraph graph caching:** graphs cached by `(shape-class, opcode, dtype, shape[, eps, weighted])` and reused across calls; new `tessera_apple_gpu_mpsgraph_cache_size()` introspection symbol. (4) **Native f16/bf16 fused MSL kernels** — deferred as a spawned perf-only follow-up (the compose path already runs these on-GPU correctly). Guards: `tests/unit/test_apple_gpu_mpsgraph_lane.py` (+gelu-large-activation +cache-reuse), `test_apple_gpu_pass_order_matrix.py` (14→17 passes), regenerated `runtime_abi.md` + `verifier_coverage.md` dashboards. **Apple GPU MPSGraph lane (2026-05-29):** added a MetalPerformanceShadersGraph-backed execution lane in `src/compiler/codegen/Tessera_Apple_Backend/runtime/apple_gpu_runtime.mm` (+ non-Darwin reference parity in `apple_gpu_runtime_stub.cpp`) exposing 12 new C ABI symbols — op-coded unary (`relu`/`sigmoid`(`_safe`)/`tanh`/`softplus`/`silu`/`exp`/`log`/`sqrt`/`rsqrt`/`neg`/`abs`), op-coded binary (`silu_mul` = silu(a)·b), and row ops `layer_norm`/`rmsnorm`(`_gpu`)/`log_softmax`/`softmax` (f32+f16 native, bf16 host-upcast, fp32 internal accumulation, no N≤256 limit). Wired into `@jit(target="apple_gpu")` via `runtime.py` dispatchers + `_APPLE_GPU_UNARY_OPCODES`/`_APPLE_GPU_ROWOP_KINDS` envelope and `driver.py::_APPLE_GPU_MPSGRAPH_OPS` gating so these Tier-1 ops report `execution_mode="metal_runtime"`. **Completed the f16/bf16 + large-N fused-chain matrix:** `matmul_gelu`/`matmul_rmsnorm`/`matmul_softmax` keep their f32 single-kernel fast paths and now compose GPU matmul + MPSGraph epilogue otherwise (the gelu epilogue uses the robust MPSGraph `gelu` node — the hand-written MSL gelu NaNs for |x|≳16, spawned as a follow-up). The MPSGraph framework was added to all four runtime-build sites (CMake, `runtime.py`, `_apple_gpu_dispatch.py` bridge loader, `benchmarks/apple_gpu/benchmark_ga_ebm.py`) plus the per-test build helpers. New tests: `tests/unit/test_apple_gpu_mpsgraph_lane.py` (24) + `tests/unit/test_apple_gpu_llama_decoder_layer.py` (6 — full pre-norm Llama decoder layer: RMSNorm + attention + SwiGLU, fp32 & fp16, validated vs numpy). `rmsnorm_safe` compiler-example now marks apple_gpu runtime-executable; runtime ABI dashboard regenerated (80→92 symbols). **MLIR verifier sprint V1–V7b + V3a/V3b/V3c + V4a/V4b (2026-05-22) closed all four originally-deferred verifier gaps in SHAPE_SYSTEM.md §11.2.** **V1 + V4b** added `let hasVerifier = 1;` + real `verify()` bodies to 7 Graph IR ops (`TransposeOp`/`LayerNormOp`/`MoeDispatchOp`/`ReshapeOp`/`CastOp`/`SoftmaxOp`/`RopeOp`/`DropoutOp`) with shape/element-type/attribute-bounds checks; **V2 + V4a** shipped `LayoutLegalityPass` (`--tessera-layout-legality`) with the canonical 8-name layout accept-set rule + matmul producer/consumer mismatch detection (`LAYOUT_LEGALITY_UNKNOWN_LAYOUT`, `LAYOUT_LEGALITY_PRODUCER_CONSUMER_MISMATCH`); **V3 + V6c** added target-aware per-SM head_dim ceilings on `FlashAttnOp` + per-SM tile_q/tile_kv ceilings on `ScaledDotProductOp` (FA-4 Tile IR) sourced from `docs/nvidia_cuda13_kernel_inventory.md`; **V5 + V6a/V6b + V2-flow + V3a + V3b + V3c** built `SymbolicDimEqualityPass` (`--tessera-symdim-equality`) progressively into the canonical symbolic-dim verifier — function-level bindings + per-op contracts (`tessera.reshape`/`transpose`/`matmul`), pipeline wiring into `tessera-lower-to-x86`/`gpu`/`nvidia-pipeline-{sm90,sm100,sm120}`, SSA-value dim-name propagation seeded from `tessera.arg_dim_names`, sum-of-products affine bindings (`D = H * Dh + K`), interprocedural cross-checks via `func.call` + module-level `SymbolTable` (`SYMDIM_CALL_ARG_MISMATCH`), and scf.for/scf.if region recursion with loop-invariant yield + branch-agreement checks (`SYMDIM_LOOP_YIELD_MISMATCH`, `SYMDIM_IF_BRANCH_MISMATCH`); **V7 + V7b** registered `tessera.attn` dialect into `tessera-opt` and wired `DialectExtension`-based eager-load so dotted-prefix parsing works in standalone IR fixtures.  64 structural guards in `tests/unit/test_mlir_verifier_sprint.py`; 12 new lit fixtures across `phase2/` + `phase3/` covering V1, V2, V3, V3a, V3b, V3c, V4a, V4b, V5, V6b, V6c.  Full Tessera-IR lit sweep: 61 PASS / 19 UNSUPPORTED / 18 XFAIL / 0 FAIL across 98 fixtures (live 2026-06-01, MLIR 22.1.6).  **Sprint M (2026-05-22) Apple GPU MLA E2E proof** lands `tests/unit/test_apple_gpu_mla_e2e.py` — first model-shaped end-to-end on Apple Silicon composing 4 jitted callables (3 projections + per-head attention + output projection) for a single-layer MHA decoder, numerically validated against numpy reference at fp32 rtol=1e-4 across 3 shape parametrizations (T=8/D=16/H=2, T=16/D=32/H=4, T=32/D=64/H=8); Target IR sanity asserts the per-head 3-op chain collapses to exactly one `tessera_apple.gpu.msl_kernel` with `fusion = "matmul_softmax_matmul"` and Darwin-specific `execution_mode == "metal_runtime"`.  **Phase G/H/I hardware-gated frontier audit (2026-05-22)** lands `docs/audit/backend/BACKEND_AUDIT.md` as the single honest answer: registry truth is **0 entries at `backend_kernel = complete` across all 432** (273 partial + 159 planned) — by the registry's own design `backend_kernel` is a per-primitive-not-per-target axis requiring all declared targets to ship real kernels, so flipping any entry to `complete` requires real NVIDIA / ROCm / Metalium hardware that this Mac cannot provide; all hardware-free pre-work (G-1..G-9, H-1..H-8, I-1..I-3) is in place.  Registry now reports **432 entries × 12 axes** (up from 374 on 2026-05-11 due to additional ops landing); residual non-hardware gaps are 53 vjp + 53 jvp planned (mostly N/A audit candidates), 37 masking_effect_rule planned (device-state ops), 30 sharding_rule partial, 40 transpose_rule partial, 36 batching_rule partial.  Tests: **5,769 passing, 19 skipped, 0 failures** in ~4:46 minutes (2026-05-29; +38 from the Apple GPU MPSGraph lane work — 29 lane + 6 Llama-decoder-layer + gelu/cache guards; was 5,731 earlier 2026-05-29, 5,254 on 2026-05-22 and 2,904 on 2026-05-11 — registry growth + new MLIR verifier guards + MLA E2E + 179 long-tail closure tests). **2026-05-29 green-up:** fixed a NumPy-2.0-only `np.asarray(..., copy=)` call in `benchmarks/visual_complex_core/core.py` (the env pinned `numpy<2.0.0` at the time; **pin relaxed 2026-05-29** — see the Tier-2 note in the lead), corrected an off-by-one float32 assumption in `test_memory_architecture.py::TestBucketSharding`, and cleared 8 mypy errors (3 stale `# type: ignore`, 2 nditer `op_flags` nesting, a `_`/`**_` unpack collision in `vjp.py`, a `dbias` None-guard in `jvp.py`, and a stale return annotation in `test_coverage_audit.py`) so the mypy ratchet is back at baseline 0.  **Graph IR cache LRU upgrade (2026-05-22):** `python/tessera/compiler/graph_ir_cache.py` moved from an unbounded process-local `dict` to a bounded `OrderedDict`-backed LRU (default capacity 1024 — comfortably above the existing `tests/unit/` working set; tunable via `TESSERA_GRAPH_IR_CACHE_CAPACITY` env or `set_cache_capacity(n)` at runtime).  All mutations wrap a `threading.Lock` (correctness under Python 3.13+ free-threaded mode without depending on the GIL); deepcopy stays outside the lock so concurrent JITs don't serialize on CPU-bound copying.  `cache_stats()` keeps the legacy `hits / misses / size` keys and adds `evictions` + `capacity` for thrashing observability.  15 new guards in `tests/unit/test_graph_ir_cache_lru.py` cover LRU bump-on-hit, eviction order, capacity shrink, env-var override, empty-source skip, and an 8-thread × 200-op concurrency smoke test that asserts hit + miss counts sum cleanly (lock-correctness gate). **Phase G/H/I hardware-free pre-work batch 3 landed (2026-05-11): G-5 + H-2 + G-6/G-7/G-8 + H-6/H-7/H-8 + G-9 + H-8.** **G-5 NVIDIATargetPipeline:** 4 new named pass aliases in `src/transforms/lib/Passes.cpp` — `tessera-nvidia-pipeline` (default SM_90) + `tessera-nvidia-pipeline-{sm90,sm100,sm120}` — chaining EffectAnnotation → Canonicalize → SwigluFusion → MLA/NSA/Hybrid/Lightning/Delta fusion → DistributionLowering → TileIRLowering → WarpSpec → AsyncCopy → WGMMA → TMA → NVFlashAttnEmitter. New lit fixture `phase3/cuda13/nvidia_pipeline_alias.mlir`. **H-2 MFMA table:** `scripts/generate_mfma_table.py` generates `src/compiler/codegen/Tessera_ROCM_Backend/include/TesseraROCM/mfma_table.inc` from `_MFMA_VARIANTS` Python source; X-macro format (`TESSERA_MFMA_VARIANT(arch_id, arch_name, M, N, K, K_blocks)`); 22 shapes across gfx90a (2) / gfx940 (6) / gfx942 (6) / gfx950 (8) / gfx1100 (0). **G-6/G-7/G-8 + H-6/H-7/H-8 toolchain validation:** new `cmake/TesseraToolchainPins.cmake` with `tessera_pin_cuda_toolkit(13.2)` + `tessera_pin_rocm(7.2.3)` + `tessera_add_{nvcc,hipcc}_compile_check` helpers; new `scripts/validate_nvcc_compile.py` (covers 8 PTX patterns from G-4 fixtures) + `scripts/validate_hipcc_compile.py` (covers 8 AMDGCN intrinsics from H-4 fixtures with CDNA-4 FP4 → gfx950 / WMMA → gfx1100 dispatch). Both validators skip gracefully when nvcc/hipcc absent. **G-9 + H-8 collective pin:** new `src/collectives/include/.../AdapterVersionPin.h` with `#error` directives enforcing NCCL ≥ 2.22 / RCCL ≥ 2.22 at C++ compile time (when libraries present) + `AdapterBuildInfo` constexpr tag for runtime introspection. New `scripts/probe_collective_libs.py` ctypes-resolves the 8-symbol surface (`ncclAllReduce`/`ReduceScatter`/`AllGather`/`Send`/`Recv`/`CommInitRank`/`GetVersion`/`GetErrorString`) and verifies installed version meets the pin. 34 new tests in `tests/unit/test_phase_ghi_lane2.py`. Toolchain pin consistency test asserts the 13.2.1 / 7.2.3 / 2.22 values are byte-identical across Python (`gpu_target.py` + `rocm_target.py`) / CMake (`TesseraToolchainPins.cmake`) / C++ (`AdapterVersionPin.h`). **Phase G/H/I hardware-free pre-work batch 2 landed (2026-05-11): G-2 + G-3 + G-4 + H-3 + H-4.** **G-3 schema extension:** `BackendKernelEntry` gained 8 optional fields (`cuda_arch_min`/`nvcc_version_min`/`wgmma_shape`/`cluster_size`/`mfma_shape`/`hipcc_version_min`/`expected_mfu`/`roofline_target`) with validation at construction (WGMMA only on NVIDIA, MFMA only on ROCm, MFU in [0,1]). Per-kernel tables `_NVIDIA_KERNEL_TILE_SHAPES`/`_NVIDIA_KERNEL_MFU`/`_NVIDIA_KERNEL_ROOFLINE`/`_ROCM_KERNEL_MFMA_SHAPES`/`_ROCM_KERNEL_MFU` codify the canonical Hopper bf16 tile `(64,256,16)`, FA `(64,128,16)` cluster `(2,1,1)`, Lightning `(32,32,16)`, CDNA bf16 MFMA `(32,32,8,1)`, attention `(16,16,16,1)`. **G-2 NVIDIA kernel inventory:** new `docs/nvidia_cuda13_kernel_inventory.md` enumerates 50+ planned fused kernels across SM_80→SM_120 (matmul/contraction/attention/fused chains/normalization/optimizer/KV-cache/RNG/spectral/recurrent families) with PTX assembly patterns documented; expanded `_NVIDIA_ARTIFACT` to match. **H-3 ROCm kernel inventory:** new `docs/rocm_mfma_kernel_inventory.md` enumerates the same surface across gfx90a/gfx940/gfx942/gfx950/gfx1100 with AMDGCN intrinsic patterns documented + CDNA 4 FP4 lanes called out; added `_ROCM_ARTIFACT`. **G-4 NVIDIA lit fixtures:** 8 new under `tests/tessera-ir/phase3/cuda13/` — `wgmma_matmul_bf16.mlir`, `wgmma_matmul_fp8.mlir`, `flash_attn_fwd_fa4.mlir`, `mla_decode_fused.mlir`, `deepseek_nsa_sparse_attention.mlir`, `lightning_attention.mlir`, `matmul_softmax_fused.mlir`, `swiglu_mlp_fused.mlir`, `tcgen05_blackwell_matmul.mlir`, `adamw_step_fused.mlir` — all FileCheck on emitted PTX patterns. **H-4 ROCm lit fixtures:** 6 new under `tests/tessera-ir/phase8/rocm_7_2/` — bf16/fp8/fp4 MFMA matmul, FA fwd, MLA decode, AdamW, RDNA 3 WMMA — all FileCheck on AMDGCN intrinsic patterns. 88 new tests in `tests/unit/test_kernel_inventory_and_lit_fixtures.py`. **Phase G/H/I hardware-free pre-work batch 1 landed (2026-05-11): G-1 + H-1 + I-1 + I-2 + I-3.** **G-1** pinned NVIDIA backend to **CUDA Toolkit 13.2 Update 1** (PTX ISA 8.6, NCCL 2.22, driver ≥555.85) — new `_CUDA_13_2_FEATURES` matrix (12 per-SM flags incl. wgmma_sparse / tcgen05_pair / cluster_launch / tma_swizzle_128b / cp_async_bulk / async_proxy_fence) + arch strings (sm_90a/sm_100a/sm_120a) + dtype matrix expanded for Hopper FP8 / Blackwell FP4/FP6/NVFP4. **H-1** created `compiler/rocm_target.py` pinned to **ROCm 7.2.3 / HIP 7.2.3** — `AMDArch` enum (GFX_90A/940/942/950/1100) + per-arch feature matrix (mfma_f8/mfma_xf32/mfma_f4/mfma_f6/lds_async_copy/cluster_mode) + per-arch MFMA instruction shape table (2→6→8 shapes across CDNA 2/3/4) + dtype matrix incl. CDNA 4 FP4/FP6 lanes; `capabilities.py` expanded with 5 new per-arch ROCm entries. **I-1** added 3 Metalium lit fixtures (softmax/layer_norm/rmsnorm) lowering through `dma + tile-local matmul` decomposition (matmul-shaped reduction since Metalium has no dedicated reduce intrinsic). **I-2** registered the `compileable` status + Metalium block-FP planned/gated target (`metalium_blockfp` with `bfp8`/`bfp4`); audit walker correctly classifies 2 planned-gated slots. **I-3** shipped `docs/metalium_kernel_inventory.md` — RISC-V grid mapping, per-core roles (BRISC/NCRISC/TRISC0/Packetizer), Phase 7 + Sprint I-1 shipped kernels, BFP planned/gated family, execution gates table. 52 new tests in `tests/unit/test_target_toolchain_pins.py`. **Sprints D + E landed earlier — full close-out plan complete (A0 → A → C → B → C2 → F → D → E).** **Sprint D — memory architecture:** shipped `MemoryShardSpec(mesh_axis, mode, eviction, persistence, bucket_fn)` in `tessera.sharding` with `MemoryMode.{BLOCK,REPLICATED,KEY_HASH,BUCKET}` partition strategies (KEY_HASH = FNV-1a deterministic content-addressed sharding); shipped `MemoryStateHandle(capacity, key_dim, value_dim, dtype, shard_spec, eviction)` in `tessera.cache` with `read/write/evict/clone/checkpoint/restore` mapping onto `STATE_COLLECTION_SPECS["memory_state"]`; added `tessera.memory.vmap_axis_for()` + `register_vmap_axis()` registry where the bank arg is tagged `"state"` so vmap never replicates or splits shared memory state. Registry impact: `memory_read/write/evict` flipped `batching_rule` and `sharding_rule` to **complete** (transpose_rule also for differentiable memory_read). 40 new tests in `tests/unit/test_memory_architecture.py`. **Sprint E — backend kernel manifest:** new `compiler/backend_manifest.py` synthesizes the per-op × per-target × per-dtype matrix from `capabilities.TARGET_CAPABILITIES` + Apple GPU MSL kernel inventory + x86 AMX backend. `BackendKernelEntry` dataclass + 4 statuses (`fused`/`reference`/`artifact_only`/`planned`). `metadata["backend_kernel_manifest"]` now attached to every OP_SPECS entry recording which backends ship kernels with which dtype coverage; `audit_backend_dtypes()` validates **0 unknown / 0 alias / 0 planned-gated dtypes across all 983 backend slots**. Shipped Apple GPU MSL kernels documented: matmul / softmax / softmax_safe / gelu / rope / flash_attn (all 3 dtypes f32/f16/bf16) + rmsnorm (f32 only). 33 new tests in `tests/unit/test_backend_kernel_manifest.py`. **Sprints F + C2 landed (2026-05-11)** — Sprint F shipped the public `tessera.dtype.Dtype` typed object (str-compatible; `.is_float`/`.is_integer`/`.bits` predicates; `|` operator for promotion) and `result_type(*dtypes, mode="standard"|"strict")` lattice (NumPy/JAX-style implicit promotion with `bf16+fp16→fp32`/`int32+fp16→fp32` safety nets; strict mode rejects mixed dtypes per JAX's `jax_numpy_dtype_promotion='strict'`); short aliases `canonicalize`/`is_canonical`/`is_planned_gated` match the doc's JAX-side vocabulary; 64 new tests. Sprint C2 promoted `numeric_policy(storage, accum, rounding, scale, quant_axis, deterministic, math_mode)` to a first-class tensor attribute — `NumericPolicy` dataclass + `_NUMERIC_POLICY_BY_NAME_FACTORIES` table attaches policies to **67 ops** across matmul / attention / spectral / normalization / stable-reduction / quantization families; **TF32 now routes through `math_mode='tf32'` on fp32 storage** (canonicalize_dtype rejects TF32 as storage). Audit walker `audit_canonical_dtypes()` scans the new `numeric_policy.{storage,accum}` slots and reports 134 canonical dtype references, 0 unknown / 0 aliased / 0 un-annotated planned-gated; 79 new tests in `tests/unit/test_numeric_policy.py`. **Sprints C + B landed (2026-05-11)** — Sprint C promoted 9 long-tail categories (control_flow / recurrent / sparse / linalg_decomposition / linalg_solver / moe / moe_transport / state_space / memory) from `partial` → `complete` in `_SEMANTIC_RULES_BY_CATEGORY`; **all 22 math/shape/dtype partials cleared in one focused pass** (no per-name overrides needed — each category's math is formally documented). Sprint B added `_GRAPH_IR_LOWERING_BY_CATEGORY` classifier covering every S2-S15 python-primitive category; **115 stub_required entries → 0** (~57 flipped to `registered` via OP_SPECS-compositional families, ~58 flipped to `not_applicable` for Python-runtime structures — pytrees / autodiff transforms / control flow / LR schedules / grad transforms / shard_map / custom_primitive escape hatches / memory primitives), and `missing` count went 4 → 1 (only selective_ssm pending a dedicated Mamba2 Graph IR op). **Sprint A long-tail (V/J)VP closure complete (2026-05-11)** — autodiff registry now reads **0 VJP-planned + 0 JVP-planned** across all 374 primitives (was 28 + 67 pre-A; +1 follow-up shipped `selective_ssm` Mamba2 closed-form JVP via forward-mode through the SSM recurrence — fp64-verified to ~4e-10 rel err against central-difference reference; basic + gated/initial-state + 1-D A paths covered by `tests/unit/test_h1_d3.py::TestSelectiveSSM`). Added 28 VJPs (`sin`/`abs`/`sign`/`floor_div`/`mod`/`cumprod`/`softmax_safe`/`dequantize_int8`/`quantize_int8`/`quantize_int4`/`calibration_observer`/`batched_gemm`/`factorized_matmul`/`einsum`/`conv2d`/`conv3d`/`conv_transpose`/`min_pool`/`adaptive_pool`/`fused_epilogue`/`qkv_projection`/`weight_norm`/`spectral_norm`/`segment_reduce`/`lamb`/`muon`/`grad_scaler_step`/`online_softmax_state`) and 66 JVPs across tensor_algebra (19 linear ops)/indexing (8)/layout_transform (5)/normalization (5)/softmax-family (3)/stencil (4)/pooling (2)/elementwise (4)/quantization (4)/long-tail (12). **Sprint A0 canonical-dtype enforcement landed (2026-05-11)** — `python/tessera/dtype.py` (15-name canonical set + 15-name planned/gated set + alias normalization + TF32 rejection), wired into `DistributedArray.from_domain` so every `tessera.zeros`/`ones`/`randn`/`Parameter` call canonicalizes at construction; `primitive_coverage.audit_canonical_dtypes()` + `assert_canonical_dtypes()` walkers gate registry dtype hygiene; 71 guard tests in `tests/unit/test_canonical_dtype.py`. **Spectral/FFT solver pass bodies landed** (LegalizeSpectral / SpectralMXP / TransposePlan / Autotune / LowerToTargetIR / DistributedFFT) with `ts-spectral-opt` end-to-end pipeline and 26 Python guard tests; closes the Spectral Production-Hardening line item. KV-cache Decision-#21 lowering shipped on x86 + Apple CPU/GPU + Metalium. Apple GPU runtime exports **26 C ABI symbols** across 9 kernel concepts × {f32, f16, bf16}. Pipelines: `tessera-lower-to-{rocm,metalium,apple_cpu,apple_cpu-runtime,apple_gpu,apple_gpu-runtime}`. Build pin: **LLVM/MLIR 22.1.6** (Homebrew `llvm`; was llvm@21 pre-2026-05-31). **Standalone compiler track (S-series) S0/S1 + S2–S15 + reasoning-model attention/RL + multi-pass contract-axis hardening + final-stage closure + long-tail VJP/JVP closure (collectives/recurrent/quant-STE/spectral/sparse/linalg) + spectral-pass-body landing + tessera-mlir diff CLI fix + Ch.7 Autodiff + AUTODIFF_SPEC.md refresh + S2–S15 sprint markers refreshed** — `primitive_coverage.py` (**374 entries × 12 axes; 75 at `explicit_semantic`**). **Three axes at zero missing: `masking_effect_rule`, `lowering_rule`, `tests`.** Remaining gates: `backend_kernel` 374 (Phase G/H/I universal), `sharding_rule` 156 (Phase G mesh), `transpose_rule` 123, `batching_rule` 102. **Eight axes at zero partial+planned across all 374 entries: `math_semantics`/`shape_rule`/`dtype_layout_rule` (Sprint C closed long-tail control_flow/recurrent/sparse/linalg/moe/state_space/memory), `vjp`/`jvp`, `masking_effect_rule`, `lowering_rule`, `tests`.** Graph IR lowering metadata: **285 registered / 88 N/A / 1 missing** (only selective_ssm pending dedicated Mamba2 Graph IR op — Sprint B closed all 115 stub_required entries). **Autodiff: 241 VJPs + 236 JVPs registered; 237 vjp-complete, 236 jvp-complete. **0 VJP + 0 JVP planned across the entire 374-primitive registry** (Sprint A closed long-tail + `selective_ssm` Mamba2 closed-form JVP shipped, fp64-verified to ~4e-10 relative error). **Registry post-Sprint-D + Phase G/H/I batches 1+2+3: 8 axes at zero partial/planned (math/shape/dtype/vjp/jvp/masking_effect/lowering/tests); memory_read/write/evict batching+sharding flipped to complete; 285 registered / 88 N/A / 1 missing on `graph_ir_lowering`. Sprint C2 attaches `numeric_policy` to 67 ops; Sprint E + G-3 attach `backend_kernel_manifest` with 8 optional toolchain/tile-shape fields to every backend-covered op; G-1 + H-1 pin NVIDIA→CUDA 13.2 U1 / AMD→ROCm 7.2.3; G-2 + H-3 + I-3 ship 3 per-target kernel inventory docs; G-4 + H-4 ship 14 lit fixtures across `phase3/cuda13/` + `phase8/rocm_7_2/`; G-5 ships 4 NVIDIATargetPipeline aliases in `tessera-opt`; H-2 ships `mfma_table.inc` generator + 22-shape C++ table; G-6/G-7/G-8 + H-6/H-7/H-8 ship `cmake/TesseraToolchainPins.cmake` + `validate_{nvcc,hipcc}_compile.py`; G-9 + H-8 ship `AdapterVersionPin.h` (NCCL/RCCL ≥ 2.22 `#error` enforcement) + `probe_collective_libs.py`.** Tests: 2,904 passing, 0 failures (2026-05-11 snapshot; see the 2026-05-22 lead above for the current 5,254-passing state).** Backend_kernel (Phase G/H/I) is now the sole remaining long-pole gate per Decision #25. See `docs/audit/coverage/COVERAGE_AUDIT.md`, `docs/audit/backend/BACKEND_AUDIT.md`, and `docs/apple_gpu_overview.md`.*
