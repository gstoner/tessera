...
> sub-decisions, and three correction banners (`Corrected 2026-08-02` on #5,
> `root-caused 2026-08-15` on #19, one `withdrawn` claim in #26a). It was
> previously headed "Do Not Revisit," which was never true of its own contents.
...
26. **The audit folder is the canonical "what's done / what's open" surface — follow its flow.** `docs/audit/` = one root audit + theme audits + generated dashboards + theme-local archives:
    ...
    2. **Drill into the theme audit:** `compiler/COMPILER_AUDIT.md`, `backend/BACKEND_AUDIT.md` (+ `backend/{apple,nvidia,rocm,x86}/`), `coverage/COVERAGE_AUDIT.md`, `domain/DOMAIN_AUDIT.md`, `roadmap/ROADMAP_AUDIT.md`.
