...
| `compiler/backend_manifest.py` | **Sprint E** — backend kernel manifest synthesizer (2026-05-11). `BackendKernelEntry(target, status, dtypes, feature_flags, notes)` dataclass with dtype canonicalization at construction; `manifest_for(op_name) -> list[BackendKernelEntry]` walks `TARGET_CAPABILITIES` + the Apple GPU MSL kernel inventory + x86 AMX backend to synthesize the per-op × per-target × per-dtype matrix. Four statuses: `fused` (optimized fused kernel ships — Apple GPU MSL / x86 AMX BF16 / Apple CPU Accelerate), `reference` (numpy/cblas fallback), `artifact_only` (Target IR lit-testable, execution gated on hardware — NVIDIA/ROCm/Metalium), `planned`. `all_manifests()` aggregates over OP_SPECS; `manifest_summary()` rolls up by (target, status) for dashboard reporting; `audit_backend_dtypes()` is the manifest-side dtype hygiene walker (parallel to `primitive_coverage.audit_canonical_dtypes`). Tests: `tests/unit/test_backend_kernel_manifest.py` (33). |
