...
| `compiler/backend_manifest.py` | **Sprint E + I-2** — backend kernel manifest synthesizer. `BackendKernelEntry(target, status, dtypes, feature_flags, notes)` dataclass with dtype canonicalization at construction; `manifest_for(op_name) -> list[BackendKernelEntry]` walks `TARGET_CAPABILITIES` + the Apple GPU MSL kernel inventory + x86 AMX backend + Metalium kernel inventory to synthesize the per-op × per-target × per-dtype matrix. **Five statuses (Sprint I-2 adds `compileable`):** `fused` (optimized fused kernel ships — Apple GPU MSL / x86 AMX BF16 / Apple CPU Accelerate), `reference` (numpy/cblas fallback), `compileable` (passes `ptxas`/`hipcc -S` validation but unexecuted — Sprint G/H follow-up state), `artifact_only` (Target IR lit-testable, execution gated on hardware — NVIDIA/ROCm/Metalium), `planned`. **Sprint I-2 Metalium entries:** `_METALIUM_KERNELS` ships `matmul/softmax/layer_norm/rmsnorm` artifacts (all bf16); `_METALIUM_PLANNED_GATED_KERNELS` ships a separate `metalium_blockfp` target for Tenstorrent block-FP (`bfp8`/`bfp4`) so the audit walker correctly classifies them as planned/gated rather than unknown. `all_manifests()` aggregates over OP_SPECS; `manifest_summary()` rolls up by (target, status); `audit_backend_dtypes()` is the manifest-side dtype hygiene walker. Tests: `tests/unit/test_backend_kernel_manifest.py` (33) + `tests/unit/test_target_toolchain_pins.py` (52). |
