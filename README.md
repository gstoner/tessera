...
| `tessera-nvidia-pipeline-{sm90,sm100,sm120}` (per-SM aliases) | implemented / lit-testable. Two sm_120 lanes execute on consumer Blackwell, and they are **not** the same thing: the canonical matmul compiles Graph→Schedule→Tile through `tessera-opt` and is packaged to PTX via `tessera-nvidia-opt` + `mlir-opt` (this is the compiled route, and the one the direction above builds out), while the older `ptx_emit.py` + `libtessera_nvidia_gemm.so` GEMM is a separate bootstrap emit/runtime lane that bypasses the IR pipeline. **The compiled lane requires `-DTESSERA_ENABLE_CUDA=ON` with `-DTESSERA_BUILD_NVIDIA_BACKEND=ON`.** Three configurations behave differently and are easy to conflate: with CUDA **on**, `tessera-opt` is a full driver carrying `core-tessera-ir` + `nvidia-backend` together and the scheduled lanes run; with the backend **on but CUDA off** it is the lean hardware-free artifact driver — the NVIDIA Target IR dialect *is* registered (that is the supported host-free artifact configuration Decision #19 exists for), but the core dialect and passes are not **registered** — `TesseraPM` is linked and PUBLIC-links `TesseraIR`/`TesseraScheduleIR`, so this is a registration gap, not a link gap — and scheduled pipelines are therefore unavailable; with the backend **off** the dialect is not built at all and a scheduled matmul fails at `--tessera-schedule-to-tile` with `requires the registered NVIDIA Target IR dialect`, having touched no GPU |
