...
python# The Tessera Promise: Write once, optimize everywhere
@tessera.function
...
└─────────────────────────────────────┘
                ↓
┌─────────────────────────────────────┐
│  Target IR (PTX, ROCm, WebGPU, CPU)  │ ← Platform-specific code
└─────────────────────────────────────┘
Key Components
...
python# Types carry numerical behavior
Tensor["B", "S", "D", 
       dtype=bf16,
...
python# Parallelism composes like functions
...
State Management
python# Stateful objects for sequences and optimization
...
class OptimizerState(StatefulObject):
    ...
    def step(self, grads: Dict[str, Tensor]) -> None:
        ...
        pass
Control Flow
python# Structured control flow for optimization
@tessera.function
...
c// Set numerical policy for reproducibility
...
Autotuning
python@tessera.kernel.autotune(
    space={
        "BM": [64, 128, 256],
        "BN": [64, 128, 256],
        "BK": [32, 64],
...
MCMC Inference
python@tessera.mcmc(algorithm="NUTS", num_samples=1000)
...
python# Built-in collectives with deterministic ordering
@tessera.function
def distributed_optimizer_step(
    ...
    return params
Fault Tolerance
python@tessera.checkpoint(interval=1000)
...
python# Tessera automatically optimizes:
@tessera.optimize(
    target="A100",
    objectives={
        "latency_p99": "<10ms",
...
pythonwith tessera.profiler() as prof:
    output = model(input)
...
python# Automatic bottleneck detection
...
Deployment
Export Formats
python# Multiple deployment targets
...
python# Production serving with batching
...
python# Define custom accelerator
...
Compiler Extensions
python# Add custom optimization passes
...
python# 1. Profile first, optimize second
...
python# Complete LLM training pipeline
@tessera.application
class LLMTraining:
...
IDE Support
python# Tessera provides rich IDE integration
