...
## v0.7
- **H100 WGMMA**: inline PTX path for `m64n64k16.bf16.f32` with a K-loop over M (guarded; falls back to FMA).
- **MI300X MFMA**: matching projection scaffold (gfx94*, guarded inline asm placeholder).
- **Pipelines**: staged cp.async hooks + `ldmatrix` path and LDSM toggle.
- **CSV metrics**: adds `tflops` and `tok_per_s` lines (`#metrics`) per autotune run using flops ≈ 2·M·Dh·S.


## v0.8 (H100-focused)
- Adds **power_attn_forward_wgmma_bf16_deg2** with inline PTX for `m64n64k16.bf16.f32` and a K loop over `M` (padded to 16).
- Hooks for **cp.async** pipeline + `ldmatrix` (swizzled SMEM); state-build remains FMA-correctness, projection accelerated.
- Example microbench: `examples/wgmma_sanity.cu` (compile with `nvcc -arch=sm_90 -DTESSERA_ENABLE_WGMMA=1`).


## v0.9
- **Autotuner** now sweeps **FMA** and **WGMMA** variants (H100) and records a `variant` column (`fma`/`wgmma`).
- **CSV** rows now include per-pick **ms**, **TFLOPs**, and **tokens/sec**; every candidate config is logged, the best is cached.
- **Runtime** dispatches the tuned variant directly (WGMMA when chosen and available; else FMA).
