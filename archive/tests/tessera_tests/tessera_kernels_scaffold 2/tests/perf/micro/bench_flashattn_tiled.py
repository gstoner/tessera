...
def run(B=1,H=16,S=1024,D=128, iters=50, warmup=10):
    ...
    for _ in range(warmup): flashattn_fwd_tiled(Q,K,V,None,None,is_causal=True)
    torch.cuda.synchronize()
    ts = []
    for _ in range(iters):
        t0 = time.perf_counter()
        flashattn_fwd_tiled(Q,K,V,None,None,is_causal=True)
        torch.cuda.synchronize()
...
if __name__ == "__main__":
    ...
    args = ap.parse_args()
    res = run(args.B,args.H,args.S,args.D,args.iters,args.warmup)
