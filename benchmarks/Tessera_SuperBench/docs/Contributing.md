...
3. Ensure the output follows the **Metric Definitions** schema. Compiler-backed
   Python benchmarks should use `BenchmarkRow` and `telemetry_for_row(...)`.
...
6. Use `runtime_status="artifact_only"` when compiler artifacts are available
   but timing comes from a reference implementation.
