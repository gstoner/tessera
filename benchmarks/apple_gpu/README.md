...
- **EBT-tiny break-even sweep**: opt-in via `--ebt-sweep` — runs a 7-point `(B, K, D, T)` ladder. **First native win at `B=32,K=128,D=512/T=64` (6.5×); peak 20.3× at `B=64,K=128,D=1024/T=256`** after the fused single-dispatch `ebm_refinement` MSL kernel landed.
- **Integration via `tessera.ga.*` / `tessera.ebm.*`**: 9 ops route through [`tessera._apple_gpu_dispatch`](../../python/tessera/_apple_gpu_dispatch.py) transparently — `ga.inner` / `ga.exp_mv` / `ga.rotor_sandwich` / `ga.norm` and `ebm.inner_step` / `ebm.refinement` / `ebm.self_verify` / `ebm.energy_quadratic` / `ebm.decode_init`. The workload benchmarks now call the public APIs (no benchmark-local ctypes for those ops).
