...
def _measure(route: Route, oracle: Any, *, reps: int) -> dict[str, Any]:
    wall: list[int] = []
    device: list[int] = []
    correct = True
    resources: Any = None
    for _ in range(reps):
        ...
        correct = correct and bool(np.allclose(output, oracle, rtol=5e-4, atol=5e-4))
...
def _row(case: Case, route: Route, trials: list[dict[str, Any]], *,
    ...
    return {
        ...
        "telemetry": {
            "end_to_end_median_ns": int(statistics.median(
    ...
    try:
        ...
        for case in cases:
            ...
            for trial in range(trials):
                ...
                for route in order:
                    measured[route.name].append(_measure(route, case.oracle, reps=reps))
            rows.extend([
...
def build_strict_ledger(reports: list[dict[str, Any]], *, valid_days: int = 30) -> dict[str, Any]:
    ...
    for decision in stable["decisions"]:
        selected = decision.get("selected_route")
        if selected is None:
            continue
        evidence = decision["route_evidence"].get(selected, {})
        decisions.append({
            **decision,
            "selected_evidence": {
                "provenance": (
                    "native_gpu" if evidence.get("placement_and_numerical_proof")
                    else "reference_cpu"),
                "correctness": bool(evidence.get("placement_and_numerical_proof")),
                "device": decision["device"],
                "timing_domain": decision["timing_domain"],
            },
        })
    return {
        ...
        "promotion_rules": stable["promotion_rules"],
        "decisions": decisions,
    }
