...
`/dev/kfd`**, ROCm 10.0. Source commit `1c8f7551` (clean tree), recorded
2026-09-26. Sync `WSL-TIMING-ADMISSION-2026-09-26`.
...
bound is 9.71x at confidence 0.980 (see `admission.json`). The previous gfx1151
...
- **Calibration per process:** in the same process, each window is bracketed by
  ...
  - The clean image itself is never modified. Stamping the kernel directly was
    measured to change its codegen: 2512 → ~1230 instructions, running 2.4x
    faster.
...
- **Per-process agreement** (all 18 admissible):
  - Serial: device clock 0.28–0.44% below the HIP event.
  - Cooperative: 1.95–3.23% below. That window is ~10x shorter, so the markers
    are a larger share of it.
  - Marker-bracketed / plain duration ratio: 0.9885–1.0283, within the
    two-sided 5% band.
