...
def render(rows, counters):
    out = []
    # Disqualified rows (null latency: WRONG / EXEC_FAIL / COMPILED_OUT /
    # SKIPPED_BY_PROBE) are excluded from every numeric table and surfaced in
    # their own section, with the rejection reason retained.
    disq = [r for r in rows
            if r.get("latency_ms") is None
            or r.get("status") in check_consistency.DISQUALIFIED]
    rows = [r for r in rows if r not in disq]
    if not rows:
        return ("# PTX Tensor-Core GEMM Study — Results\n\n"
                "No measured rows — every candidate was disqualified.\n\n"
                + _disq_section(disq))
...
def _disq_section(disq):
    if not disq:
        return ""
    lines = ["## Disqualified / skipped candidates\n",
             "Excluded from the numeric tables above; reason retained "
             "(a fast-but-wrong or probe-rejected kernel is never reported as a "
             "throughput number).\n",
             "| kernel | dtype | N | status |", "|---|---|---|---|"]
    for r in sorted(disq, key=lambda r: (r.get("dtype", ""), str(_key_n(r)))):
        lines.append(f"| `{r.get('kernel')}` | {r.get('dtype')} | "
                     f"{_key_n(r)} | {r.get('status', '?')} |")
    return "\n".join(lines) + "\n"
