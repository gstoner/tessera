#!/usr/bin/env python3
"""Scoped dynamic public frames and asynchronous capture device proof."""
import argparse
import ctypes as ct
import hashlib
import json
...
def main():
    ...
    original_init = BoundNativeGPUStorage.__init__
    ...
    try:
        ...
        for length in (0,2,4):
            ...
            with program.submit(stream.value,upload(x),upload(seed),shape,upload(np.array([length],np.int64)),scoped=True) as frame:
                assert not hasattr(frame,'results')
                poll(frame)
                with frame.read(stream.value) as results:
                    np.testing.assert_array_equal(download(results[0]),2*x[:length])
                frame.retire(stream.value)
                deadline=time.monotonic()+20
                while not frame.poll_retired():
                    if time.monotonic()>deadline: raise TimeoutError('public retirement failed')
                    time.sleep(.001)
        ...
        matrix_backward=materialize_ad_public_results(dynamic.replace('?xf32','?x?xf32'),compiler=args.compiler,
            llvm_bin=Path('/usr/lib/llvm-23/bin'),backend=args.backend,chip=chip,capacity=64,input_capacity=64,role='backward')
        for dims in ((8,8),(1,64),(0,64)):
            x=np.arange(1,65,dtype=np.float32)
            with matrix_backward.run(upload(x),upload(np.ones(64,np.float32)),
        ...
        def checked_init(self, package):
            original_init(self, package)
            def forbidden(): raise AssertionError('asynchronous capture used a context barrier')
            self._sync = forbidden
        BoundNativeGPUStorage.__init__ = checked_init
        for iteration in range(8):
            frame=pair.capture_async(streams[0].value,upload(np.full(4,2,np.float32)))
        ...
        BoundNativeGPUStorage.__init__ = original_init
    ...
    names=['src/transforms/lib/NativeTapeToGPUPass.cpp','python/tessera/compiler/native_public_result.py',
           ...
           'python/tessera/compiler/native_storage_contract.py','python/tessera/compiler/native_module_retirement.py']
    ...
    print('Scoped dynamic frames and asynchronous capture passed')
