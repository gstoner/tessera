#!/usr/bin/env python3
"""Owning-device correctness for snapshot isolation and shared-input SSD DAG VJP."""
import argparse
...
def main():
    ...
    try:
        ...
        with ResidentObjectPool(4, 8, 1, stream=s0, **options) as pool:
            ...
            snapshot = pool.snapshot(s0)
            with snapshot.read(s1) as historical:
                ...
                with pool.read(s1) as views:
                    ...
                    assert state[0, 0] == 2
                np.testing.assert_array_equal(read(historical[0])[:, 2], [1, 1, 1, 1])
            snapshot.close()
            heap_binding = pool._seeded.package.binding_digest
        rng = np.random.default_rng(744)
        shapes = [(3, 2, 2), (3, 2), (3, 2, 2), (3, 2, 2), (2, 2, 2)]
        values = [rng.uniform(-.5, .5, shape).astype(np.float32) for shape in shapes]
        ...
        with ResidentSSDProgram(logical, **options) as program:
            ...
            def dag(*x):
                first = program(*x)
                second = program(first, *x[1:])
                return program(second, x[1], first, x[3], x[4])
            trace = ResidentSSDTrace(dag)
            ...
            def composed(v):
                first = reference(v)
                second = reference([first, *v[1:]])
                return reference([second, v[1], first, v[3], v[4]])
        ...
        args.output.write_text(json.dumps(dict(schema=1, backend=args.backend, chip=options['chip'],
            ...
            traced_ssd_calls=3, public_input_gradients=5, snapshot_read_scope_during_live_sweep=True,
            sum_bindings=[b.package.binding_digest for b in program._sum_bindings.values()], gradient_max_abs_errors=errors,
