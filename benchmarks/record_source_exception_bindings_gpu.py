...
class DomainError(RuntimeError):
    def __init__(self,payload):
        _constructor_calls.append(1)
        super().__init__(payload)
        self.domain='native-device-completion'
...
def checked_cube(x):
    ...
    except ValueError as inner:raise DomainError(x*x) from inner
...
def main():
    ...
    try:
        for fn in (checked_cube,loop_error):
            ...
            program=materialize_source_vjp(source,compiler=args.compiler,llvm_bin='/usr/lib/llvm-23/bin',backend=args.backend,chip=chip,capacity=4,exception_types={"DomainError":DomainError})
            for asynchronous in (False,True):
                for value in (2.,-2.):
                    ...
                    try:
                        ...
                        assert should_fail and type(error) is (DomainError if fn is checked_cube else RuntimeError),error
                        if fn is checked_cube:assert error.domain=='native-device-completion'
                        constructor_count=len(_constructor_calls)
                        ...
                        if asynchronous:
                            ...
                            assert len(_constructor_calls)==constructor_count
                        rows.append(dict(family=fn.__name__,asynchronous=asynchronous,value=value,exception_matched=True,derivative_exposed=False,exception_class=type(error).__name__))
                    finally:
        ...
        assert len(_constructor_calls)==2
        retained=to_native_source_ir(trace(retained_exception,np.ones(1,np.float32),source_control_flow=True,max_steps=2,
            source_error_specs=(((1,),'f32'),)),autodiff='reverse')
        retained_program=materialize_source_vjp(retained,compiler=args.compiler,llvm_bin='/usr/lib/llvm-23/bin',backend=args.backend,chip=chip,capacity=4,exception_types={"DomainError":DomainError})
        try:
    ...
    sources=['python/tessera/compiler/source_exception_heap.py','python/tessera/compiler/source_control_flow.py','python/tessera/compiler/trace.py','python/tessera/compiler/native_public_result.py','python/tessera/compiler/native_source_state.py','python/tessera/compiler/native_module_retirement.py','python/tessera/compiler/native_gpu_storage.py','python/tessera/compiler/native_gpu_tensor.py','src/transforms/lib/NativeTapeToGPUPass.cpp','src/transforms/lib/AutodiffPairedPass.cpp','benchmarks/record_source_exception_bindings_gpu.py']
