#!/usr/bin/env python3
"""Native source state generations, without in-place external mutation."""
import argparse
import ctypes as ct
import hashlib
import json
...
def main():
    ...
    args.output.write_text(json.dumps(dict(backend=args.backend,chip=chip,execution_kind='native_gpu',
        rows=rows,in_place_mutation=False,promotion_eligible=False,
    ...
    print('Two device state generations matched; original input unchanged')
