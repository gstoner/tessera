...
| MetalPerformancePrimitives cooperative ops — `mpp::tensor_ops::matmul2d` **and `convolution2d`** | Apple's matrix-unit GEMM **and conv**, both with a `cooperative_tensor` destination for in-register bias/activation epilogues | matmul2d → M6/M7/M8; conv → P8 (f16/bf16 conv on the matrix units via **GPU im2col** + the matmul2d epilogue; opt-in — materialized im2col still loses to MPSGraph's fused conv ~0.55–0.77×, so no default-win). Native `convolution2d` op cracked single-tile; multi-tile tiling undocumented (integration review P8) |
