...
| 5 ✅ | **`mla_decode`** | **DONE (compressed-KV phase)** — promoted the host-reference `mla_decode_f32` symbol to a real on-GPU kernel: one cached MPSGraph fuses the latent down-projection `c = X@Wdkv`, the K/V up-projections `K = c@Wuk` / `V = c@Wuv`, and attention `O = softmax((Q@Kᵀ)·scale)@V`, folding the B·S_q query rows to a single matmul dim (K/V shared across batch — no batched-broadcast), fp32 accumulation, host reference fallback. `tests/unit/test_apple_gpu_mla_decode_gpu.py` (6) + the existing 30 MLA tests. **Decoupled-rope phase** (RoPE-carrying dims + per-head split) is the remaining follow-on. | Med–High |
