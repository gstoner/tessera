...
|---|---|---|---|---|
| `flash_attn` core (MHA, scale+causal) | ✅ compiled (WMMA) | ✅ device-verified (mma.sync)¹ | ✅ native | ✅ native |
| `flash_attn` variants (GQA/MQA/sliding/softcap) | ✅ compiled (WMMA) | ✅ **device-verified (in the emit/ FA lane)²** | ✅ native | ✅ native |
| **`attn_bias`** | ✅ compiled (WMMA, #328) | ✅ device-verified (dense additive bias) | ✅ native (pre-softmax add) | ✅ native |
...
**¹ NVIDIA flash-attention — status.** A real `mma.sync` tensor-core FA
kernel exists in the `emit/` plugin framework
([`python/tessera/compiler/emit/nvidia_cuda.py`](../../../python/tessera/compiler/emit/nvidia_cuda.py):1955
`_synthesize_mma_attn_cuda`, candidate `NvidiaMmaAttnCandidate` :2051) — nvcc-compiled
...
gated by the F4 oracle. It is now **device-verified**: the compiler-emitted CUDA
Flash Attention lane is recorded in `_NVIDIA_DEVICE_VERIFIED_JIT`
(`backend_manifest.py:2158`) with forward + VJP, causal/sliding-window masking,
dense additive `attn_bias`, and logit soft-cap, and executes via
`runtime.launch()` on sm_120. `benchmarks/baselines/nvidia_sm120_hot_paths.json`
now carries a `flash_attention` (`mma_sync_attention`) baseline row alongside
matmul, `fused_gemm_bias_gelu`, and `quantize`. It rides the `device_verified_jit`
tier (nvcc-compiled + launched) rather than the `hardware_verified` C-ABI tier
that `_NVIDIA_HARDWARE_VERIFIED` (`backend_manifest.py:2111`) still reserves for
matmul's packaged GEMM symbol; it does not use the PTX launch bridge (that's
GEMM-only).

**² NVIDIA FA variants (GQA/MQA/sliding/softcap) — present.** The `emit/` FA
candidates now implement the full attention ABI
(`Q,K,V,bias,O,B,Hq,Hkv,Sq,Sk,D,Dv,scale,causal,wl,wr,softcap` —
`emit/nvidia_cuda.py:162` scalar, `:1955` mma): explicit `Hq`/`Hkv` GQA/MQA
head-group mapping, `wl`/`wr` sliding-window bounds, a `softcap` logit-softcap
transform, and an optional dense `bias`. The device-verified manifest row lists
all four variants plus `attn_bias` (`backend_manifest.py:2158-2162`). (ROCm and
x86 carry all four variants too.)
...
| CUDA emit/ FA kernel | `emit/nvidia_cuda.py:1955` (mma) / `:162` (scalar); arbiter `emit/candidate.py`, `emit/autotune.py` |
...
> **Largely landed.** The `emit/` mma.sync FA lane is now recorded in
> `_NVIDIA_DEVICE_VERIFIED_JIT` (device-verified) and has a committed
> `flash_attention` baseline row in `nvidia_sm120_hot_paths.json`. The steps
> below are retained for provenance; remaining work is keeping the perf ratchet
> and MASTER_AUDIT prose in sync with the measured baseline.

The `emit/` mma.sync FA lane executes and is now recorded in the manifest with a
committed baseline — earlier it was invisible to the manifest, the execution
matrix, and the perf ratchet, so MASTER_AUDIT's prose and the recorded status
disagreed. Close the remaining gap (bookkeeping + keeping the hardware proof
current, not a kernel build):
...
| **CUDA** | ✅ **landed** — the bias operand is wired into the `emit/` FA kernels (`_synthesize_mma_attn_cuda` + `_synthesize_attention_cuda`); the C ABI carries an optional dense `bias` (`[B,Hq,Sq,Sk]`) and the device-verified manifest row lists `attn_bias`. Remaining: confirm the committed F4 accuracy budget + baseline. | **sm_120-gated** |
