...
| [compiler/AUTODIFF_EXECUTION_PLAN.md](compiler/AUTODIFF_EXECUTION_PLAN.md) | Consolidated native autodiff integration and proof gates — make differentiation a compiler request with a native fwd+bwd execution path and a per-family × per-target proof ledger, replacing implicit-tape-reported-as-compiled. |
| [compiler/compiler_enhancement.md](compiler/compiler_enhancement.md) | External assessment (CAKE compiler–agent co-design, arXiv:2608.12629) + statistical audit of its clean-start A/B (the headline speedup is not significant at n=3; convergence is), + three in-tree findings: the Tile sync/memory surface is untyped where a schedule verifier needs it, `WarpSpecLegalityPass` is type-blind by construction, and no author can state a schedule. Scopes Phase 1 (typed + derived sync surface) and Phase 2 (roles / producer-consumer sets); global sequencing defers to `INTEGRATED_COMPILER_PLAN.md`. `plan` role, `plan_state: open` — not a status surface. |
