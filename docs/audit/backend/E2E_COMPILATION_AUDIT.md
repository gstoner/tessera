...
| 13 | **APPLE-NATIVE-E2E-2** | P3 | landing | Apple native breadth and fleet proof beyond the bounded E2E-1 contracts | Completed slices package static rank-2 f16/bf16 Apple CPU matmul/gemm through existing BNNS ABIs, static rank-2 f32 CPU last-axis softmax through the owned stable row-softmax ABI, and f32/f16/bf16 GPU GELU through owned static and rank-2 dynamic `Elements:int64` ABIs. The low-precision GELU slice includes canonical Graph-IR dtype admission, raw 16-bit descriptor bindings, exact-device execute/compare/replay and rejection proof, plus a 50-repetition two-shape benchmark corpus. GPU reduced SVD has a semantic adapter ABI with ordered `(U,S,Vh)` bindings for static tall rank-2 and batched rank-3 contracts; Apple ReplaySSM has a session-private lifecycle descriptor with stable resource identity, complete resident-ring bindings, ordered transitions, and drain-before-release teardown. The local CPU candidate audit finds no further owned static non-linalg ABI. Composite preparation has a host-free MLPKG ownership/cache schema, but package execution still requires a Metal-4-capable exact device and reflected package. Further non-linalg CPU, broader dynamic families beyond the explicitly proven GELU/popcount contracts, and independently sealed second-device/fleet evidence remain open. It must not relabel retained/reference routes as native before a static ABI or a new explicit ABI family plus exact-device oracle exists. |
