...
| 6 | **APPLE-E2E-1** | P1 | landing / partial Level C | Canonical compilation selects the executable Apple GPU pipeline and defaults eligible static contracts to their hash-bound native image plus descriptor. Static f32/f16/bf16 batched GEMM; strict and mask/reference-KL/entropy PPO; EBM energy/Langevin/refinement/partition; cl30 Clifford geometric product; and static/batched Cholesky, Cholesky-solve, and triangular-solve execute/compare through exact-target hooks. Composite, unsupported, and multi-result routes retain explicit legacy states. Existing Metal selectors and schedules do not change without Apple evidence. |
...
| 11 | **APPLE-CPU-E2E-1** | P2 | landing / partial Level C | Apple CPU breadth | Canonical descriptors cover static f32 rank-3 BMM plus single-result Cholesky, triangular solve, and Cholesky-solve C ABIs. Multi-result LU/QR/SVD and unsupported/dynamic contracts retain their explicit value/reference routes. |
