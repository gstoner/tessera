...
| **R2 🟡** | **Mechanism proven** — `AppleGPUEncodeSession` + `ts_enc_begin` / `tessera_apple_gpu_bmm_dev_f32_enc` / `ts_enc_commit_wait`: a chain of device-resident ops `encodeToCommandBuffer:`s into **one** command buffer, committed + waited **once**. Dependent ops are correctly ordered by Metal's automatic hazard tracking (verified: 5-deep dependent chain vs numpy). **Measured 4.38× on a 32-op chain (31 syncs → 1 commit)** — the per-op-sync cost the architecture targets, removed with no extra uploads. `tests/unit/test_apple_gpu_encode_session.py` (7) + `benchmarks/apple_gpu/benchmark_encode_session.py`. **Decode-step ops landed:** device + encode variants for **rowop** (layer_norm / rmsnorm / softmax / log_softmax) and **gumbel** (sample), plus `DeviceTensor.reshape_view` (non-owning alias so shape changes between ops stay resident). A **full per-token decode step — rmsnorm → bmm → softmax → bmm → bmm → gumbel — now encodes into ONE command buffer** with zero host reads mid-chain, validated end-to-end vs numpy. `tests/unit/test_apple_gpu_resident_decode_step.py` (6). | Large | **High** |
| **R3 ✅** | **DONE** — `tessera.cache.ResidentMLADecoder`: a single-sequence decode loop where each step (rmsnorm → bmm(Q,Kᵀ) → softmax → bmm(·,V) → bmm(·,W_logit) → gumbel) runs entirely on-device in **one command buffer**, the model weights stay **resident across steps** (uploaded once — `weight_uploads == 1` over the whole loop), and **only the sampled token id reads back** (logits/activations never round-trip; the Gumbel sampler consumes the resident logits handle). numpy fallback for portability. `tests/unit/test_apple_gpu_resident_decode_loop.py` (6, incl. an 8-step growing-cache loop vs numpy). | Med | Med |
| **R4 ✅** | **DONE (contiguous latent cache)** — `tessera.cache.ResidentLatentKVCache`: `c_kv`/`k_rope` live in resident device buffers; `append` writes only the new token **in place** (host write through the unified-memory view — no upload, no realloc), and the window is a **zero-copy prefix view** (`DeviceTensor.prefix_view`) that feeds a device-resident bmm with no host copy. Single allocation across all appends. `tests/unit/test_apple_gpu_resident_kv.py` (8). **Device-resident block-paged cache landed:** `tessera.cache.ResidentBlockPagedKVCache` — a resident physical block pool, per-sequence block tables, on-demand alloc / free-and-reuse, and an **on-GPU non-contiguous block-table gather** (`tessera_apple_gpu_gather_blocks_dev_f32`, MPSGraph `gather` along the block axis) that assembles a sequence's scattered blocks into a contiguous resident `DeviceTensor` window — no host round-trip; the window feeds a device bmm directly. `tests/unit/test_apple_gpu_resident_block_paged.py` (7). **The full GPU-resident plan (R0–R4 + block-paged) is now implemented.** | Large | Med–High |
