...
| 29 | APPLE-DEVICE-EVENT-1 | **open** | The value-descriptor C ABI lane does not populate the Metal command-buffer device timer. After a descriptor launch, `tessera_apple_gpu_last_dispatch_device_time_ns()` returns `-1` and `tessera_apple_gpu_last_dispatch_timing_source()` returns `0`, even when read through the runtime's own handle; only the tile/dispatch lane sets them (`apple_gpu_runtime.mm`). Consequence: the sealed `apple_gpu`/`apple7` fleet packet can only claim `kernel_wall` + `end_to_end`, the weaker of the two admissible domain pairs — a host clock relabelled `device_event` would be fabricated evidence, so the recorder refuses and records the reason in `resources.json`. Closing this means feeding the same completed-command-buffer `kernelStart`/`kernelEnd` interval from the descriptor submit path, then re-recording the packet with `required_timing_domains = ["device_event", "end_to_end"]`. Independent of `CAP_DISPATCH_BOUNDARY_SAMPLING` (bit 4), which this M1 Max does not report — the command-buffer interval needs no counter sampling. |
