...
**Apple outcome: not applicable (architecture-specific).** Apple GPU timing uses the Metal timestamp heap / `GPUStartTime`, and the Mac is not a WSL host; no Apple gate changed.
...
**Measured: no effect on small dispatch.** Two copies of the runtime were
rebuilt from `build/`'s commands, as-is and `-O2` (1.80 MB → 1.15 MB), and
loaded through `TESSERA_APPLE_GPU_RUNTIME_LIB` (confirmed by `vmmap`). Apple
GPU matmul, add and softmax at 64² and 512², 200 calls each, two alternating
runs per build, gave medians that agree within run-to-run noise (for example
matmul 512: 0.378/0.419 vs 0.400/0.378 ms). Metal shaders are compiled by the
Metal compiler at its own level.

**Not measured:** host-heavy Apple paths, such as FP8/FP4 host packing for
`matmul2d`, which the queue records as costing 4–10× the matmul. Those may
still carry `-O0` cost. No Apple packet is claimed stale on this evidence
alone.
...
The shared example moves two residual adds through its existing backend API
and adds a score-scaling method; Apple retains its existing host score-scale
glue. Apple parity is a focused host regression, not a new Metal proof.
...
`GFX1201-SPECTRAL-LIVE-ARCH-2026-09-23`. The Python candidate probe now
checks ROCm FFT output numerically and binds source compilation to the live
HIP architecture. No Metal FFT package, Apple ABI, or Apple device behavior
changes; Apple parity is not applicable and its selector is unchanged.
...
Owner ROCM-MXFP4-W4A8-1 / IKF-1; sync
`GFX1201-PACKED-VECTOR-PAIR-2026-09-23`. Not applicable to Metal: the
opt-in fragment-word vector loads and v4 HIP-event/HSACO benchmark schema
are RDNA4-specific. Physical ABI, shared numerical policy, and Apple
selection are unchanged; no Apple performance claim transfers.
...
Owner ROCM-MXFP4-W4A8-1 / IKF-1; sync
`GFX1201-MXFP4-DEVICE-GRAPH-2026-09-23`. Not applicable to Metal:
HIP graph capture, RDNA4 HSACO, stream-owned pointers, and the graph
benchmark schema are AMD-specific. Shared Graph/Target IR, MXFP4
numerical policy, and Apple execution state are unchanged.
...
`GFX1201-DTYPE-EXACT-EXECUTOR-2026-09-21`. Not applicable to Apple physical
...
Metal IR, ABI, dtype, numerical, runtime, or device claim changes.

## ROCm MXFP4 physical-contract sibling assessment — 2026-09-21

Owner ROCM-MXFP4-W4A8-1; sync
`ROCM-MXFP4-PHYSICAL-CONTRACT-2026-09-21`. The new module is a host reference
for a gfx1201-specific W4A8 physical route and does not alter canonical dtypes,
Graph IR, Metal auxiliary-plane binding, Apple numeric policy, or execution.
Its distinction between exact per-group scaling and an approximate row-reference
fold is reusable evidence, but no Metal lowering or device proof is inferred.
...
Not applicable to Apple physical execution: no Apple IR, ABI, dtype, numerical,
or runtime contract changed, and no ROCm proof transfers to this backend. The
ROCm-only post-launch selected-HIP-device attestation fix does not affect Metal
attestation. The shared dtype-flow audit gained only the missing exact
`rocm_gfx1201` ISA row; Apple rows and generated states are unchanged.
...
APPLE-SIMDGROUP-IR-1 (2026-09-15, owner E2E-REAL-6; recorded here 2026-09-25 — it
was previously noted only in `CLAUDE.md`): the Apple dialect gained its first
...
| 17 | APPLE-DTYPE-1 | **partial — SDK gate lifted 2026-09-14** | Was "blocked — SDK" until the macOS 27 Metal tensor path shipped. FP8 E4M3/E5M2 and FP4 E2M1 now execute as Metal 4 `matmul2d` operands with fp32 accumulation on the owning M1 Max — **emulated**, 0.77–0.90× matched fp16 at 2048³, not an acceleration claim ([§macOS 27 / Metal 4.1 low-precision validation](#macos-27--metal-41-low-precision-validation--2026-09-14), APPLE-MATMUL2D-1). Still open: block-scaled MX/NVFP4 (UE8M0/NVFP4 scale planes are accepted by the descriptor bridge, nothing executes through them), GPU operand packing, and any simdgroup-fragment FP8/FP4 route (row 25 refusals stand). Keep older-host int4/int8/f16/bf16 regression coverage. |
...
Not applicable to Metal physical package generation: the new native package
...
Follow-up required: shared attention descriptor projection preserves the existing Apple ancestry boundary. No MSL dynamic public-frame ABI, asynchronous capture or Metal execution proof is supplied by this loop. CUDA/HIP ownership results do not transfer. No Apple runtime source changed.
...
Follow-up required: no Apple effectful source runtime or multi-status MSL tape ABI is supplied. Shared SSA/status design applies; no Metal execution proof transfers.
...
Follow-up required: the source completion/state contract has host implementation, but no MSL state binding or Metal fan-in packet is supplied here. CUDA/HIP status and pool proof does not validate Apple execution.
...
Follow-up required: shared source/state contracts have no MSL consumer or Metal proof in this increment.
...
Follow-up required: no MSL consumer for this owned-state ABI was implemented or measured. CUDA/HIP proof does not establish Metal support.
...
Sequencing: [live plan](../../compiler/INTEGRATED_COMPILER_PLAN.md).
Follow-up required: MSL source-state binding is absent; CUDA/HIP evidence does not establish Metal support.
...
Sequencing: [live plan](../../compiler/INTEGRATED_COMPILER_PLAN.md).
Follow-up required: no MSL consumer for writable source views or typed exception payloads.
...
Sequencing: [live plan](../../compiler/INTEGRATED_COMPILER_PLAN.md).
Follow-up required: no MSL source-state binding; shared ownership changes do not establish Metal execution.
...
Sequencing: [live plan](../../compiler/INTEGRATED_COMPILER_PLAN.md).
Follow-up required: no MSL source-state/exception binding or Apple device proof is supplied.
...
Sequencing: [live plan](../../compiler/INTEGRATED_COMPILER_PLAN.md).
Follow-up required: no Apple MSL implementation or device evidence for these source/AD contracts.
...
Sequencing: [live plan](../../compiler/INTEGRATED_COMPILER_PLAN.md).
Follow-up required: no MSL source-state or exception-aware VJP binding; CUDA/HIP proof does not establish Apple execution.
...
Sequencing: [live plan](../../compiler/INTEGRATED_COMPILER_PLAN.md).
Shared compiler/API changes assessed; no Apple device proof for this increment. MSL binding of these signed maps and asynchronous source products remains follow-up work.
...
**One Apple defect fixed here, and it was misfiled as an OS regression.** `test_f16_cond_native_selects_branch` has been red since the macOS 27 upgrade and was recorded as "MPSGraph f16 cond off 10%". It is ours. `mpsg_build_branch` built every authored-branch op at the **boundary** dtype, so an f16 cond branch computed its internals in f16 — against this ABI's own policy of a low-precision boundary with f32 internals, which the authored-*package* path in the same file states in its comment. The tell is that the parts were fine and the composition was not: in an f16 cond branch `matmul` is accurate to 2e-3, `sigmoid` to 3e-4, `relu` to 3e-4, and `silu(x @ w)` — exactly those composed — was **34% off**. Writing it as an explicit `mul(m, sigmoid(m))` reproduced the error, so it was not the `silu` node, and the identical graph through the bf16 boundary (which already upcast) stayed inside its boundary rounding. The fix casts once at the branch edges. Standing lesson for this queue: do not file a numeric red as a vendor regression before bisecting our own lanes — check whether the individual ops are accurate and only their composition is wrong, which points at our precision policy, not theirs.

Changing the runtime invalidated both sealed artifacts, as the standing rule says. Both were **re-measured, not re-stamped**: `benchmark_legacy_retune.py --profile extended --runs 5` reproduced all 24 decisions and 6 ineligible rows with identical key sets, selected routes and incumbents (only `runtime_fingerprint` moved), and the e2e packet was re-recorded through `record_apple_packet.py --lane apple_gpu`, which refuses a modified tree and so required the runtime fix to land first. **Still open on this box:** the f16 `sliceTensor` → bf16 rounding, which *is* MPSGraph behaviour on 27.0 and needs a slice route that avoids it.
