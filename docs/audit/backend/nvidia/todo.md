...
|---:|---|---|---|---|
| 1 | NVIDIA-CALIB-1 | Calibrate the hardware-free scores against the committed sm_120 corpus | Re-derive CUDA shared-memory constants, then compute per-family and per-shape rank correlations without transferring AMD bank assumptions. | The analysis records score definitions, corpus identity, rank correlations, and a retain/reject verdict; no new device run is required. |
