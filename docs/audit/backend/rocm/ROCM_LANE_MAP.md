...
| Exact MXFP4 native package | Named packed W4A8 Graph container contract → Schedule → scaled-partial Tile carrier → gfx1201 Target ABI, plus scalar HIP oracle or load-batched LLVM/ROCDL FP8-WMMA kernel → HSACO + five-buffer launch descriptor | `gfx1201` only: scalar and WMMA ABIs pass two bit-exact shapes on RX 9070 XT. | The carrier binds the packed form to the proved WMMA ABI without declaring MXFP4 a first-class Graph storage dtype. The Target directive still needs its in-pipeline binary materializer before this becomes the default public selector. |
