...
| Exact MXFP4 native package | Named packed W4A8 Graph container contract → Schedule → scaled-partial Tile carrier → self-contained gfx1201 Target ABI → strict materializer → load-batched LLVM/ROCDL FP8-WMMA HSACO + five-buffer launch descriptor; the scalar HIP package remains the oracle | `gfx1201` only: scalar and WMMA ABIs pass two bit-exact shapes on RX 9070 XT; the generic carrier-to-package route has its own exact execution gate. | The carrier binds only the named packed form to the proved WMMA ABI without declaring MXFP4 a first-class Graph storage dtype. Logical and approximate Target directives remain fail-closed; selector tuning is still open. |
