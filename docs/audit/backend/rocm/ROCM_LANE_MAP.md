...
| Exact MXFP4 native package | Below-Graph physical contract → scalar HIP oracle or load-batched LLVM/ROCDL FP8-WMMA kernel → HSACO + five-buffer launch descriptor | `gfx1201` only: scalar and WMMA ABIs pass two bit-exact shapes on RX 9070 XT. | The proof registry admits both exact ABIs; the generic `tessera.scaled_matmul` Schedule carrier still fails closed before Tile, so this does not imply a public dtype/op lane. |
