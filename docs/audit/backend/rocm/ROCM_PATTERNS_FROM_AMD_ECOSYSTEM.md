...
| Native counter evidence | `collect_rocm6_counters.py` runs retained production/candidate commands under installed `rocprofv3 --pmc`, preserving experiment, variant, command, and raw native outputs. | Record counter sets supported by gfx1151 for each G6 rung and join them with latency-ratchet rows. |
