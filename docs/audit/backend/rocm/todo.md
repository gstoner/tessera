...
compiles with only `--offload-arch=gfx1151`. Unlike CUDA, **the HIP driver
then passes no `-O` to the amdgcn device compile either** (`clang++ -###`), so
the device kernels are unoptimized too:
...
level and are unaffected. On Tajasarus, `build-assertions/` keeps assertions
despite `Release` (the trailing `-UNDEBUG` wins).
...
Owner ROCM-MXFP4-W4A8-1 / IKF-1; sync
`GFX1201-PACKED-A-OFFSET32-2026-09-23`. A signed-32-bit lane-local A
offset with a checked K bound passed five Tajasarus exact-device cases and
matched exact K32 BF16 output. The v6 HIP-event/selected-HSACO packet shows
188.8→184.3 µs versus the A-base control at 256×5120×8704 but
1184.4→1186.4 µs at 1024×17408×5120. Static integer ISA falls, while
VGPRs and waits stay flat. This is an opt-in, no-promotion result; no
shared ABI or numerical-policy change. [Evidence](../../../../benchmarks/baselines/gfx1201_mxfp4_a_offset32_20260923/README.md).
Next investigate wide-N producer traffic or K-step scheduling with matched
Radiance and the exact K32 oracle.
...
The bounded 32-bit lane-local A-offset follow-up is recorded above.
...
**ROCm outcome: not applicable as written; one latent consumer to note.**
The gfx1151 route builds Tile IR and serializes hsaco through `tessera-opt`;
it does not re-render decoration-time Graph IR, so the preflight does not gate
it and no numerics change. The dtype-annotation fix does reach ROCm indirectly
and favorably: shorthand-annotated args (`tessera.bf16["M","K"]`) now carry
`tessera.dim_names`, which is exactly what `SymbolicDimEqualityPass` consumes —
previously those names were dropped, so that pass had nothing to check on the
shorthand path. That is a strengthening, not a behavior change, and it is
**host-free**; the exact-device gfx1151 lanes were not run for this PR and no
ROCm claim is made. **Follow-up (small):** when a ROCm route becomes
parser-bound on Graph IR, call the preflight instead of relying on the parser.
...
here directly: **the four `paged_kv_decode` device rows are now refused** until
re-recorded, because they predate `unmeasured` and that generation is known to
have dropped untimeable candidates. The end-to-end rows are unaffected.

Then the same question NVIDIA just answered: **which ROCm candidates lack
`measure_device_latency`?** Neither `rocm_wmma_gemm` (T3) nor
`rocm_flash_attn` (T3) implements it — the gap already recorded under
`DELEGATE-CONTRACT-SYNC-2026-08-30`. So every ROCm device-timed race silently
excludes both hand-tuned lanes, which is the *mirror* of the NVIDIA defect: on
NVIDIA the excluded lane turned out to be the fastest by 1.5–1.7×, and nothing
here says the ROCm exclusions are harmless.

Owed on Princess-Luna: device timers for the two Tier-3 lanes, then a
re-recorded gfx1151 corpus. **The sm_120 finding does not transfer** — RDNA3.5
...
E1 is `python/tessera/compiler/recorded_product.py`: the per-class (R)
requirements, (C) confinement, occurrence-keyed region totality, and the
content address. No pass, kernel, or runtime path on this backend reads it —
that begins at E2 — so there is no gfx1151 evidence to record and none is
implied. What ROCm still owns, unchanged: it is one of the two E5
exact-device hosts, and the collective class needs native RCCL deterministic
evidence because the reduction tree is part of the value (a mock mesh cannot
establish result bit-identity).
...
Princess-Luna, where the test PASSES rather than skips.** The gfx1151 packaging
test now calls `native_packaging_available()`, the predicate this backend
already owns for the purpose.

**Worth a sweep here.** That predicate distinguishes "ROCm tools present" from
"AMD clang present", and the gap between them is invisible on a box that has
both. Any other ROCm test that checks `tools_available()` alone has the same
latent defect and will only show it on a host with a partial toolkit — which is
where this one surfaced.
...
**Outcome for this backend: `follow-up required` — a sibling instance was found
here and fixed, and it is the worse-placed of the two.**
`RocmKernelRunner.run_fused_attention` has always declined `head_dim % 16 != 0`
to the numpy reference (WMMA alignment), and `RocmFlashAttnCandidate` is
**Tier-3** — the highest-priority attention lane on the one AMD device that
executes. So on a ragged head_dim it took the dispatch and returned numpy while
`rocm_generic_hip` stood by: the same starvation
`RocmWmmaGemmCandidate.available` was hardened against on the *availability*
axis (PR #289 review, "starving the working generic lane"), one axis over.
`applies_to_inputs` now declines it before selection; a Q/K head_dim mismatch
defers, since that is an operand error and must still raise through `run`.

`rocm_wmma_gemm` and `rocm_generic_hip` were audited and need nothing — neither
declines by shape.

Owed from Princess-Luna (gfx1151): a decode/prefill shape with `head_dim % 16
!= 0` showing the dispatch now reaching a lane that runs, plus a corpus row for
that bucket whose field excludes `rocm_flash_attn` rather than listing it with a
numpy latency. Host-free coverage exists for the head_dim arithmetic itself
(it is numpy, above any HIP call); the dispatch claim is not made here.
...
**ROCm — not applicable, for the same three reasons as NVIDIA, on its own
sources; the existing bounded-wait follow-up is unchanged.**

1. *Nothing to publish.* `hip_backend.cpp:184` is `hipStreamSynchronize` and
   `hip_backend.cpp:233` is `hipEventSynchronize` — blocking, no timeout
   argument. A gfx1151 kernel that stops answering blocks once rather than
   returning a deadline that could be reported, so there is no expiry to
   publish. Still the open follow-up above, not a new gap.
2. *No shared-event hazard.* `hip_backend.cpp:191` creates a fresh
   `hipEvent_t` per event object, so no concurrent dispatch can satisfy
   another's wait.
3. *No pool to quarantine.* Same as NVIDIA: the pool is in the Apple runtime
   only.
...
**rocm outcome:** Shared-contract parity validated on Princess-Luna WSL, using the rebuilt tessera-opt and reference suites. Curvilinear fields have no HIP ABI or registered native candidate. No ROCm kernel or schedule changed; gfx1151 visibility is not device execution proof.
...
ROCm parity validated for 39 existing gfx1151 WMMA compiler/runtime checks,
including SSA/LDS producer structure. The new bounded tensor operations currently
lower only to SM120; ROCm package migration remains follow-up required. The
...
Checked host tickets pass independent derivative generations and injected failure refusal on Radeon 8060S/gfx1151. No RDNA4/CDNA evidence transfers. Tracked checked readers, dynamic returned shapes and measured overlap remain open.
...
gfx1151 exact-device validation covers logical lengths 0/3/8, nested failure refusal and checked device-reader chaining. Release remains context-synchronous; no overlap or hardware-counter claim. Scoped ANN evidence does not transfer to gfx1201/gfx950.
...
gfx1151 validates generated dynamic forward lengths, capacity refusal and checked generation retirement. The same 16x8 ANN experiment retains the incumbent (0.9997x median); no CDNA/RDNA4 performance or counter evidence transfers.
...
gfx1151 device proof covers runtime-shaped backward products, multiple matrix results and eight scoped frame retirements. No RDNA4/CDNA execution or hardware-counter attribution transfers.
...
Parity validated independently on gfx1151 for scoped dynamic public frames, checked readers, asynchronous capture/backward and whole-frame retirement. This is no proof for gfx1201, CDNA or hardware-counter attribution. Terminal-square ANN evidence remains architecture-specific; see the packet selection result rather than inferring promotion from CUDA.
...
Parity validated independently on gfx1151: all four capture/upstream status combinations and checked reader retirement. No RDNA4/CDNA or hardware-counter proof transfers.
...
Parity validated on Princess-Luna / gfx1151: four incoming statuses passed all sixteen truth-table combinations with scoped upstream readers and successful frame retirement. No RDNA4/CDNA proof transfers. Eight-input device proof, GPU external mutation and overlap/performance promotion remain follow-ups.
...
Parity validated on gfx1151: two immutable source-state generations and all 256 eight-status truth-table combinations. No in-place mutation, overlap or performance promotion is claimed; RDNA4/CDNA remain separate.
...
Parity validated on gfx1151: three owned in-place state updates preserve allocation identity, and active/expired readers refuse. Other AMD architectures require separate proof.
...
Sequencing: [live plan](../../compiler/INTEGRATED_COMPILER_PLAN.md).
Parity validated: gfx1151 asynchronous computation/copyback and pending-reader exclusion; other AMD chips remain separate.
...
Sequencing: [live plan](../../compiler/INTEGRATED_COMPILER_PLAN.md).
Existing single-state asynchronous consumer regression validated on gfx1151. Writable multi-input views and exception execution require new device bindings; other AMD chips remain separate.
...
Sequencing: [live plan](../../compiler/INTEGRATED_COMPILER_PLAN.md).
gfx1151 exact-device validation is recorded in the linked packet; other AMD architectures require independent proof.
...
Sequencing: [live plan](../../compiler/INTEGRATED_COMPILER_PLAN.md).
Parity validated on gfx1151: mapped forward/backward and synchronous/asynchronous static/dynamic exception completion. Other AMD architectures require separate proof.
...
Sequencing: [live plan](../../compiler/INTEGRATED_COMPILER_PLAN.md).
gfx1151 validated twenty runtime-map/context cases. Cooperative Block AttnRes passed numerical checks; mixed operation-total evidence keeps it opt-in and selector-ineligible. Other AMD architectures remain separate.
...
Sequencing: [live plan](../../compiler/INTEGRATED_COMPILER_PLAN.md).
gfx1151 owning-host cases validate runtime indices, generation contexts and synchronous checked VJP. Other AMD devices and asynchronous VJP staging require separate proof.
...
Sequencing: [live plan](../../compiler/INTEGRATED_COMPILER_PLAN.md).
gfx1151 independently passed 19 signed/nested map, checked VJP and retained-exception cases. These are correctness checks, not overlap/counter attribution or performance promotion.
...
Princess-Luna provides owning x86 CPU validation only; no gfx1151 fp64 GPU schedule or performance claim follows. Caller candidates and helper-to-emitter paths are recorded in `benchmarks/baselines/f64_route_ownership_20260910/`; they are not device certificates.

### Dynamic readers and row-private ANN (2026-09-10)

Owners: W2.4a / MSW-9. Sync: `DYNAMIC-READERS-ROW-ANN-2026-09-10`.

gfx1151 independently executes 64x8 square ANN after compaction. No new ROCm hardware-counter evidence; no promotion. Checked public/paired frames now support declared multi-stream reads; general heterogeneous capture and external-pointer escape remain open. Evidence: `benchmarks/baselines/ann_row_private_20260910/`.
...
Not applicable to rocm execution: Apple-only language probes and fresh
...
SDK27 bridge follow-through (`APPLE-METAL41-20260914`): Apple enum/extent/scale-plane fixes and execution admission assessed. Not applicable to rocm code generation or runtime ABI; no sibling dtype, schedule or execution proof changes.

Packed numeric binding (`APPLE-METAL41-20260914`): explicit Apple-only status ABI and Python numerical entry added. Not applicable to rocm execution; shared ABI inventory updated, no dtype or operation promotion. Owning-device numerical evidence does not transfer.
...
## ROCM-MIXED-FP8-1 closed: the pair was a Schedule field, and two gates were not checking their own claim — 2026-09-19

Sync `GFX1201-PARITY-2026-09-17`; owner COMPILER-DEVEX-1 with W4-PRODUCT-1.

**All four OCP FP8 pairings execute natively on gfx1201, each selecting its own
instruction.** `fp8_fp8` rel 0.00e+00, `fp8_bf8` 2.60e-08, `bf8_fp8` 2.59e-08,
`bf8_bf8` 5.21e-08 — the mixed figures are accumulation order alone, since an
fp8 product is exact in f32.

The blocker recorded on 2026-09-19 was `MatmulSchedule`'s single `StringRef
storage`, and that was right as far as it went. Widening it exposed **eight more
layers holding the same assumption**, each of which refused rather than
computing wrong numbers — which is why this was a reachability gap and never a
correctness bug:

| Layer | What it assumed |
|---|---|
| `MatmulSchedule` (`PMPasses.cpp`) | one `storage`, written to both descriptor slots |
| `GenerateWMMAGemmKernel` | both operand buffers typed from A |
| fragment materialization | B's fragment element inherited from A |
| generator descriptor gate | `a == b` |
| Target matmul gate (`TileToROCM.cpp`) | `a == b` |
| `package_scheduled_matmul` | `a == b` |
| launch ABI selection | keyed on A alone; no id named a pair |
| launcher bindings | B given A's dtype |
| submit-time validation | B checked against A's dtype |

`resolveFragmentLayout` already selected `V_WMMA_F32_16X16X16_FP8_BF8` and its
mirror from the descriptor's two types before any of this, so the instruction
selection was never the missing piece — expressing the pair was.

**Then two gates turned out not to be checking what their names claimed, and
both had been green since they were written.**

*The instruction claim.* Six device rows across four files assert which matrix
instruction was emitted, each with its own copy of finding `llvm-objdump` and
**three different policies for not finding it** — one of which (`if objdump is
not None:`) dropped the assertion and still reported a pass, in a row named
`..._select_their_instruction_...`. They now share `tests/_support/rocm_isa.py`,
where a missing disassembler is a **failure and not a skip**: the caller has
already asserted it is the owning device, and every ROCm install ships
`llvm-objdump` under `$ROCM_PATH/llvm/bin`, so absence is a broken fixture
rather than an honest capability gap. Confirmed on Tajasarus, where the resolver
picks the assertions toolchain's copy.

Two claims were weaker than their names:

* `test_gfx1201_double_k_int4_emits_its_instruction_and_is_exact` asserted only
  the numbers — and **the numbers cannot distinguish the form.** The double-K
  int4 WMMA and two K=16 int4 WMMAs compute the identical exact integer
  product, so the sole owner of RDNA4's double-K capability would have passed on
  a generator that quietly kept K=16. It now requires
  `v_wmma_i32_16x16x32_iu4` and forbids `v_wmma_i32_16x16x16_iu4`; the device
  shows the double-K form and no K=16 one, which is the first time that
  capability had the right witness.
* Every row required a mnemonic and forbade none, so **none could see a swap.**
  Finding `fp8_bf8` does not establish that `bf8_fp8` is absent, and an operand
  exchange anywhere in the stack emits the mirror while satisfying a presence
  check. Falsified on device: asking the `e4m3 x e5m2` row for the mirror fails
  with `expected v_wmma_f32_16x16x16_bf8_fp8 ... saw
  {'v_wmma_f32_16x16x16_fp8_bf8': 4}` — one mnemonic, four times, no mirror.

*The reachability list.* It checked one direction: a listed form that became
reachable failed. **Two of its own entries were self-fulfilling** — the helper
returned `False` for the reduced-precision accumulators from the same hardwired
rule that had written the entries, without asking the route. Probing them found
the f16 entry naming `ROCM_WMMA_ACCUM_UNSUPPORTED`, a C++ diagnostic that never
fires on this path (the route refuses earlier, in Python), and the bf16 entry
"proved" by a `KeyError` in the test fixture's own output-dtype map — evidence
about the fixture, not the compiler. This is the same shape that had already
bitten once, when the helper assumed a mixed fp8 pair was inexpressible and had
to be taught otherwise before the pairs could be retired from the list.

Entries now carry the **marker the actual refusal must contain**, every answer
comes from the route, and a form still refused *for a different reason than
recorded* fails. A form with K != 16 is delegated to a named device row whose
existence is asserted, so a rename cannot orphan it. All directions were
falsified before being recorded.

**Two adjacent defects that the marker required.** The typed route's
fall-through refusal named neither the dtypes nor the target, and was identical
for a reduced-precision accumulator, an unsupported storage, and a target with
no GEMM for the pair; it now names all three
(`SCHEDULED_MATMUL_DTYPE_CONTRACT_UNSUPPORTED`, Decision #21). Registering that
code exposed the same class once more: the **Python diagnostic scanner is a
prefix allowlist**, so 16 code-shaped diagnostics raised from Python — nine
`ROCM_FRAGMENT_*`, five `APPLE_*`, plus two others — were invisible to their own
registry gate. Its own `GRAPH_IR_` comment records that failure happening
before, for a whole family, while the gate reported green. It now matches the
code *shape*, as the C++ scan always has. The 15 pre-existing codes are a
**shrink-only ratchet** (`DIAG-PY-BACKLOG-1`), failing if the set grows and
failing again when one is registered without leaving the list, because
authoring three other subsystems' registry entries inside this change is how a
focused diff stops being reviewable.

**The route was confirmed, not assumed.** Tile IR (`tile.matmul_kernel`,
`tile.mma_desc`, `tile.epilogue`) → `tessera_rocm.wmma_gemm` with the consumer
stamped and **zero unconsumed `tile.*`** → upstream
`createConvertGpuOpsToROCDLOps` → `createGpuROCDLAttachTarget` →
`createGpuModuleToBinaryPass` → gfx1201 ISA. `_compile_native_tile_ir` hard-fails
without `tessera-opt`, its cache key includes that binary's hash, and there is
no HIPRTC lane in `rocm_native.py`, so the scheduled route cannot degrade into
the Python bootstrap packager. `amdgpu.global_load_tr` is the sharpest witness:
no lowering for it was written here — upstream `convert-gpu-to-rocdl` produces
`rocdl.global.load.tr.b128` and LLVM's AMDGPU backend emits the instruction.

| Host | Suites | Lit (lane, not suite) |
|---|---|---|
| Tajasarus (gfx1201) | **95 passed** (scheduled + reachability), **46 passed** (sparse), 0 failed | 433/495, 62 unsupported; `check-tessera-rocm` 75/75 |
| Princess-Luna (gfx1151) | 168 passed, 103 skipped — regression lane; RDNA3.5 has no FP8 WMMA at all, so it is no evidence for this item | 491/495, 4 unsupported; `check-tessera-rocm` 75/75 |
| Mac (M1 Max) | 129 passed, 162 skipped — host portability; the ROCm backend is not configured there, so a green Mac build is no evidence for this C++ | 452/495, 43 unsupported |

**Still owed.** The 8-bit `TR_B64` mapping, measured the same way, which would
extend the transpose load to fp8 and int8; the 2048³ transpose-load reversal,
which needs counters neither WSL2 ROCm box can produce; AMD's identity-matrix
in-register transpose as the int4 fallback; implementing the register-pressure
reduction now that `ROCM-VGPR-PRESSURE-1` is scoped to per-tile addressing;
raster-order selection; the sparse `V_SWMMAC_I32_16X16X64_IU4`, reachable and
unmeasured; `DIAG-PY-BACKLOG-1`.
