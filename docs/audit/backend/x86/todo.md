...
**x86 outcome: not applicable (architecture-specific).** The marker is a GPU kernel; x86 timing uses `tsc_cycles` with its own open follow-up (independent TSC frequency in the probe, `WSL-TIMING-ADMISSION-2026-09-26`). The `measurement_digests` requirement applies to any future WSL x86 corpus.
...
Owner ROCM-MXFP4-W4A8-1 / IKF-1; sync
`GFX1201-PACKED-A-BASE-2026-09-23`. Not applicable to AVX-512: the HIP
A-stage address ablation and v5 HSACO/event schema do not alter x86 runtime
or numerical selection. The vector-pair sync-key correction is ROCm-only
artifact provenance; no x86 device result transfers.
...
Owner ROCM-MXFP4-W4A8-1 / IKF-1; sync
`GFX1201-MXFP4-DEVICE-GRAPH-2026-09-23`. Not applicable to AVX-512:
HIP graph nodes, GPU streams, and retained HSACO buffers have no CPU
executor or benchmark consumer. Shared numerical and selector contracts
are unchanged; no x86 proof is implied.

## GFX1201 resident MXFP4 HIP lifecycle — 2026-09-23

Owner ROCM-MXFP4-W4A8-1 / IKF-1; sync
`GFX1201-MXFP4-RESIDENT-HIP-2026-09-23`. Not applicable to AVX-512
execution: retained HSACO/stream/device buffers and HIP event timing do not
define a CPU schedule or launcher. The shared numerical policy is unchanged;
no x86 route or performance proof transfers.
...
Not applicable to AVX-512 execution: the fragment order, E8M0 scale-plane
layout, and Target ABI are RDNA4-specific. Shared Graph verification adds an
explicit physical branch only; no x86 schedule or numerical proof transfers.

## GFX1201 folded K64 staging codegen — 2026-09-22

Owner ROCM-MXFP4-W4A8-1; sync `GFX1201-FOLDED-K64-STAGING-2026-09-22`.
Not applicable to AVX-512 execution: the specialization is a gfx1201 HIP
copy-loop codegen choice, and its HIP-event packet has no x86 runtime or
selector consumer. No CPU physical schedule or proof transfers.
The timed-HSACO ISA evidence schema has no x86 machine-code consumer.
...
`ROCM-MXFP4-KSTEP-PREFILL-2026-09-22`. Not applicable to x86 execution. The
...
no AVX ABI, schedule, numerical policy, selector, or host-proof claim changes.
...
Parity validated for native LLVM execution of recovered nested branches/early returns and single-carry bounded while, and scheduled forward attention including extended modifiers. Descriptor projection fixes the extended runtime window field. GPU pool ownership/module unloading is not applicable to the x86 host ABI. Non-f32 matmul, backward attention/cohorts and generic Graph-owned routes remain follow-ups.
...
Parity validated: native LLVM execution of mixed local-state break/continue and ordered assertions. Failure-effect tests invoke compiled code in a subprocess and require SIGABRT; no richer runtime error ABI is claimed. GPU two-status pool ownership is not applicable to the host ABI.
...
Parity validated: native LLVM differential execution covers nested/tuple loop returns, handled exception/finally edges and declared exact full-tensor input aliases. Exclusive host ownership and synchronous copyback are required. GPU status storage and pool retirement are not applicable to this host ABI.
...
Parity validated on native LLVM CPU: strided/reversed state views, explicit builtin exception transport and bounded public source-JIT ownership. GPU ownership does not apply to this host ABI.
...
Parity validated for native CPU declared object fields, static exception payloads and explicit pure-tensor paired VJP. GPU ownership proof is not applicable to the host copyback ABI.
...
FP64 matmul now consumes replay-verified Schedule/Tile artifacts. Mixed uint8/int8 still needs its Graph constructor; cohort, elementwise and breadth remain. Caller candidates and helper-to-emitter paths are recorded in `benchmarks/baselines/f64_route_ownership_20260910/`; they are not device certificates.

### Dynamic readers and row-private ANN (2026-09-10)

Owners: W2.4a / MSW-9. Sync: `DYNAMIC-READERS-ROW-ANN-2026-09-10`.

WSL host tests cover heterogeneous checked-reader failure unwinding. GPU private allocation proof does not change x86 execution. Checked public/paired frames now support declared multi-stream reads; general heterogeneous capture and external-pointer escape remain open. Evidence: `benchmarks/baselines/ann_row_private_20260910/`.
...
Follow-up required: host extraction contracts apply, but CUDA/HIP stream ownership and traced SSD execution are not x86 proofs. Negative-infinity raised attention admission stays disabled here pending native proof.
...
Follow-up required: native CPU AD composition needs its own canonical graph/accumulation integration. Host discovery improvements apply; CUDA/HIP stream and snapshot evidence does not transfer.
...
Follow-up required: threaded native allocator/epoch and LLVM atomic proof; host models do not establish GPU execution.
...
Follow-up required: the model is host logic, not a threaded native CPU heap integration. CPU allocation/reader epochs need their own native producer and validation; CUDA/HIP events do not transfer.
...
Follow-up required: threaded native host heap binding and owning-host synchronization proof. The finite model is host analysis, not a CPU collector implementation; CUDA/HIP events do not establish x86 reader retirement.
...
Not applicable to CUDA/HIP execution: the host supplies model, replay and fault-injection checks. Native host multi-writer atomics and collection need their own implementation/proof.
...
Not applicable to GPU execution: finite race models and lifecycle fault tests run on the host; a native CPU collector needs its own atomic integration and execution proof.
...
Not applicable to GPU execution: host replay/ownership tests cover the shared contract. A native host heap owner needs its own execution proof; process death does not prove CPU/GPU performance.
...
Not applicable to x86 execution: Apple-only language probes and fresh
...
SDK27 bridge follow-through (`APPLE-METAL41-20260914`): Apple enum/extent/scale-plane fixes and execution admission assessed. Not applicable to x86 code generation or runtime ABI; no sibling dtype, schedule or execution proof changes.

Packed numeric binding (`APPLE-METAL41-20260914`): explicit Apple-only status ABI and Python numerical entry added. Not applicable to x86 execution; shared ABI inventory updated, no dtype or operation promotion. Owning-device numerical evidence does not transfer.
