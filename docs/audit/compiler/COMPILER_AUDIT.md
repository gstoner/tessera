...
| **Target IR / runtime** | x86 AVX-512, Apple GPU, NVIDIA `sm_120`, and ROCm `gfx1151`/`gfx1201` all have checked-in native execution rows. Per-target row counts live in [`generated/runtime_execution_matrix.md`](../generated/runtime_execution_matrix.md) (an earlier "24 NVIDIA and 69 ROCm rows" copied here went stale — corrected 2026-09-25); the E2E fleet additionally seals release packets for NVIDIA softmax/reduction and ROCm softmax/reduction/paged-KV/MoE. `fusion.py` remains real runtime MSL codegen for matmul-epilogue regions, and `tessera_jit` is a real MLIR→LLVM CPU JIT. | Native breadth is architecture-specific, not backend-wide: NVIDIA `sm_80`/`sm_90`/`sm_100` and the ROCm target-map tail remain artifact-only or exact-device gated. Apple still has a name→lane→ctypes dispatcher seam, and reference execution remains explicit for unsupported rows. | Close the dispatcher seam and promote the remaining target-map tails only with exact-device execute-and-compare evidence. |
