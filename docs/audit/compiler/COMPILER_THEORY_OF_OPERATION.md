...
| **W3** | **Memory planning** (buffer reuse / liveness / allocation, global rematerialization) | Peak-memory is often the binding constraint in training; fusion alone doesn't plan buffers | Tile reuse groups materialize as aligned address-space-3 arenas plus typed offset views. Static arenas use real workgroup globals. The ROCm post-ROCDL materializer follows pointer-carrying LLVM descriptor aliases, colors runtime arenas by SSA lifetime interference, and records `aligned_sum_of_slot_maxima`: sequential, nested branch, loop-local, forwarded-alias, and conservative escaping lifetimes are handled. A target-independent `LaunchDescriptor.dynamic_local_memory_expression` now serializes checked argument/constant/add/multiply/max/path-max/alignment arithmetic, retains legacy descriptor identity when absent, and is consumed by NVIDIA's existing dynamic-shared probe. Exact gfx1151 execution verifies an outer 8,192-byte slot plus `max(4,097, 12,289, 32,001)` loop/branch slot at a 40,208-byte launch and one active block/CU. Rematerialization preserves explicit CLI/function override precedence, otherwise derives an activation-byte budget from device capacity/reserve, marked parameters, gradient/optimizer copies, and persistent state; dynamic parameters require explicit byte bounds. The retained measured gfx1151/AVX-512 cost corpus remains the selection cost source. **Remaining:** emit arbitrary local arithmetic from ROCm IR into the shared expression field, inject target/device capacity through production launchers, and validate model-level rematerialization on full training graphs. |
