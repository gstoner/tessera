...
> cost modeling (arXiv:2607.22432), the mock-model finding it surfaced, and the
> T1 reuse/cache first slice that replaced both mock estimators on 2026-07-30.
> See §4 and W7 below.
...
| Cost model | measured (M1+); T1 cache semantics open | T1 reuse/cache pruning + measured (sm_120) | T1 reuse/cache pruning + `MmaDescriptor` footprint + measured (gfx1151) | T1 reuse/cache pruning; hierarchy calibration open |
...
3. **Score the surviving (in-budget) candidates** — on a system with real silicon
   ...
   silicon, prune by the Tier 2 T1 model (symbolic GEMM tile reuse through an
   explicit cache capacity, then a compute/DRAM roofline) plus any
   architecture-owned descriptor score. **Caveat (2026-07-30):** T1 is a
   pruning signal, not promotion authority. It has no T3 action-DAG ordering or
   T4 overlap model, and its cache semantics still require per-backend
   correlation. Exact-device measured latency remains the only load-bearing
   selector.
...
| **W2** | **Dynamic shapes** (symbolic dims, bucketed specialization, guards) | LLM serving is variable seq-len + growing KV cache; static-only is a non-starter for inference | **Production routes live (updated 2026-07-24):** generic emitters have shape-independent dynamic identities; x86 and gfx1151 execute guarded contiguous last-axis reduction/softmax, runtime-sequence attention, and growing KV-cache movement. x86 also owns guarded rank-2 matmul. Shared rank/contraction/extent/side-buffer/ABI guards run before native entry. On ROCm, reduction/softmax and KV capacity/logical lengths remain fully dynamic; attention sequence lengths are dynamic while head dimension, dtype, GQA, window, softcap, bias, and two-wave features remain compile/cache buckets. ROCm RMSNorm/LayerNorm fuse unary ReLU/SiLU/GELU and same-shape residual add/multiply into one shape-independent runtime kernel. Regression/BCE losses already feed the general SGD/AdamW training consumers through the shared single-use fusion contract. **Remaining:** architecture-owned dynamic policies for Apple MSL/tensor-core lanes and broader multi-op fusion beyond the registered consumers. |
| **W3** | **Memory planning** (buffer reuse / liveness / allocation, global rematerialization) | Peak-memory is often the binding constraint in training; fusion alone doesn't plan buffers | Tile reuse groups materialize as aligned address-space-3 arenas plus typed offset views. Static arenas use real workgroup globals. The ROCm post-ROCDL materializer follows pointer-carrying LLVM descriptor aliases, colors runtime arenas by SSA lifetime interference, and records `aligned_sum_of_slot_maxima`: sequential, nested branch, loop-local, forwarded-alias, and conservative escaping lifetimes are handled. A target-independent `LaunchDescriptor.dynamic_local_memory_expression` serializes checked argument/constant/add/multiply/max/path-max/alignment arithmetic, retains legacy descriptor identity when absent, and is consumed by NVIDIA's dynamic-shared probe. Exact gfx1151 execution verifies an outer 8,192-byte slot plus `max(4,097, 12,289, 32,001)` loop/branch slot at a 40,208-byte launch and one active block/CU. ROCm production launchers inject retained HIP total/free capacity into the model-derived rematerialization budget; explicit CLI/function overrides retain precedence and dynamic parameters require byte bounds. **Remaining:** emit arbitrary local arithmetic from ROCm IR into the shared expression field, add equivalent production capacity injection to sibling launchers, and validate measured rematerialization selection on full training graphs. |
| **W4** | **Layout & data-movement optimization** (layout propagation, transpose elimination, packing) as a *wired* pass | A large share of real DL latency is data movement, not FLOPs | `LayoutAssignmentPass` carries row/column-major contracts through transpose, pointwise/fused epilogues, GEMM, and last-axis reduction while preserving packed storage. x86 consumes row/column-major/BHSD/NHWC Graph casts. NVIDIA and x86 default layout assignment on because they own immediate physical Graph-cast consumers. ROCm's signed-INT4 terminal pack feeds IU4 WMMA, group-scaled dequant-GEMM, nibblewise ReLU, indexed sparse gather, and packed cache append without host unpack/repack. Exact gfx1151 operation-total medians are 1.93 ms dequant-GEMM, 3.88 ms elementwise, 2.31 ms sparse gather, and 2.26 ms cache append. Terminal packing remains opt-in outside operations with one of these physical ABIs; Apple and ROCm retain architecture-owned default boundaries. **Remaining:** Apple/ROCm default-enable decisions per proven consumer envelope, sibling-device physical evidence, and retirement of the coarse string-layout compatibility contract after every sibling path has structured physical metadata. |
| **W5** | **Training / backward-graph optimization** (backward fusion, optimizer-step fusion, remat as global opt) | The compiler must optimize the *whole* training step, not just forward inference | Native Graph-IR adjoints cover tensor algebra, reductions, activations, softmax, affine normalization, regression/BCE/class/distribution losses, and explicit Momentum/Nesterov/Adam/AdamW state. ROCm owns compiler-generated Adam/AdamW and arbitrary-axis KL/JS VJPs, a compiled Lion VJP under the canonical stop-gradient-through-sign policy, and factored/full Adafactor forward plus physical adjoint execution. Single-use MSE/MAE/Huber/SmoothL1/BCE prediction gradients fuse directly into SGD or AdamW; one dynamic Linalg loop and exact AVX-512/gfx1151 ABIs preserve dTarget while eliminating dPrediction. Dynamic RMSNorm/LayerNorm additionally fuse canonical softcap on gfx1151. Runtime mean extent lowers through `tensor.dim`; max/min retain equal-share finite-tie semantics. **Remaining:** the runtime-ambiguous aligned dynamic-broadcast case, additional loss/optimizer adjoints, broader training-step epilogues, and architecture-owned Apple/NVIDIA Lion, Adafactor, and general training/backward materializers. |
| **W6** | **Distributed optimization** (sharding propagation, collective placement, comm/compute overlap scheduling) | Frontier training is multi-GPU; overlap scheduling is a top differentiator | Schedule IR emits mesh/ZeRO/1F1B and MegaMoE steps. The shared runtime consumes their unique-clock order, overlaps selected collectives, and enforces OptimizerShard ownership with deterministic two-rank adapter fixtures. Placement/overlap remains runtime machinery rather than a middle-end optimization pass, and real multi-rank NCCL/RCCL/Metal packets are still architecture-owned gaps. |
