...
| **W8** | **Long-tail op codegen** (generic elementwise/reduction/scatter/gather synthesis) | ~125 numpy-only ops; residency planner routes them but doesn't *close* them | Fusion synthesizer covers matmul-epilogue + pointwise DAGs; the structural/indexing tail has no codegen path |
