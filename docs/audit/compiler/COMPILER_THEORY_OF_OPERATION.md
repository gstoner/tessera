...
| **W2** | **Dynamic shapes** (symbolic dims, bucketed specialization, guards) | LLM serving is variable seq-len + growing KV cache; static-only is a non-starter for inference | **Production routes live (updated 2026-07-23):** generic emitters have shape-independent dynamic identities; x86 and gfx1151 execute guarded contiguous last-axis reduction/softmax, runtime-sequence attention, and growing KV-cache movement. x86 also owns guarded rank-2 matmul. Shared rank/contraction/extent/side-buffer/ABI guards run before native entry. On ROCm, reduction/softmax and KV capacity/logical lengths remain fully dynamic; attention sequence lengths are dynamic (including unequal query/key lengths) while head dimension, dtype, GQA, window, softcap, bias, and two-wave features remain compile/cache buckets. ROCm RMSNorm/LayerNorm can now fuse a ReLU or SiLU consumer into the same shape-independent runtime kernel; the retained 7x300 packet is exact and operation-total measured. **Remaining:** broader normalization/loss consumers and architecture-owned policies for Apple MSL/tensor-core lanes. |
| **W3** | **Memory planning** (buffer reuse / liveness / allocation, global rematerialization) | Peak-memory is often the binding constraint in training; fusion alone doesn't plan buffers | Tile reuse groups materialize as aligned address-space-3 arenas plus typed offset views. Static arenas use real workgroup globals (not LLVM allocas); gfx1151 reports the requested LDS bytes. Dynamic descriptors form dominance-scoped cohorts, including branch-local and later-defined descriptors. A ROCm post-ROCDL materializer replaces the single runtime arena alloca with the external dynamic-LDS symbol, and the launch supplies the exact byte count; 16/32 KiB executions are verified and report 4/2 active blocks per CU. Multiple runtime arenas are rejected pending a packed-offset ABI. Autodiff labels backward IR, and function-budgeted rematerialization consumes exact consumer-chain and shape measurements when available, otherwise stable work units. The corpus now spans 64³/128³/192³ matmul with ReLU/GELU/SiLU on gfx1151 and AVX-512. **Remaining:** multiple dynamic-LDS cohorts in one kernel, more producer families, and workload-level rematerialization policies. |
| **W4** | **Layout & data-movement optimization** (layout propagation, transpose elimination, packing) as a *wired* pass | A large share of real DL latency is data movement, not FLOPs | `LayoutAssignmentPass` carries row/column-major contracts through rank-2 transpose, agreed-layout pointwise/fused epilogues (including shape-preserving MSE), GEMM, and last-axis reduction while preserving packed storage and cast provenance. x86 now consumes row/column-major/BHSD/NHWC Graph casts; its generic ABI materializes C/Fortran buffers and emits order-correct pointer strides. On the local AVX-512 host, preserving already-Fortran 128³ inputs is correct and 1.005x the row-major repack route, so column-major is enabled for such inputs. ROCm now owns an opt-in compiled signed-INT4 terminal pack/unpack ABI and a direct packed-memory IU4 WMMA consumer: ragged/odd storage is exact, physical input size is approximately halved, and no consumer-side host repack occurs. Apple/NVIDIA retain their architecture-owned consumers and opt-in policy. **Remaining:** broader packed operation families, target policies, and sibling-device physical consumers. |
| **W5** | **Training / backward-graph optimization** (backward fusion, optimizer-step fusion, remat as global opt) | The compiler must optimize the *whole* training step, not just forward inference | Native Graph-IR adjoints cover tensor algebra, reductions, activations, softmax, affine normalization, regression/BCE/class/distribution losses, and explicit Momentum/Nesterov/Adam/AdamW state. Single-use MSE/MAE/Huber/SmoothL1/BCE prediction gradients now fuse directly into SGD or AdamW; one dynamic Linalg loop and exact AVX-512/gfx1151 ABIs preserve dTarget while eliminating dPrediction. Runtime mean extent lowers through `tensor.dim`; max/min retain equal-share finite-tie semantics. **Remaining:** the runtime-ambiguous aligned dynamic-broadcast case, Lion/Adafactor and additional loss/optimizer adjoints, broader training-step epilogues, standalone compiled Adam/AdamW adjoints on ROCm, and Apple/NVIDIA training/backward materializers. |
