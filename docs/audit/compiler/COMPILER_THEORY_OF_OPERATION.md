...
| **W3** | **Memory planning** (buffer reuse / liveness / allocation, global rematerialization) | Peak-memory is often the binding constraint in training; fusion alone doesn't plan buffers | Tile reuse groups now materialize as one aligned address-space-3 byte arena plus typed offset views, and ROCm/NVIDIA production backend pipelines execute the reuse+arena chain. The post-autodiff rematerialization pass consumes a function budget, performs deterministic liveness-aware global selection, and sinks selected pure activations. **Remaining:** exact-device occupancy/peak-memory benchmarks, dynamic-size arenas, and a cost model that balances recompute latency against memory saved. |
| **W4** | **Layout & data-movement optimization** (layout propagation, transpose elimination, packing) as a *wired* pass | A large share of real DL latency is data movement, not FLOPs | `LayoutAssignmentPass` carries row/column-major contracts through rank-2 transpose, agreed-layout pointwise/fused epilogues, GEMM, and last-axis reduction while preserving packed storage and cast provenance. Legality checks the GEMM and reduction accept sets. x86, Apple, and NVIDIA consumers remain as documented; assignment stays opt-in until every enabled family has a materializer. **Remaining:** default enablement by target and exact-device transpose/packing performance proof. |
| **W5** | **Training / backward-graph optimization** (backward fusion, optimizer-step fusion, remat as global opt) | The compiler must optimize the *whole* training step, not just forward inference | Native Graph-IR adjoints cover matmul, tanh/sigmoid, collectives, add/multiply, static and guarded-dynamic broadcast inversion, sum/mean/max/min reductions, GELU/SiLU/ReLU, softmax, and unary/affine RMSNorm/LayerNorm. Runtime mean extent lowers through `tensor.dim`; max/min have explicit equal-share finite-tie semantics and a common linalg lowering. Normalization and existing ROCm/AVX-512 execution evidence remain as recorded. **Remaining:** the runtime-ambiguous aligned dynamic-broadcast case, loss-family adjoints, optimizer-step/backward-epilogue fusion, and Apple/NVIDIA normalization-backward materializers. |
