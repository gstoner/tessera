...
| **W5** | **Training / backward-graph optimization** (backward fusion, optimizer-step fusion, remat as global opt) | The compiler must optimize the *whole* training step, not just forward inference | Native Graph-IR adjoints now cover matmul, tanh/sigmoid, collectives, same-shape add/multiply, static broadcast inversion, kind-aware sum/mean reductions, GELU/SiLU, and softmax. Emitted backward IR is directly interpreted and oracle-matched on CPU; dynamic broadcast and dynamic mean retain explicit Python-VJP fallbacks, while max/min remain placeholder until tie semantics are specified. **Remaining:** ReLU (needs a registered comparison primitive), RMSNorm/LayerNorm (needs a stable statistics/reciprocal-square-root carrier), backward epilogue and optimizer-step fusion, and global rematerialization. |
