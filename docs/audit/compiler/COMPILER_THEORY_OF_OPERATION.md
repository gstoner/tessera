...
|---|---|---|---|
| **Mac** (dev/authoring hub) | Apple M1 Max (Apple7) GPU + CPU | Homebrew LLVM/MLIR 22.1.6, macOS, off-venv `python3` 3.14.5 | Apple MSL/`metallib` compile + execute; Metal 4 / MPSGraph; **all hardware-free work** (lit, mypy, unit tests, IR, **golden-IR generation**) |
| **Strix Halo** (AMD box) | Ryzen AI Max+ 395 — Zen 5 (AVX-512 VNNI/BF16, **no AMX**) + Radeon 8060S **gfx1151** RDNA 3.5 | Ubuntu 24.04 + ROCm 7.2.4, LLVM/MLIR 22 (apt.llvm.org), `.venv` numpy<2.2 | ROCm gfx1151 WMMA **execute-and-compare**; **x86 AVX-512 native execute** (Zen 5); AMDGCN codegen via open LLVM AMDGPU; measured autotune on gfx1151 |
...
| **W2** | **Dynamic shapes** (symbolic dims, bucketed specialization, guards) | LLM serving is variable seq-len + growing KV cache; static-only is a non-starter for inference | **Interface half pulled into the spine (decision 2026-07-02):** the emitter/plugin API carries symbolic dims + a `static\|bucket\|dynamic` policy, the arbiter keys on shape-bucket, and first impls emit `bucket` (Refactor B2/C1/D1). Today's `requires static shapes` gate (`TileToX86Pass`, `MatmulToAppleCPU`) becomes a policy. **Remaining:** the full `dynamic` (runtime-arg + guards) emitter — drops in without an API break |
...
| **W4** | **Layout & data-movement optimization** (layout propagation, transpose elimination, packing) as a *wired* pass | A large share of real DL latency is data movement, not FLOPs | `LayoutAssignmentPass` landed but **unwired** (no backend consumer); the transposed-K attention block (OPTIMIZING deferred note) is a live symptom |
| **W5** | **Training / backward-graph optimization** (backward fusion, optimizer-step fusion, remat as global opt) | The compiler must optimize the *whole* training step, not just forward inference | `AUTODIFF_SPEC` + tape + the MLIR `AutodiffPass` (reverse-mode via `AdjointInterface`) exist. **First slice landed (2026-07-11):** the middle-end now *does* apply to the backward graph — `tanh`/`sigmoid` adjoints emit NATIVE Graph IR (`dy·(1−tanh²)` / `dy·s·(1−s)`, from mul/sub + the forward op) instead of opaque `custom_adjoint_call` placeholders, so the backward is first-class compiler IR: `--tessera-autodiff --cse` collapses the recomputed forward activation (one `tanh` for the whole fwd+bwd graph) — an optimization an opaque call blocks. Numerics match the numpy-tape VJP exactly. **Remaining:** native adjoints for the compare/reduction-needing ops (relu/gelu/layernorm/softmax), backward epilogue fusion, optimizer-step fusion (ops already monolithic), remat as a global pass |
