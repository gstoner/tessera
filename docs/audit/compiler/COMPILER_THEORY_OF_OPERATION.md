...
| **W4** | **Layout & data-movement optimization** (layout propagation, transpose elimination, packing) as a *wired* pass | A large share of real DL latency is data movement, not FLOPs | `LayoutAssignmentPass` now propagates an agreed layout through pointwise chains and into last-axis reduction, inserts provenance-bearing casts, and preserves packed-storage attributes; the generic x86 emitter separately consumes executable row/column-major binding contracts. Graph cast insertion remains deliberately opt-in because no Graph-cast materializer exists yet. **Remaining:** finish the complete matmul → epilogue → reduction route, add Apple/NVIDIA architecture-owned cast materializers, broaden packed-storage rewrite coverage, then enable only pipelines with a real consumer. |
| **W5** | **Training / backward-graph optimization** (backward fusion, optimizer-step fusion, remat as global opt) | The compiler must optimize the *whole* training step, not just forward inference | Native Graph-IR adjoints now cover matmul, tanh/sigmoid, collectives, same-shape add/multiply, and static broadcast inversion. Add/multiply/broadcast emitted backward IR is directly interpreted and oracle-matched on CPU; dynamic broadcast retains an explicit Python-VJP placeholder. **Remaining:** kind-aware sum/mean reduction promotion (without overclaiming max/min tie semantics), ReLU/GELU/SiLU, RMSNorm/LayerNorm, backward epilogue and optimizer-step fusion, and global rematerialization. |
