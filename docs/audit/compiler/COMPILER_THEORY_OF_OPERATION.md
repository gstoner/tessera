...
| **W5** | **Training / backward-graph optimization** (backward fusion, optimizer-step fusion, remat as global opt) | The compiler must optimize the *whole* training step, not just forward inference | Middle-end is forward/inference-shaped; `AUTODIFF_SPEC` + tape exist but the synthesizer isn't applied to backward graphs |
...
| **W8** | **Long-tail op codegen** (generic elementwise/reduction/scatter/gather synthesis) | **Largely closed (reassessed 2026-07-08).** `generated/e2e_op_coverage.md` = 280 complete / 6 runnable_reference / 0 artifact_only; the elementwise/reduction/scatter/gather/sort families are `fused`/`compiled`/`hardware_verified` (ROCm 327 native · 0 reference, x86 317 native) after the warp-shuffle lanes. The old "~125 numpy-only" was registry-conservative prose (Decision #26). | Remaining non-native = the **6 collective/MoE-transport** ops at `reference` (need real NCCL/RCCL → W6, not op synthesis) + `not_applicable` structural/view/host ops (reshape / kv_cache / visual-complex — no compute kernel). No generic-synthesis gap left on the compiled targets. |
