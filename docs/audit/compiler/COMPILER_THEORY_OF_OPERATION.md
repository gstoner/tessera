...
| **W3** | **Memory planning** (buffer reuse / liveness / allocation, global rematerialization) | Peak-memory is often the binding constraint in training; fusion alone doesn't plan buffers | Tile reuse groups materialize as aligned address-space-3 arenas plus typed offset views. Static arenas use real workgroup globals. The ROCm post-ROCDL materializer packs runtime byte arenas with 16-byte aligned prefix sums. Direct mutually-exclusive branch successors reuse offset zero and carry a host-visible `max_of_aligned_sums` expression; sequential, nested, looping, or escaping lifetimes remain a named rejection rather than an under-sized launch. Exact ragged 12,289-versus-32,001-byte branch launches use 32,016 bytes (12,304 fewer than a sum), execute both paths, and report two active blocks/CU. Function-budgeted rematerialization consumes exact matmul, softmax, RMSNorm, MSE, Huber, SmoothL1, and BCE rows on gfx1151/AVX-512. The retained four-layer, six-family corpus measures every instance at widths 512--896; a 17,301,504-byte activation set is reduced below a 2 MiB budget independently per target. **Remaining:** general nested/loop CFG lifetime expressions and model-derived rather than packet-configured memory budgets. |
