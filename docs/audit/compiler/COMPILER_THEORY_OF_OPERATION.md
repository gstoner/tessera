...
| **W3** | **Memory planning** (buffer reuse / liveness / allocation, global rematerialization) | Peak-memory is often the binding constraint in training; fusion alone doesn't plan buffers | Tile reuse groups materialize as aligned address-space-3 arenas plus typed offset views. Static arenas use real workgroup globals. The ROCm post-ROCDL materializer follows pointer-carrying LLVM descriptor aliases, colors runtime arenas by SSA lifetime interference, and records `aligned_sum_of_slot_maxima`: nested branch and loop-local lifetimes reuse slots, while outer-live values receive distinct aligned slots. CFG-forwarded size arguments resolve back to host-visible kernel-argument leaves; arbitrary local size arithmetic is rejected rather than under-sized. Exact gfx1151 execution verifies an outer 8,192-byte slot plus `max(4,097, 12,289, 32,001)` loop/branch slot at a 40,208-byte launch and one active block/CU. Rematerialization preserves explicit CLI/function override precedence, otherwise derives an activation-byte budget from device capacity/reserve, marked parameters, gradient/optimizer copies, and persistent state; dynamic parameters require explicit byte bounds. The retained measured gfx1151/AVX-512 cost corpus remains the selection cost source. **Remaining:** a serializable host expression carrier for arbitrary local size arithmetic, target/device capacity injection by production launchers, and model-level validation on full training graphs. |
