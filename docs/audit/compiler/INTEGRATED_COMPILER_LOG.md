...
**The more important result is one I did not get to conclude.** The 2026-09-18 packet recorded the panel axis as exhausted because 4x8 and 8x8 fell off a VGPR cliff. Compile-only measurement shows every panel compiling to `vgpr_count` 256 with the **shipped** 4x4 panel already spilling 126 registers (1433 at 4x8, 4247 at 8x8), and the ROCm generators set no occupancy attribute anywhere. An external RDNA4 FP8 recipe attributes exactly this to occupancy policy — one wave per SIMD gets the full 512-VGPR file — so a `waves-per-eu` option now stamps `rocdl.waves_per_eu`, plumbed through the pipeline and its cache key, defaulting to the backend default. It changed nothing at 0, 1 or 2. Two hypotheses survive and this loop separated neither: 256 is RDNA4's architectural wave32 limit and the 512 figure is a CDNA result that does not transfer, or the attribute is not reaching the LLVM function. The emitted backend artifact is already a serialized `gpu.binary`, the Target IR dump carries no `gpu.func`, and our extracted RDNA4 archive holds section titles with no VGPR-allocation text, so none of the three could settle it. Recorded as `ROCM-OCCUPANCY-1` and the panel docstring no longer states the cliff as a hardware fact — asserting which of the two it was, without excluding the other, is how the original "panel axis is exhausted" reading happened in the first place.
