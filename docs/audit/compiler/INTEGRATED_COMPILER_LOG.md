...
Remaining: **`AUTODIFF-SHAPE-WHILE-FORWARD-2026-09-17`.** With the gate fixed, three shape-varying `scf.while` tests compile and then **crash inside JIT-compiled code** on the forward invoke — the second defect, which the gate had masked. Pre-existing and independent: main's own `tessera-opt` emits a byte-identical forward and it faults identically; the declared product ABI matches the emitted signature exactly, so it is neither an ABI nor a harness mismatch; and the residual tape's 2 slots for a 3-iteration loop are intentional, so the sizing is not it. The three are skipped with that reason rather than left to segfault, because a crash takes the whole pytest process down and loses every later result — a deferral of a newly visible defect, not a fix. Separately, the x86 JIT AD lane those tests exercise has **no host in any automated check**, so defects in it surface only in a manual sweep.

**Two red zones nobody had swept, found by doing so.** Clearing one box prompted the first full unit sweep ever run on the other two. **Tajasarus (gfx1201) reports ~1578 failures and they are one defect**: the `test_rocm_*_compiled.py` families each document themselves as gfx1151 lanes, gfx1201 has no promoted family plugins, and their shared guard checks only that *a* usable AMD GPU exists — never which chip — so ~1500 tests assert a gfx1151 proof on a host that cannot have one and fail instead of skipping. Not fixed here on purpose: the guard is shared by 119 files and making it chip-aware would skip the lanes gfx1201 genuinely does have (scheduled packages, the WMMA audit, 2:4 sparse, the cooperative EBM kernel), trading false failures for false skips — the worse error. It needs a per-family answer with owning-device confirmation. **Super-Bear reports 48, none of them this branch's** — measured with main and the branch each built from its own sources in a fresh worktree, same configure: main 50, branch 48, nothing failing only on the branch. The first attempt compared the lived-in branch tree against a fresh worktree of main and reported four regressions, all `UnicodeDecodeError` from 49 gitignored AppleDouble resource forks present in the lived-in tree only; three source-registry gates were bitten and are now debris-proof, and `tests/conftest.py` warns at session start when such forks are present, because `git status` reads clean over them. A bisect that varies the tree as well as the commit attributes tree noise to the commit; use two worktrees.
