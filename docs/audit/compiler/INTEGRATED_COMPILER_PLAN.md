...
**The scale contract exists; the plain matmul and the ROCm route do not carry it**

- Owner: [COMPILER_REFACTOR_PLAN.md](COMPILER_REFACTOR_PLAN.md)
- Gate: **Corrected 2026-09-19.** This entry previously said "Tessera has **no FP8 scale concept at all** — no `block_shape`, `weight_scale` or `scale_inv` anywhere in `python/tessera/`". That was wrong, and wrong by searching in vLLM's vocabulary and reading zero hits as zero capability — the same failure this session twice flagged in the diagnostic scanner's prefix allowlist. What exists, under Tessera's own names: `microscaling.ScaleLayout` (block_size along an axis, `scale_dtype` ∈ e8m0 / **fp8_e4m3, named for NVFP4** / fp32, with `block_size == 0` meaning per-tensor), `LowPrecisionFormat` (element + layout) and `format_for_dtype`; the sibling `grouped_layout.ScaleLayout` with granularity/block/packed format; and `Tessera_ScaleLayoutAttr` as a first-class IR attribute carried by **four grouped-GEMM ops** alongside optional `x_scale`/`w_scale` operands. It has real consumers — `runtime.py`, `apple_packed_numeric.py`, `apple_target.py`, `backend_manifest.py`, `capabilities.py`, `execution_matrix.py` — and `rocm_isa_contract.py` already carries a `microscaling: bool` with mxfp4/6/8 marked `planned_gated`. **The actual gap is narrower and the work is smaller than recorded:** (1) plain `tessera.matmul` carries **no** scale operands — zero references, only the grouped GEMM ops have them; (2) the ROCm typed route (`scheduled_matmul.py`, `rocm_native.py`, `rocm_pipeline.py`) never consults the scale model at all. Gate: extend the existing contract to the plain matmul (operands + `scale_layout`, not a new vocabulary), teach the ROCm typed route to read it, and fail closed by name where a pairing cannot be honoured (Decision #21a) — with the scale carried through Schedule into the descriptor rather than dropped (Decision #32). The 20 tuned vLLM configs for the R9700 remain the measured target: `dtype=fp8_w8a8`, `block_shape=[128,128]`, and our route still picks a 16×16 macro tile for all 60 shape/M combinations against their 64/128/128.
- Depends on: —
- Start: device
- Latest: [ROCM-MIXED-FP8-1: the mixed OCP FP8 pairs execute, and two gates that were not checking what they claimed](INTEGRATED_COMPILER_LOG.md#2026-09-19--rocm-mixed-fp8-1-the-mixed-ocp-fp8-pairs-execute-and-two-gates-that-were-not-checking-what-they-claimed)
...
- Start: host-free
- Latest: [recorded increment](INTEGRATED_COMPILER_LOG.md#2026-09-15--matmul-epilogue-markers-from-both-frontends-activation-after-the-reduction)
