...
| 6 | **E2E-REAL-6 — delete duplicate authorities — first cohort landing 2026-08-11.** | The tracer now promotes directly to verified canonical Graph IR and owns native forward-product specialization. Family plugins, rather than `JitFn`, own reduction, normalization, FFT, DCT, and compound-spectral product planning. The v2 parent binds source-Graph and paired-JVP digests. ROCm scheduled attention backward consumes its content-addressed Tile artifact directly instead of reconstructing Graph IR. General solver products generate exact residual/JVP/VJP children for typed pointwise, sum/mean, rank-2 reduced-storage matmul, bounded-dynamic/mixed-storage, distinct parameter-space, statically counted regions, and pure scalar data-dependent `if`/bounded `while`. Continue family-by-family before deleting the AST frontend or retained backend candidates. **Current authority debt is concrete:** decoration still constructs AST-owned Graph IR through `_OpExtractor`; ordinary straight-line `JitFn.__call__` does not trace; and `JitFn` still owns target/family backward selection plus the `_native_*` implementations. Migrate selection and package construction behind explicit family plugins before switching the default frontend. | Native-JVP tests prove one plugin owner and digest-bound children; expanded 30-sample WSL packets provide exact AVX-512/gfx1151 correctness proof for all listed solver families plus ISTFT window differentiation. Direct program tests prove reduction broadcast duality, matmul JVP/VJP formulas, bounded-region expansion, predicate replay, and runtime shape/dtype guards. The fresh gfx1151 multi-clock probe confirms valid `wall_clock64` but zero HIP events and no ROCprofiler activity on WSL, so these timings remain regression-only. Global Decision #31 closeout remains open until every retained second implementation is named `oracle` or `candidate` and differentially gated. |
...
| # | Item | Effort |
|---|---|---|
| W4.1 | **Portable structured tracing landed.** `for`/`if` and bounded `while_loop` carry nested bodies into real SCF. Apple may retain an architecture-owned physical choice, but the semantic IR is no longer Apple-only. General source-CFG recovery and multi-block regions remain open. | landing |
| W4.2 | **Fail-closed affine boundary landed.** Python owns canonical integer-affine expressions with three-valued `proven`/`violated`/`unknown` legality queries; variable products remain explicitly nonlinear. `SymbolicDimEqualityPass` propagates symbolic identity through calls and `scf.for`/`if`/`while` and retains concrete-witness checking for nonlinear products. Remaining: a typed Presburger carrier and proofs for unresolved affine systems. | landing |
| W4.3 | **Single-block SCF reverse execution landed.** `RegionAdjointInterface` differentiates `scf.if`, positive-step counted `scf.for`, and the tracer's canonical bounded `scf.while`, including implicit-capture cotangents. Loops use an exact recompute-all reverse schedule; effects and noncanonical while forms fail closed. `tessera.control_scan` itself is already registered, traced, lowered through Tile, and executable in the proven ROCm/NVIDIA forward lanes; the open item is its region adjoint/JVP and checkpoint-policy consumption, not forward control-flow execution. Remaining: measured checkpoint schedules, multi-block/general control flow, native region-product execution, and scan differentiation. | landing |
...
| W5.2c | **COMP-SCHED-OVERLAP-1/R3 — closed 2026-08-10.** The compiler-owned `composition_cost` boundary consumes R2 vectors as explicit Tile actions, validates dependency DAGs and calibration provenance/digests, searches bounded legal topological orders, and models independent compute, memory, and communication lanes with queue serialization. Results are stamped `eligible_for_promotion = false` and `selector_authority = latency_ms`. Only exhaustively searched candidates outside an explicit margin may be pruned; bounded/inexact searches are retained. Direct tests reproduce the M3 scalar-order reversal and prove that no winner API exists. W2.1/W2.2 now supply registered effect, alias, liveness, and value-scoped memory-dependence queries. Existing R3 inputs retain explicit dependency edges; automatic synthesis for future Graph-derived Tile DAG producers is follow-on integration and does not reopen the completed rank/prune model. | TileRT R3 + TileSight T3 | closed |
...
| W5.2e | **Automatic dependence-edge generation and selector proof.** Convert W2.1 value, alias, liveness, registered-effect, memory-dependence, mutation, stochastic-identity, and ordered-collective facts into explicit action-DAG edges. Unknown facts create conservative edges; they never disappear. Differentially compare generated edges with hand-authored R3 fixtures, then collect clean calibrated target packets before allowing scalar measured latency to select a retained schedule. WSL and analytical vectors remain prune/rank-only. | W2.1 + TileRT R3/R4 | open |
...
| W5.4 | **Sharding propagation (GSPMD/Shardy-style).** The generated queue splits into a local-layout proof (`factorized_matmul`), native-transport rows (`moe_dispatch`/`moe_combine`), and attention/EBM/linalg/sparse/spectral/state domain rules. Build a typed lattice over replicated, tiled, partial-reduction, and unknown placements; propagate to a fixed point; insert reshard collectives only from explicit incompatibilities; and fail closed at region/effect/alias boundaries. Mock-mesh proofs precede native transport packets. | Sweep §F4 | re-estimate by family |
...
| # | Item | Source | Effort |
|---|---|---|---|
| W6.1 | **Bounded native product landed (`AD-FWD-CORE-1` / `AD-FWD-PRODUCT-2` / `AD-FWD-NATIVE-1` / `AD-FWD-DIST-3`).** `TangentInterface`, paired functions, public request/provenance, stable `wrt_indices`, and direct tangent families are live. Public `jvp`/`jacfwd` never silently substitute finite differences, and bounded SCF carries primal/tangent state together. The content-addressed parent supports multiple active operands and named children. Sum, RMSNorm, affine LayerNorm, RFFT, spectral-filter, DCT, generated typed matrix-free solver, and exact ISTFT spectrum/window products pass their bounded correctness gates. Solver generation includes reduction, rank-2 reduced-storage matmul, bounded-dynamic/mixed-storage, counted regions, and pure scalar predicate replay, with exact AVX-512/gfx1151 correctness packets. The ISTFT rule differentiates both overlap-add numerator and window-energy denominator. Typed collectives reach NCCL/RCCL launch contracts. Remaining: exact forward-over-reverse HVP, broader ISTFT layouts/dtypes, MPI/OFI/SHMEM and subgroup transport, native multi-rank packets, clean performance packets, and Apple/NVIDIA packages. | Autodiff D2 | landing |
