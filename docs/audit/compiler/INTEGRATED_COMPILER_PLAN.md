...
- Gate: Exact dense unmasked f32 attention loops now raise to a symbolic recipe and instantiate two native buckets through Schedule/Tile. Extend recognition, bind executable attention candidates and measure them; preserve complete witnesses and IR/image-bound admission.
- Depends on: [E2E-REAL-6](#e2e-real-6): canonical artifact boundary for this workload, not every family migration.
- Start: host-free
- Latest: [recorded increment](INTEGRATED_COMPILER_LOG.md#2026-09-10--native-heap-attention-recipes-and-ssd-device-proof)
