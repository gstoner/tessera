...
`lds_copy_width` defaults to **1** (scalar, the measured-faster arm); the vector path stays reachable behind the knob as the measured-negative arm. Remaining gate, unchanged in target and changed in shape: `global_load_b128` → `ds_store_b128` requires an **unmasked** `vector.load` on the in-bounds common path with the ragged tail split into its own branch — two paths, deliberately. See `docs/backends/rocm/wmma-fragment-layout.md` §10j.
- Depends on: —
- Start: device
- Latest: [ROCM-MIXED-FP8-1: the mixed OCP FP8 pairs execute, and two gates that were not checking what they claimed](INTEGRATED_COMPILER_LOG.md#2026-09-19--rocm-mixed-fp8-1-the-mixed-ocp-fp8-pairs-execute-and-two-gates-that-were-not-checking-what-they-claimed)
