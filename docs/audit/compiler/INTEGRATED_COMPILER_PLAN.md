...
| W2.3 | **Straight-line client landed; region interface remains.** Reverse AD consumes W2.1 activity and `stop_gradient` semantics instead of its private walker. A region-aware `ActivityInterface`, loop-carried activity, and inactive-region pruning remain coupled to W4 structured differentiation. | Autodiff D3 | landing |
...
| # | Item | Source | Effort |
|---|---|---|---|
| W6.1 | **Bounded native product landed (`AD-FWD-CORE-1` / `AD-FWD-PRODUCT-2` / `AD-FWD-NATIVE-1`).** `TangentInterface`, paired functions, public request/provenance, stable `wrt_indices`, and 35 direct tangent families are live. The content-addressed parent now supports multiple active operands and named child outputs. Sum, RMSNorm, affine LayerNorm, RFFT, spectral-filter, and diagonal matrix-free solver JVPs pass AVX-512 and exact-device gfx1151 correctness; solver lineage distinguishes residual-JVP/non-transposed solve from the adjoint chain. Native collective products require a live multi-rank NCCL/RCCL adapter and reject nonlinear reductions. Remaining: native transport packets, broader solver/spectral products, Apple/NVIDIA packages, exact HVP/jacfwd, then structured-region differentiation. | Autodiff D2 | landing |
