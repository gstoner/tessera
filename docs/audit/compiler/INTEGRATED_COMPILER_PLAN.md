...
| 5E | **TSOL-ROCM-E2E-1 — compound spectral programs** — **typed Schedule→Tile carriers plus bounded x86/Zen 5 and ROCm/gfx1151 consumers implemented 2026-08-06.** | `schedule.spectral_program` content-addresses child FFT Schedule/Tile digests, layout, axis, pad/crop, window/hop/frames, normalization, exact persistent workspace, accumulation, native entry, and mutation lineage for `spectral_filter`, `dct`, `spectral_conv`, `stft`, and `istft`. The production Schedule→Tile pass re-derives the digest and emits one `tile.spectral_program_kernel`; both runtimes consume that exact artifact without Graph re-entry. The native x86 image owns framing, packing, complex multiply, overlap-add, and thread-local digest-bound workspace. The prebuilt HIP image keeps the same work device-resident and owns a bounded digest-keyed composite plan whose device allocation persists across calls. | Production `tessera-opt` structural/tamper gates pass. Exact Zen 5 aligned, ragged, and prime/Bluestein cases and exact WSL-visible gfx1151 aligned, batched, ragged, and prime/Bluestein cases agree with NumPy. The loader prioritizes exact-target ROCm build trees so a generic code object cannot shadow the gfx1151 package. `gfx1200`/`gfx1250` fail closed; Apple/NVIDIA physical consumers remain architecture-owned follow-ups. |
| 5F | **ROCM-MATH-EVIDENCE — stable statistics and boundary semantics** — **bounded gfx1151 correctness slice implemented 2026-08-06.** | ROCm var/std use centered parallel Welford. Unary/binary codegen preserves `expm1(-0)`, NumPy signed-zero min/max, infinity-aware modulo/floor division, and the shared atan2 helper preserves signed zero, infinity diagonals, and NaNs. | Exact gfx1151 tests cover large-offset variance, arbitrary/ragged axes, fp16/bf16 storage, large-argument trig, log/exp boundaries, lgamma/digamma poles, and 4097-element scans. This is correctness evidence only, not a selector/performance promotion; sibling backends require their own physical validation. |
