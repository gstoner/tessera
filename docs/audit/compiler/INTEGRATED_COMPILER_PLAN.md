...
| # | Item | Source | Effort |
|---|---|---|---|
| W1.1 | **Design landed 2026-08-03: [`W1_1_TYPING_DESIGN.md`](W1_1_TYPING_DESIGN.md)** (inventory: [`W1_1_TYPING_INVENTORY.md`](W1_1_TYPING_INVENTORY.md), 2026-08-02). **Decisive finding: the typed fragment form cannot cross a block-argument edge.** `MMAOp::verify()` recovers the contract by chasing the producing op, and a block argument has none — so neither a function argument nor an `scf.for` `iter_args` accumulator verifies. A K-loop accumulator is a block argument by construction, so the typed form is unusable by every real GEMM; that explains the zero-producer count better than the (now-fixed) token conflict does. **Consequence: parameterize the type FIRST, then migrate producers** — the reverse of the inventory's order, which would have stalled on the K-loop with the type system half-changed. Split is principled: the type carries what makes two fragments interchangeable (m/n/k, elem, acc, role, layout **and `family`**), `#tile.mma_desc` keeps how the op executes (`k_blocks`). `family` moved into the type after PR #501 review: it selects a physical register ABI (wave 32 for RDNA/WMMA vs 64 for CDNA/MFMA), and `TileToROCM` says outright that those descriptors "are intentionally non-interchangeable" — so leaving it on the op would have weakened an existing contract exactly at the block-argument edge this design opens. **Verifying is not lowering:** both backends also require the accumulator's direct defining op to be `FragmentZeroOp` (3 sites each in `NVIDIALowering.cpp` / `TileToROCM.cpp`, and only those two files), so a typed K-loop would verify and still not compile — hence a block-argument-aware target-lowering step with per-backend fixtures, and steps 3–5 are routine rather than mechanical. Blast radius is smaller than 5w implies (3 fixture files, 3 C++ files, 7 `FragmentType` uses) precisely because the typed form has no adoption; re-cut the estimate after step 2. Composes with W1.3: the interim carried `numeric_policy` attribute becomes `!tile.fragment<elem, acc>`, at which point the boundary verifier's `represented_in_type` reason applies. — the precondition this item states ("inventory every backend producer/consumer before tightening each op"). Headline correction: **W1.1 is a migration already in progress, not a design job.** `MMAOp::verify()` already enforces a full typed-fragment contract and explicitly keeps a legacy permissive branch "during migration"; but **no C++ pass emits the typed form** (0 of 10 producers set `#tile.mma_desc` — the only producers are Python text emitters), and **no lit fixture pairs `!tile.fragment` with `tile.mma`**. Also found: `Tile_AsyncTokenType` is declared and never used (a live Decision #29 violation), and `tile.async_copy` has no verifier at all, so it has no typed form to migrate onto — those two close each other. Recommended order and reproducible counts are in the inventory. **Finish typing true Tile primitives and verify target matrix variants.** Preserve the typed alloc/pipeline/TMA/mbarrier/TMEM ops; parameterize `!tile.fragment`/`!tile.buffer` on element type, tile shape, layout, memory space, role, and accumulator `numeric_policy`. Define target contracts by `(arch, instruction, shape, operand dtype, accumulator dtype, role)`; treat the documented gfx11 f16 vectors as one fixture, not a universal ROCm/NVIDIA signature. Inventory every backend producer/consumer before tightening each op | IR Stack §U1 + Target §X2 | 5w design/migration estimate |
