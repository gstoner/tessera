...
**The "8x" in this entry is withdrawn 2026-09-19.** It came from the same broken harness; corrected, the LDS body is **2.2–4.0x** off the register body, not 8x. Three of the four lines pointing at the staging copy are external and stand (the radiance kernel's 9.5 TFLOP/s ungated-global result, AMD's LDS-heavy Tensile library, CK's `ScalarPerVector=8`); the fourth was ours and is gone. The copy **is** vectorised now — A masked-loads and vector-stores `vecW` contiguous K, B vector-loads its coalesced global side and scalar-stores into the transposed layout, with `vecW` derived from the padded stride since a V-wide vector needs every LDS row start V-aligned. **Its benefit is UNMEASURED**: the corrected grid was only ever run against the vectorised code, so there is no valid scalar baseline. Gate: restore the scalar copy behind the corrected grid, measure the delta, and only then claim anything for the vectorisation.
- Depends on: —
- Start: device
- Latest: [ROCM-MIXED-FP8-1: the mixed OCP FP8 pairs execute, and two gates that were not checking what they claimed](INTEGRATED_COMPILER_LOG.md#2026-09-19--rocm-mixed-fp8-1-the-mixed-ocp-fp8-pairs-execute-and-two-gates-that-were-not-checking-what-they-claimed)
...
**Implementation path, scoped 2026-09-19 — it reuses the K unroll rather than restructuring the accumulation.** The typed K loop is an `scf::ForOp` carrying the accumulators as iter_args, with `mainPanel(...)` threading them through `tile.mma` (`C += A*B`, in place). Block scaling needs *this K block's* product isolated before it is scaled and added, which reads at first like a structural change — but the existing `kUnroll` mechanism already issues N panels per iteration with the accumulators threaded through, so the block-scaled form is that same loop with two edits: **start each iteration from zeroed accumulators** instead of the carried ones, and after the N panels **scale by the outer product and add into the carried set**. The unroll depth then *is* the scale group: `unroll = GROUP_K / fragK` — 128/16 = **8 panels per scale block** — which is exactly the `GROUP_K == BLOCK_SIZE_K` invariant AITER asserts, arrived at from the other direction. So the change is contained to the K-loop body plus the scale operands reaching it, not a rewrite of how accumulation works.
- Depends on: —
- Start: device
- Latest: [ROCM-MIXED-FP8-1: the mixed OCP FP8 pairs execute, and two gates that were not checking what they claimed](INTEGRATED_COMPILER_LOG.md#2026-09-19--rocm-mixed-fp8-1-the-mixed-ocp-fp8-pairs-execute-and-two-gates-that-were-not-checking-what-they-claimed)
