...
| W5.2a | **COMP-SCHED-OVERLAP-1/R1 — explicit-lineage await sinking.** First migrate remaining Python/stage-only async carriers to explicit future/token lineage, then sink awaits to the first true SSA use. Movement justified only by the explicit edge may proceed before W2.2; all alias/effect-dependent movement waits for W2.2. | TileRT R1 + async reconciliation | landing |
| W5.2b | **COMP-SCHED-OVERLAP-1/R2 — measured resource vectors.** Add compute time, bytes moved, communication bytes, queue/resource identity, and provenance to the declared open benchmark metadata. Scalar measured latency remains selector authority until a composition consumer is proven. | TileRT R2 + Evaluator | open |
| W5.2c | **COMP-SCHED-OVERLAP-1/R3 — Tile action-DAG cost model.** Consume calibrated resource vectors and legal topological orders as a prune/rank model; never promote from analytical output alone. Depends on W2.2 and W5.2b. | TileRT R3 + TileSight T3 | open |
| W5.2d | **COMP-SCHED-OVERLAP-1/R4 — MoE overlap consumer.** Connect the existing communication-overlap analysis to the live MegaMoE pipeline with chunk slicing, true-use waits, capacity limits, and deterministic reduction order. Native multi-device transport and packets remain architecture-owned. | TileRT R4 + TileSight T4 | open |
...
| # | Item | Source | Effort |
|---|---|---|---|
| W6.1 | **Bounded compiler foundation landed (`AD-FWD-CORE-1` / `AD-FWD-PRODUCT-2`).** `TangentInterface`, paired forward functions, public request/provenance ABI, stable `wrt_indices`, direct straight-line tangent families, and CPU numerical oracles are live; unsupported active operations and regions fail closed. Remaining: reduction/normalization, spectral, solver, and collective tangent expansion; architecture-owned native JVP packages; exact HVP/jacfwd products; then structured-region differentiation. | Autodiff D2 | landing |
