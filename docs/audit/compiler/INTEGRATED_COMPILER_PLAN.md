...
| 5E | **TSOL-ROCM-E2E-1 — compound spectral programs** — **bounded gfx1151 package implemented 2026-08-06; shared ODS carrier remains landing.** | A content-addressed compound artifact binds child FFT Schedule/Tile digests, layout, axis, pad/crop, window/hop/frames, normalization, exact workspace, accumulation, native entry, and mutation lineage for `spectral_filter`, `dct`, `spectral_conv`, `stft`, and `istft`. The prebuilt HIP image keeps all intermediate framing/packing/multiply/overlap-add work device-resident and reuses persistent child plans. Runtime requires the compound artifact and does not reconstruct the program from Graph metadata. | Exact gfx1151 aligned, batched, ragged, and prime/Bluestein cases agree with NumPy; tampering and gfx1200/gfx1250 reject. This is not yet claimed as complete Graph→Schedule→Tile E2E: an ODS compound Schedule/launch Tile carrier and an x86 physical consumer remain open. |
