...
| W2.4a | **CAKE Phase 1 — typed Tile sync/memory surface** (owning row for [`compiler_enhancement.md`](compiler_enhancement.md) §5). **§5.1 experiments run and §5.2 rows 1–5 landed 2026-08-15** (§5.1.1/§5.2.1): both hardest-case experiments **fail open** (block-arg edge invisible to every legality pass; stale pipeline ring silent); the wait gains a typed `!tile.mbarrier_token` slot (arrive→wait expressible), dependencies fail closed on untyped values, gates-on-nothing waits/copies are rejected, keyless retire-all is an explicit marker, descriptor/TMEM/alloc operands typed. Negative fixtures: `tests/tessera-ir/phase2/tile_sync_typed_invalid.mlir`; probes: `research/tile_sync/`. **§5.3 first increment landed same day:** `TileDataflowLegalityPass` (W2.4's named pass) + the shared `TileValueProvenance` loop-carry resolver derive arrive→wait pairing with slot identity, barrier origin, pipeline-ring advancement, and SSA-keyed TMA expect agreement across `scf.for` block-argument edges, failing closed on the underivable; WarpSpecLegality's producer walk resolves through the same resolver. Every §5.1.1 silent probe now fires (`tests/tessera-ir/phase2/tile_dataflow_legality.mlir`; lit 324/0). **§5.3/§5.4 closed same day (second increment):** four registered sync ops (`cta_sync`/`fence`/`tma.store`/`buffer_write`) absorb the husk vocabulary (WarpSpecialization's by-name emission resolves to registered ops unchanged); the six substring predicates are typed `isa<>` queries; the five unproduced attribute escape hatches are deleted; both legality fixtures run without `--allow-unregistered-dialect` (the §3.3 disjoint-sets finding closed); and `--tessera-tile-dataflow-legality` is wired into both NVIDIA pipelines, whose own lowered output passes the derivation gate. Exact-name matches remain only for `cp_async.commit_group` + the two producer-less collectives (annotated per Decision #29). **Open:** §5.5 gate 5 gfx1151 numerics; barrier-at-birth emission restructure (rows 6–7 revision); row 8 after W1.1 step 5. CAKE Phase 2 (roles) is unblocked on the verifier side. | CAKE §5 | floor + §5.3/§5.4 closed; HW gate + rows 6–8 open |
