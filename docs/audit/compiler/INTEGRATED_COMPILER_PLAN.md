...
| W4.3 | **Single-block SCF products plus native residual SSA execution landed for scan-form and generic counted loops.** `RegionAdjointInterface` differentiates `scf.if`, positive-step counted `scf.for`, canonical bounded `scf.while`, and lowered `control_scan`. The execution-derived `tessera.region_residual_abi.v1` binds the exact checkpoint set, CFG identity, state contract, and residual digest. SAVE performs zero replay; HYBRID stores only selected checkpoints and replays from the nearest predecessor. Generic counted loops now allocate one typed tape per statically shaped carried tensor, preserving different shapes/dtypes and stable per-state identities instead of packing them unsafely. Data-dependent `scf.if` exposes the taken predicate and canonical `scf.while` exposes the executed trip count. Remaining: branch-local saved intermediates, bounded `while` state tapes, dynamic carried shapes, general source CFG, native region-product execution, and target packets. | landing |
