...
- Depends on: —
- Start: device
- Latest: [ROCM-SPLIT-K-1: ordered cross-workgroup split-K lands on gfx1201](INTEGRATED_COMPILER_LOG.md#2026-09-26--rocm-split-k-1-ordered-cross-workgroup-split-k-lands-on-gfx1201)
...
**Corrected 2026-09-25 — the "default is now 4" below is superseded.** Commit `61bdb2b5` (2026-09-20) moved the default **4 → 1** on paired, interleaved, 6-rep evidence from *both* parts — gfx1201 1.10–1.19×, gfx1151 1.21× at 2048³ and 1.29× at 4096³, 6/6 in every cell, double-buffering on and off — and the code carries it (`kDefaultLdsPadDwords = 1`, `GenerateWMMAGemmKernel.cpp`; `lds_pad_dwords: int = 1`, `rocm_pipeline.py`). §10k's 4 rested on one shape in a configuration predating issue depth and double-buffering. See `docs/backends/rocm/wmma-fragment-layout.md` §10u/§10v. The text below is kept as the record of how 4 was reached.
...
**Corrected 2026-09-25 — the K1-blocked layout was refuted before building (2026-09-20, `wmma-fragment-layout.md` §10u); do not queue it.** K1-blocking keeps the same 2-way fragment-read conflict, so its only benefit is a smaller LDS footprint (A 3072 → 2048 elements). That pays only when occupancy is LDS-limited, and on gfx1201 at 4096³ it is not: LDS binds only at pad=4; at pad ≤ 2 the 244-VGPR ceiling caps residency at 6 workgroups, and throughput tracks that exactly. The loop's actual yield was the pad-default correction in [ROCM-LDS-BANKPAD-1](#rocm-lds-bankpad-1). What remains open here is a lever that attacks the **VGPR** ceiling on the LDS body (or a demonstrated LDS-bound configuration), not a wider copy and not K1.
- Depends on: —
- Start: device
- Latest: [ROCM-MIXED-FP8-1: the mixed OCP FP8 pairs execute, and two gates that were not checking what they claimed](INTEGRATED_COMPILER_LOG.md#2026-09-19--rocm-mixed-fp8-1-the-mixed-ocp-fp8-pairs-execute-and-two-gates-that-were-not-checking-what-they-claimed)
