...
| 1 | **E2E-REAL-1 — make Schedule IR real** — **registration slice implemented; SSA lowering remains open.** | The generated Schedule ODS library is compiled, linked into, and registered by both full and lean production `tessera-opt`; its op verifiers now execute. Still open: the mixed-level SSA contract, replacing `__pending__` with a content-addressed static-matmul artifact, and removing the annotation skeleton when E2E-REAL-2 consumes it. | LLVM/MLIR 23 production build succeeds; `--show-dialects` lists `schedule`; positive and fail-closed fixtures parse without `--allow-unregistered-dialect`. Pass help continues to label Graph→Schedule and Schedule→Tile as annotation-only. |
