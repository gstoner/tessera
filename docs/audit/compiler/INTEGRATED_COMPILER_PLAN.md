...
|---:|---|---|---|---|
| 1 | **E2E-REAL-6E — bounded stateful-training native VJP authority complete; family migration continues** | Lion, factored/full Adafactor, and the causal gated/Kimi/modified DeltaNet backward families now use explicit non-reexecuting plugins. Adafactor has a compiler-visible full/factored tensor-state ABI; each plugin binds tracer Graph identity, one-execution frontend proof, versioned state/workspace lineage, Schedule, exact Tile, and aggregate package identity. `JitFn` no longer constructs x86 or ROCm packages for these families. NVIDIA's existing Lion/DeltaNet compatibility packages and Apple sequence-mixer package remain architecture-owned follow-ups rather than inheriting this authority. Continue one family per PR until `JitFn` contains call binding only. | One-execution Adafactor and sequence-mixer regressions; flat/full/factored numerical oracle; package/state/Schedule/Tile tamper validation; existing AVX-512 and WSL-visible gfx1151 physical cohorts remain green; runtime receives no source Graph or operation metadata from the compiler route. | E2E-REAL-6D and the typed E2E-REAL-5C state-lineage carriers. |
...
**E2E-REAL-6E is complete:** bounded factored/full Adafactor and causal
sequence-mixer backward now share the non-reexecuting state-lineage plugin
boundary established by Lion. The next authority slice should migrate another
remaining compatibility family selected from the live `JitFn` inventory;
