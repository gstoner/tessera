...
| 6 | **E2E-REAL-6 — delete duplicate authorities — third cohort landing 2026-08-12.** | Ordinary pure straight-line tensor calls cache tracer-owned canonical Graph IR; native forward and reverse products now consume that module rather than decoration-time AST Graph. A mandatory, cached content-addressed differential certificate compares SSA-independent topology and concrete outputs before native JVP and pure native VJP execution. It refuses effectful/stochastic double execution. Native-product plugins own family selection and v3 package construction. Reduction now uses two actual `schedule.reduce → tile.reduce_kernel → native descriptor` children. Normalization uses a digest-validated composite Schedule action program for forward normalization, tangent projection, and affine products and no longer reads Graph-op metadata at runtime. FFT/DCT/compound spectral retain their canonical carriers. `JitFn` retains call binding/cache and backward helper implementations, not source-Graph authority or JVP package assembly. **Remaining debt:** `_OpExtractor` remains only as the named decoration-time candidate and CPU/oracle substrate; effectful/unmigrated execution needs non-reexecuting proof; backward family dispatch and `_native_*` helper bodies still need extraction into plugins. | Exact WSL-visible gfx1151 tests pass for reduction, normalization, FFT/DCT, affine normalization, compound spectral, and ISTFT products. The new v2 packet binds source, paired-JVP, Schedule-program, Tile-program, and parent digests. A real audit defect was fixed: AST capture alphabetically swapped layer-normalization beta/gamma; the declared ABI is now data/gamma/beta. Derived spectral lengths are normalized before differential comparison. x86 packaging remains blocked on this host because its production AVX-512 shared image is unavailable. Global Decision #31 remains open for effectful families and backward-helper extraction. |
...
| W4.2 | **Typed Presburger boundary, including exact divisibility, landed 2026-08-12.** Python emits a versioned, content-addressed coefficient-vector carrier (`eq`/`ge`/`mod`) before compilation. `SymbolicDimEqualityPass` consumes it with MLIR `IntegerPolyhedron`; each modular row introduces an existential local quotient and the exact equality `expr - remainder - modulus*q = 0`. Available dimension witnesses inhabit the same integer set, while malformed, inconsistent, and integer-empty systems fail closed. Range, equality, and `Divisible` frontend constraints use this typed path; nonlinear products remain explicit compatibility guards. Symbol identity still propagates through calls and single-block `scf.for`/`if`/`while`. Remaining: general source-CFG/multi-block capture and carrying typed constraints through those general regions. | landing |
| W4.3 | **Single-block SCF products plus `control_scan` JVP/VJP landed.** `RegionAdjointInterface` differentiates `scf.if`, positive-step counted `scf.for`, and the tracer's canonical bounded `scf.while`, including implicit-capture cotangents. `control_scan` now lowers its body into one `scf.for` carrying recurrent state and stacked output; exact `extract_slice`/overwrite-`insert_slice` tangent and transpose rules make both products executable. Forward AD preserves tangent iter-args even when an initially-zero carried tangent becomes active in the body. The C++ product stamps and consumes `recompute_all`; requested SAVE/HYBRID policies fail closed until residual ABI lowering is real. Remaining: selected checkpoint-plan lowering, multi-block/general source CFG, native region-product execution, and target packets. | landing |
