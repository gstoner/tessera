...
| 3 | **SO-3 + W5.2e-PRODUCER-1 — one schedule authority** | Stamp the Schedule Object digest through lowering, delete scalar pipeline reconstruction, and make two representative physical producers—spectral and collective/MoE—consume inferred dependence edges. **First producer adopted 2026-08-24: the MegaMoE R3 candidate now derives its edges from `infer_action_dag` over a registered-semantics Graph representation (`megamoe_graph_function`); the hand-authored plan DAG became the fail-closed coverage oracle (#31), additional conservative edges report separately, and the candidate carries a content-addressed `ScheduleObject` digest that binds the plan's own artifact digest. Adopting the first producer also EXPOSED three over-conservatisms in the shared inference that made any pipeline containing transport infer a total chain — ordered collectives serialized against all local work, registered effectful producers discarded their declared `aliasing="none"`, and any effectful/pure pair was assumed memory-dependent — each corrected with the sound rule and pinned by tests (a 12-action MegaMoE plan went from all 66 edges of the complete order to 36, oracle-covering, with the extras collective-ordering only). Remaining: the spectral producer (no action-DAG surface yet), digest stamping through the physical lowering seam, and the scalar-pipeline-reconstruction deletion.** | Generated DAG covers hand-authored oracle edges; unknown facts conservatively serialize; emitted artifact preserves digest/roles/resources; numerical output is unchanged on x86 and gfx1151. | SO-1/SO-2 and W5.2e inference. |
