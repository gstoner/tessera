...
audit_role: plan
plan_state: open
status: MSW-1 LANDED (2026-09-01) — reverse-over-reverse autodiff fails closed
        instead of returning a silent zero gradient. `Tape.shadowed_by_nested_tape`
        + `_forward_closed` gate; consumed by `grad` and `jacrev`; 12 regression
        tests (tests/unit/test_autodiff_nested_tape_guard.py) including the
        rematerialize non-regression. mypy clean. All other items open.
...
|---:|---|---|---|---|
| 1 | **MSW-1 — reverse-over-reverse fails closed** ✅ **landed 2026-09-01** | `Tape.shadowed_by_nested_tape`, set on the tape that *loses* the ops when an inner `tape()` opens while its forward is still running. `Tape._forward_closed` distinguishes that from the legitimate nested tape a VJP rule opens during backward (`rematerialize`). `grad` and `jacrev` consume the flag in their zero-gradient branches and raise `raise_nested_tape_shadowed`, naming the cause, AD-WEIL-1, and `hvp`. | Landed. 12 tests in `tests/unit/test_autodiff_nested_tape_guard.py`: both defect paths raise; the refused answer is proven wrong by analytic + finite difference; the diagnostic names cause/plan-item/alternative; forward and reverse nesting are symmetric; first-order `grad`/`jacrev`/`hvp` unchanged; a **genuinely unused argument still returns zeros** (no false positive); `rematerialize`'s nested tape is not flagged. 1433 autodiff-surface tests pass; mypy clean. | — |
