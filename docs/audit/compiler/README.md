...
| 11 | **E2E-REAL-6 — active family-migration cohort** | Native products use tracer-produced canonical Graph IR and explicit family plugins. E2E-REAL-6F adds SGD and Momentum/Nesterov on x86/gfx1151 plus Adam/AdamW on gfx1151 to the non-reexecuting state-lineage authority through typed `schedule.optimizer_vjp` → `tile.training_kernel`; unsupported target pairs fail closed. `JitFn` no longer owns those optimizer package helpers. `_OpExtractor` and other unmigrated native-backward families remain until their own lineage, differential, and target gates close. |
