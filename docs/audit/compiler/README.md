...
| 8 | **AD-FWD-PRODUCT-2 / AD-FWD-NATIVE-1 — bounded native products landed** | Public requests carry forward/JVP mode and stable `wrt_indices`; 35 `TangentInterface` families now include compound spectral product rules. A tamper-evident multi-active product artifact binds emitted JVP IR to native children. Sum, RMSNorm, affine LayerNorm, RFFT, spectral-filter, and diagonal matrix-free solver products execute on AVX-512 and gfx1151. Native NCCL/RCCL collective products are software-gated to a live multi-rank hardware adapter; exact transport evidence remains open. Exact HVP/jacfwd, Apple/NVIDIA consumption, structured regions, broader solver families, and clean performance packets remain open. |
...
| 10 | **COMP-SCHED-OVERLAP-1 R1–R4 + W2.2 — shared software boundary closed** | Explicit async lineage, registered Graph effect/alias/stochastic semantics, fixed-point call propagation, fail-closed await sinking, measured resource vectors, prune-only Tile action-DAG analysis, and the content-addressed MegaMoE overlap consumer are landed. Native transport and exact-device performance packets remain architecture-owned; W2.1 still gates automatic memory-dependence synthesis beyond explicit DAG edges. |
