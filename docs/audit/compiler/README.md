...
| 11 | **E2E-REAL-6 — active family-migration cohort** | Native products use tracer-produced canonical Graph IR and explicit family plugins. Normalization, bounded compound-spectral reverse, canonical rank-4 attention reverse, Lion, factored/full Adafactor, and causal sequence-mixer backward now declare Graph/Schedule/Tile/Target owners for x86/gfx1151. E2E-REAL-6E extends the structural non-reexecuting frontend certificate and exact state/workspace lineage to Adafactor and DeltaNet families; the state-bearing source executes once and the runtime receives no source Graph metadata. `JitFn` no longer constructs x86/ROCm packages for these migrated families. NVIDIA/Apple stateful packages remain explicit compatibility or target follow-ups; `_OpExtractor` and remaining native-backward helpers cannot be deleted until their lineage, correctness, and architecture-owned evidence are complete. |
