...
> | softmax / logsumexp / rmsnorm (gamma-less core) | **RETIRED (AD-RETIRE-2)** — production pairs are first-order specializations of the structured jets (`jet.register_jet_derived_structured_rules`); VJPs by the stated transpose structure (softmax and the rmsnorm kernel are symmetric — pullback = pushforward; logsumexp's transpose is the softmax broadcast-multiply); displaced hand rules join the #31 ledger | Envelope audit recorded in `jet.py`: axis / axis-None+keepdims / eps are the whole pair envelopes, all jet-covered; differential-tested across every combination at 1e-14 with a symmetric-adjoint law check. Adjacent gap CLOSED by AD-DATUM-POLYGAMMA: the hand `vjp_rmsnorm` was x-only while the canonical forward takes `gamma` (tape-reverse with γ predated retirement as broken), and the derived pair now carries the γ operand — dx via the symmetric core kernel on the scaled cotangent, dγ broadcast-reduced; proof is adjoint + finite differences + tape end-to-end (`test_retired_structured.py`), since no displaced oracle ever had a γ half |
...
> | lgamma / digamma | **RETIRED (AD-DATUM-POLYGAMMA)** — the polygamma tower now exists (`algebra._polygamma`: n = 0/1 are the displaced hand VJPs' own helpers verbatim, n ≥ 2 a machine-precision shift+asymptotic+reflection core), with `_jet_from_tower` composition and an AUXILIARY recurrence lookup (`recurrence_for`) so digamma's unbounded derivative chain (trigamma, ψ⁽²⁾, …) stays closed at every depth WITHOUT the tower rungs becoming production ops (#29). Values mirror the canonical forwards bit-for-bit; k=1 slopes carry the displaced series conventions | Proofs: exact anchors (−2ζ(3), π⁴/15, −24ζ(5), half-integer values), FD cross-order incl. the reflection branch, contour-integral sweep vs scipy loggamma/digamma through order 5, differential vs the displaced oracles, k=1..4 ODE-table + nested-dual, and the law sweep — all picked up by the existing parametrized tests when the entries were declared (`test_polygamma_datum.py` owns the tower-specific claims) |
