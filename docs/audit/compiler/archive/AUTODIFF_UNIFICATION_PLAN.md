...
> Archived on 2026-09-06 as a superseded review/design, not as evidence that its remaining work is complete. Current ownership and acceptance gates live in the parent audit and integrated compiler plan. Historical counts, estimates and external comparisons below are not current claims.
...
| `generated/runtime_execution_matrix.md` | Per-target **forward** executability truth | Source for backward runtime binding and exact-target `device_verified_jit` / `device_verified_abi` evidence | P0 (read) · P4 (write) |
...
## 3. The connection ledger — a *derived projection*, not a new registry

One row per **op family**, with exact-target proof columns. These are
**independent axes, not a single ordinal ladder**: a hand-written Tier-3 backward
can be device-verified without being emitted by `AutodiffPass`, while an
`ir_adjoint` can exist without any runtime binding. No "lowest rung wins" or
single leader status may erase that distinction. **The ledger introduces zero
new source of truth** — it is generated by *joining*
...
| `device_verified_jit` | generated target binary launched on the exact target and numerically verified? | backward `runtime_execution_matrix` row with exact evidence target + execute-and-compare fixture |
| `device_verified_abi` | shipped stable C ABI symbol launched on the exact target and numerically verified? | backward `runtime_execution_matrix` row with exact evidence target + execute-and-compare fixture |

`runtime_bound` is deliberately weaker than either device-verified axis. An
executable row, `execution_kind="native_gpu"`, or a family target such as `rocm`
is not device proof by itself. Device proof requires the canonical status, an
architecture-specific evidence target (for example `rocm_gfx1151`), and a
checked-in numerical fixture exercising that same lane. Summary counts and the
leaders list are derived from those row fields; prose does not name leaders.
...
> succeed and the first exact-target device-verified backward rows in the ledger.
...
counts and leader list from those fields; this plan intentionally does not copy
the live family list into prose (Decision #26).
...
### Pre-A2 state (historical provenance)
...
| Ledger | backward target/runtime/oracle/device-proof columns were **hardcoded `()`** ("empty by construction"). | `python/tessera/compiler/autodiff_ledger.py` history |
...
2. `autodiff_ledger.py`: replace the hardcoded backward fields with a read of
   the matrix. Keep all executable backward rows in `bwd_runtime_bound`, while
   verified rows additionally require canonical proof status, exact target, and
   an architecture-aligned numerical fixture.
...
**Exit:** the ledger's former zero-device-proof blind spot (§8) closes for
...
matrix, not asserted"* — is satisfied: A2 sources the ledger's runtime and
device-proof backward axes from the matrix; A3 makes
...
### Evidence-source decision — hardened 2026-07-12

The original Inc-1 rule treated `execution_kind` as device proof. That was too
weak: `native_gpu`/`native_cpu` proves dispatch classification, not generated-vs-
ABI provenance, exact architecture, or numerical comparison. Backward
`ExecutionRow` now carries `device_proof`, `evidence_target`, and
`numerical_fixture`. `native_backward_targets()` rejects an unknown proof status
or a verified claim without an exact target and checked-in fixture. This retains
