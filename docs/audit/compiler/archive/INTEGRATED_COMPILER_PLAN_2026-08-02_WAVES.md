...
# Integrated compiler sequencing and waves — archived provenance

Archived on 2026-09-07. This preserves the superseded central queue, E2E reset,
wave tables, estimates and governance copies as historical evidence. Dates,
status words, fleet facts and “next” lists below describe their recorded scope;
they are not current priority or execution claims.

Current owners and remaining gates: [integrated plan](../INTEGRATED_COMPILER_PLAN.md#routing-index).
The reconciliation section there carries unresolved obligations forward.
...
7. **E2E-REAL-6:** remove duplicate lowering authorities only after migrated
   ...
   **Gate ACTIVE (2026-08-20):** a migrated differentiable family also
   ...
   ([archived AD-LAW-1 record](AUTODIFF_NEXTGEN_PLAN.md) §7) — 300
...
| # | Item | Source | Effort |
|---|---|---|---|
| W5.1 | **Executable policy cohort landed; architecture packets remain open.** Complete-backward samples and unique retained residual allocations produce exact evidence; only exact-device rows stamp compiler selection attributes. Counted regions execute candidate plans and report the policy actually implemented (`SAVE`, `RECOMPUTE`, or `HYBRID`) plus forward, replay, and backward work. Model-only candidates remain ineligible. Remaining: lower chosen plans into the C++ region adjoint and collect exact-family x86/gfx1151 packets. The [deleted-functionality reassessment](../INTEGRATED_COMPILER_LOG.md#2026-09-05--deleted-functionality-reassessment) specifies the native policy-consumption gate; restoring inert EBM annotations is not completion. | Autodiff D5 + GA/EBM §1.5 | landing |
...
## Original foundation formulation

Preserved verbatim apart from rebased links; the live foundation map supersedes
these sequencing statements.
...
## Historical ID dispositions

### W0.1

Recorded row; not a current-priority claim.
...
Preserved for provenance; superseded by the current routing index.
