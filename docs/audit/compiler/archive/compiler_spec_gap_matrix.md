...
| Schedule IR | C++ `LowerScheduleToTargetPass` | Scaffold only — pseudocode body, never registered in `tessera-opt`/any pipeline; now fails loudly if invoked (was a silent no-op). Real Schedule→Target lowering is the Python spine above. | `src/compiler/tile_opt_fa4/lib/Conversion/TesseraScheduleToTarget/LowerScheduleToTarget.cpp` | stubbed | Do NOT claim a C++ Schedule→Target lowering; the working path is the Python object-model spine (G2). |
