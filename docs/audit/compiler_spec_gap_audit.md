...
| `docs/spec/CONFORMANCE.md` | The body uses better mixed status labels, but the header still says Phases 1-3 complete and Phases 4-6 planned; test-suite rows duplicate `tests/unit/`; static `mypy --strict` requirement is not validated in the current CI notes. | Rebuild from `COMPILER_REFERENCE.md`, remove stale header, and separate aspirational checks from currently run validation. |
