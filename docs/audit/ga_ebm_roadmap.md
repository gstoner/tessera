---
status: Active (development roadmap — all 6 scope-lock questions resolved; GA7/GA8/EBM5/EBM6 dialects build on MLIR 21; **all 17 of 17** GA primitives ship fused MSL kernels on Apple GPU and route through public APIs; **8 of 9 EBM primitives** ship native MSL kernels — all reachable through `tessera.ebm.*`; **`ebt_tiny` fused single-dispatch kernel** peaks at 116× speedup; **JIT / compiler bridge landed** — `tessera.compiler.jit_bridge.dispatch_via_manifest` connects Python frontends → manifest resolve → shared loader, with thread-local route trace recorded in the benchmark report; only `ebm_partition_exact` stays Python-ref. Single canonical status: [`docs/status/ga_ebm_milestone.md`](../status/ga_ebm_milestone.md))
