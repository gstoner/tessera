---
status: Active (development roadmap — all 6 scope-lock questions resolved; GA7/GA8/EBM5/EBM6 dialects build on MLIR 21; **all 17 of 17** GA primitives ship fused MSL kernels on Apple GPU and route through public APIs; **8 of 9 EBM primitives** ship native MSL kernels — all reachable through `tessera.ebm.*`; **`ebt_tiny` fused single-dispatch kernel** collapses refinement+energy+argmin into one MSL pass — break-even drops to `B=32,K=64,D=512/T=32` (17.9×) and peaks at **116× at `B=64,K=128,D=1024/T=256`**; `ga_feature_pipeline` workload 33× speedup; **public-API GPU coverage now complete** (17 GA + 9 native EBM, including new `ebm.ebt_tiny`). Only `ebm_partition_exact` stays Python-ref. Single canonical status: [`docs/status/ga_ebm_milestone.md`](../status/ga_ebm_milestone.md))
