...
| `Runtime execution matrix` | closed | 142 | 142 | 0 | apple_cpu=2, apple_gpu=11, cpu=2, nvidia_sm120=1, rocm=65, x86=61 | Add rows only when a launch path actually executes. |
...
| Item | Status | Ready | Total | Open | Detail | Next |
|---|---|---:|---:|---:|---|---|
| `Apple CPU` | closed | 143 | 143 | 0 | runtime: apple_cpu=2; target_map: fused=2, reference=139 | Keep as regression baseline for CPU value-call/runtime ABI. |
| `Apple GPU` | mixed | 151 | 152 | 1 | runtime: apple_gpu=11; target_map: absent=1, compiled=71, fused=58, hardware_verified=11 | Close the remaining absent target-map lane or document why it is host-only. |
