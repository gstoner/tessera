...
| `Runtime execution matrix` | closed | 144 | 144 | 0 | apple_cpu=2, apple_gpu=13, cpu=2, nvidia_sm120=1, rocm=65, x86=61 | Add rows only when a launch path actually executes. |
...
| Item | Status | Ready | Total | Open | Detail | Next |
|---|---|---:|---:|---:|---|---|
| `Apple CPU` | closed | 152 | 152 | 0 | runtime: apple_cpu=2; target_map: fused=2, reference=148 | Keep as regression baseline for CPU value-call/runtime ABI. |
| `Apple GPU` | mixed | 162 | 163 | 1 | runtime: apple_gpu=13; target_map: absent=1, compiled=80, fused=58, hardware_verified=11 | Close the remaining absent target-map lane or document why it is host-only. |
