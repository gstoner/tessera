...
| `Runtime execution matrix` | closed | 139 | 139 | 0 | apple_cpu=2, apple_gpu=8, cpu=2, nvidia_sm120=1, rocm=65, x86=61 | Add rows only when a launch path actually executes. |
...
| Item | Status | Ready | Total | Open | Detail | Next |
|---|---|---:|---:|---:|---|---|
| `Apple CPU` | closed | 132 | 132 | 0 | runtime: apple_cpu=2; target_map: fused=2, reference=128 | Keep as regression baseline for CPU value-call/runtime ABI. |
| `Apple GPU` | mixed | 137 | 138 | 1 | runtime: apple_gpu=8; target_map: absent=1, compiled=60, fused=58, hardware_verified=11 | Close the remaining absent target-map lane or document why it is host-only. |
