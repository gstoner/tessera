...
| `Runtime execution matrix` | closed | 130 | 130 | 0 | apple_cpu=2, apple_gpu=2, cpu=2, nvidia_sm120=1, rocm=64, x86=59 | Add rows only when a launch path actually executes. |
...
| `ROCm / HIP` | closed | 100 | 100 | 0 | runtime: rocm=64; target_map: compiled=34, hardware_verified=2 | Close the artifact-only target-map tail and preserve CDNA as hardware-gated. |
