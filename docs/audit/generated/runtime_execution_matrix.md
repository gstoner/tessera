...
| `rocm` | `rocm_compiled` | `rocm_compiled` | `native_gpu` | `hip_runtime` | ROCm matmul artifact runs the COMPILER-GENERATED RDNA WMMA GEMM (Stage L): tessera-opt generates + serializes the kernel to hsaco in-process (no mlir-opt), then HIP loads + launches it. Opt-in; the hand-written rocm_wmma lane stays the default + oracle. |
