...
**182/479 primitives are accelerator-proven on Apple GPU today.** Of the 322 accelerator-relevant primitives (proven + eligible + special), **128 are *eligible*** — FLOP-bearing numeric ops that a Metal kernel would accelerate but which aren't routed through the envelope yet. That is the actionable accelerator-proof gap; the rest are host-only or hardware-blocked by design.
...
| `host` | 147 | structural / orchestration / shape — accelerator not-applicable |

## By category

| Category | n | proven | eligible | class |
|----------|--:|-------:|---------:|-------|
| `acceptance_verification` | 3 | 0 | 0 | `host` |
| `aot` | 6 | 0 | 0 | `host` |
