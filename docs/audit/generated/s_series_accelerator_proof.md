...
**182/480 primitives are accelerator-proven on Apple GPU today.** Of the 322 accelerator-relevant primitives (proven + eligible + special), **128 are *eligible*** — FLOP-bearing numeric ops that a Metal kernel would accelerate but which aren't routed through the envelope yet. That is the actionable accelerator-proof gap; the rest are host-only or hardware-blocked by design.
...
| `eligible` | 128 | numeric — route-able to a Metal kernel (the actionable gap) |
...
| `host` | 148 | structural / orchestration / shape — accelerator not-applicable |
...
## Eligible worklist (128) — the accelerator-proof gap
