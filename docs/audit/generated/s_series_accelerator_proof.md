...
**175/457 primitives are accelerator-proven on Apple GPU today.** Of the 311 accelerator-relevant primitives (proven + eligible + special), **124 are *eligible*** — FLOP-bearing numeric ops that a Metal kernel would accelerate but which aren't routed through the envelope yet. That is the actionable accelerator-proof gap; the rest are host-only or hardware-blocked by design.
...
| `eligible` | 124 | numeric — route-able to a Metal kernel (the actionable gap) |
...
## Eligible worklist (124) — the accelerator-proof gap
