...
| `tests/tessera_numerical_validation` | scaffold | `tests/tessera_numerical_validation/run_all.sh` | Numerical validation harness (reference-vs-runtime comparisons for device_verified_jit CPU + future hardware backends).  Today the directory contains ``README.md`` + ``requirements.txt`` + ``run_all.sh`` + a ``tessera_numerics/`` Python package, but **no test_*.py files** — pytest doesn't pick up any tests here.  Modernization onto current APIs (``ts.jit``, ``fn.explain()``, ``execution_kind``, fallback_reason) is deferred until a workload genuinely needs it. |
