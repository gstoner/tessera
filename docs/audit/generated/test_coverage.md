...
- **1969** total Python-test references, **500** total lit-fixture references.
- **104** ops have **zero** references in either test surface.
- **208** ops have ≤1 reference ("thinly tested").
...
These **208** ops have at most one test reference across the whole test surface.  Many will be legitimate — variant aliases, structural ops, or category rollups that inherit coverage from a parent family — but each one is a candidate for explicit per-op test coverage.
...
_(148 additional thinly-tested ops omitted; see `collect_op_test_coverage()` for the full list.)_
...
**208** ops have ≤1 direct test reference.  They break down as:

| Bucket | Count | Meaning |
|--------|------:|---------|
| `covered_by_family`      |   84 | Tested via a parent op or family wrapper |
...
_(54 additional family-covered ops omitted; see `classify_thinly_tested()` for the full list.)_
