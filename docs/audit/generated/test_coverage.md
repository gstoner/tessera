...
- **3445** total Python-test references, **866** total lit-fixture references.
- **99** ops have **zero** references in either test surface.
- **146** ops have ≤1 reference ("thinly tested").
...
These **146** ops have at most one test reference across the whole test surface.  Many will be legitimate — variant aliases, structural ops, or category rollups that inherit coverage from a parent family — but each one is a candidate for explicit per-op test coverage.
...
_(86 additional thinly-tested ops omitted; see `collect_op_test_coverage()` for the full list.)_
...
**146** ops have ≤1 direct test reference.  They break down as:
