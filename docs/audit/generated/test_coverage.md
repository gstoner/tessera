...
- **3468** total Python-test references, **866** total lit-fixture references.
- **98** ops have **zero** references in either test surface.
- **144** ops have ≤1 reference ("thinly tested").
- **85** ops have ≥10 references ("well tested").
...
| `flash_attn` |  104 |   57 |  161 |   8 | `bf16`, `f32`, `fp16`, `fp32` … |
...
These **144** ops have at most one test reference across the whole test surface.  Many will be legitimate — variant aliases, structural ops, or category rollups that inherit coverage from a parent family — but each one is a candidate for explicit per-op test coverage.
...
_(84 additional thinly-tested ops omitted; see `collect_op_test_coverage()` for the full list.)_
...
**144** ops have ≤1 direct test reference.  They break down as:

| Bucket | Count | Meaning |
|--------|------:|---------|
| `covered_by_family`      |   45 | Tested via a parent op or family wrapper |
| `structural_only`        |   93 | Registry/metadata/wrapper; no direct numerical test meaningful |
...
_(63 additional structural ops omitted.)_
