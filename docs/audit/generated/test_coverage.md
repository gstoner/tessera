...
- **2897** total Python-test references, **825** total lit-fixture references.
- **107** ops have **zero** references in either test surface.
- **192** ops have ≤1 reference ("thinly tested").
- **62** ops have ≥10 references ("well tested").
- **88** ops have at least one associated `pytest.raises` negative test.
...
| `softmax` |   99 |   37 |  136 |  11 | `bf16`, `f16`, `f32`, `fp16` … |
...
| `silu` |   84 |    2 |   86 |   5 | `bf16`, `f16`, `f32`, `f64` … |
| `rmsnorm` |   72 |   10 |   82 |   2 | `bf16`, `f16`, `f32`, `f64` … |
| `gemm` |   78 |    2 |   80 |   7 | `bf16`, `f16`, `f32`, `fp16` … |
| `gelu` |   45 |   19 |   64 |   1 | `bf16`, `f16`, `f32`, `f64` … |
...
These **192** ops have at most one test reference across the whole test surface.  Many will be legitimate — variant aliases, structural ops, or category rollups that inherit coverage from a parent family — but each one is a candidate for explicit per-op test coverage.
...
_(132 additional thinly-tested ops omitted; see `collect_op_test_coverage()` for the full list.)_
...
**192** ops have ≤1 direct test reference.  They break down as:

| Bucket | Count | Meaning |
|--------|------:|---------|
| `covered_by_family`      |   72 | Tested via a parent op or family wrapper |
...
_(42 additional family-covered ops omitted; see `classify_thinly_tested()` for the full list.)_
