...
| Measure | Live value |
|---|---:|
| Tracked primitives | 504 |
| Existing public/runtime primitives | 471 |
| Partial registry rows | 479 |
| Planned registry rows | 25 |
| Rows with at least one open contract axis | 405 |
| Graph IR registered | 449 |
...
|---|---:|---:|---:|---:|
| `math_semantics` | 478 | 26 | 0 | 0 |
| `shape_rule` | 472 | 26 | 6 | 0 |
| `dtype_layout_rule` | 478 | 26 | 0 | 0 |
| `vjp` | 302 | 166 | 1 | 35 |
| `jvp` | 293 | 168 | 0 | 43 |
| `batching_rule` | 448 | 56 | 0 | 0 |
| `transpose_rule` | 351 | 153 | 0 | 0 |
| `sharding_rule` | 398 | 56 | 50 | 0 |
| `masking_effect_rule` | 74 | 430 | 0 | 0 |
| `lowering_rule` | 452 | 52 | 0 | 0 |
| `backend_kernel` | 0 | 99 | 396 | 9 |
| `tests` | 504 | 0 | 0 | 0 |
...
The compiler-facing table contains **338** callable/IR rows; the remaining **166** registry entries are host APIs, runtime-only surfaces, or planned domain primitives. The aggregate table chooses the strongest evidence across targets and must not be read as support on every architecture.

| Compiler layer | Live status counts |
|---|---|
| `api` | `public` 338 |
| `frontend` | `public` 338 |
| `graph_ir` | `registered` 306, `not_applicable` 32 |
| `schedule_ir` | `not_applicable` 2, `complete` 336 |
| `tile_ir` | `not_applicable` 26, `complete` 4, `partial` 10, `fused` 291, `no_kernel_required` 7 |
| `target_ir` | `not_applicable` 26, `fused` 72, `device_verified_abi` 13, `device_verified_jit` 205, `reference` 14, `no_kernel_required` 8 |
| `runtime` | `fused` 6, `ready` 332 |
| `bench` | `benchmarked` 99, `none` 239 |

Exact-target backend-manifest rows:

| Target | Total | Status counts |
|---|---:|---|
| `apple_cpu` | 379 | `fused` 4, `reference` 375 |
| `apple_gpu` | 199 | `device_verified_abi` 10, `device_verified_jit` 139, `fused` 46, `artifact_only` 2, `reference` 2 |
| `cpu` | 356 | `reference` 356 |
...
| Queue | Count | Scope |
|---|---:|---|
| Tile IR partial | 10 | `depth_attn`, `game_boltzmann_value`, `game_coalition_excess`, `game_coalition_marginal`, `game_mex`, `game_semivalue`, `game_subset_mobius`, `game_subset_zeta`, `game_superset_mobius`, `game_superset_zeta` |
...
| VJP / JVP planned | 35 / 43 | Autodiff rules, not forward compiler execution. |
