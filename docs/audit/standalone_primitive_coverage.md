...
| Measure | Live value |
|---|---:|
| Tracked primitives | 488 |
| Existing public/runtime primitives | 455 |
| Partial registry rows | 463 |
| Planned registry rows | 25 |
| Rows with at least one open contract axis | 389 |
| Graph IR registered | 433 |
...
|---|---:|---:|---:|---:|
| `math_semantics` | 462 | 26 | 0 | 0 |
| `shape_rule` | 456 | 26 | 6 | 0 |
| `dtype_layout_rule` | 462 | 26 | 0 | 0 |
| `vjp` | 293 | 164 | 0 | 31 |
| `jvp` | 284 | 166 | 0 | 38 |
| `batching_rule` | 432 | 56 | 0 | 0 |
| `transpose_rule` | 337 | 151 | 0 | 0 |
| `sharding_rule` | 389 | 56 | 43 | 0 |
| `masking_effect_rule` | 69 | 419 | 0 | 0 |
| `lowering_rule` | 436 | 52 | 0 | 0 |
| `backend_kernel` | 0 | 99 | 380 | 9 |
| `tests` | 488 | 0 | 0 | 0 |
...
The compiler-facing table contains **325** callable/IR rows; the remaining **163** registry entries are host APIs, runtime-only surfaces, or planned domain primitives. The aggregate table chooses the strongest evidence across targets and must not be read as support on every architecture.

| Compiler layer | Live status counts |
|---|---|
| `api` | `public` 325 |
| `frontend` | `public` 325 |
| `graph_ir` | `registered` 293, `not_applicable` 32 |
| `schedule_ir` | `not_applicable` 2, `complete` 323 |
| `tile_ir` | `not_applicable` 26, `complete` 4, `fused` 288, `no_kernel_required` 7 |
| `target_ir` | `not_applicable` 26, `fused` 72, `device_verified_abi` 13, `device_verified_jit` 202, `reference` 4, `no_kernel_required` 8 |
| `runtime` | `fused` 6, `ready` 319 |
| `bench` | `benchmarked` 99, `none` 226 |

Exact-target backend-manifest rows:

| Target | Total | Status counts |
|---|---:|---|
| `apple_cpu` | 363 | `fused` 4, `reference` 359 |
| `apple_gpu` | 199 | `device_verified_abi` 10, `device_verified_jit` 139, `fused` 46, `artifact_only` 2, `reference` 2 |
| `cpu` | 340 | `reference` 340 |
