...
| Measure | Live value |
|---|---:|
| Tracked primitives | 489 |
| Existing public/runtime primitives | 456 |
| Partial registry rows | 464 |
| Planned registry rows | 25 |
| Rows with at least one open contract axis | 390 |
| Graph IR registered | 434 |
...
|---|---:|---:|---:|---:|
| `math_semantics` | 463 | 26 | 0 | 0 |
| `shape_rule` | 457 | 26 | 6 | 0 |
| `dtype_layout_rule` | 463 | 26 | 0 | 0 |
| `vjp` | 293 | 165 | 0 | 31 |
| `jvp` | 284 | 167 | 0 | 38 |
| `batching_rule` | 433 | 56 | 0 | 0 |
| `transpose_rule` | 337 | 152 | 0 | 0 |
| `sharding_rule` | 390 | 56 | 43 | 0 |
| `masking_effect_rule` | 70 | 419 | 0 | 0 |
| `lowering_rule` | 437 | 52 | 0 | 0 |
| `backend_kernel` | 0 | 99 | 381 | 9 |
| `tests` | 489 | 0 | 0 | 0 |
...
The compiler-facing table contains **326** callable/IR rows; the remaining **163** registry entries are host APIs, runtime-only surfaces, or planned domain primitives. The aggregate table chooses the strongest evidence across targets and must not be read as support on every architecture.

| Compiler layer | Live status counts |
|---|---|
| `api` | `public` 326 |
| `frontend` | `public` 326 |
| `graph_ir` | `registered` 294, `not_applicable` 32 |
| `schedule_ir` | `not_applicable` 2, `complete` 324 |
| `tile_ir` | `not_applicable` 26, `complete` 4, `fused` 289, `no_kernel_required` 7 |
| `target_ir` | `not_applicable` 26, `fused` 72, `device_verified_abi` 13, `device_verified_jit` 203, `reference` 4, `no_kernel_required` 8 |
| `runtime` | `fused` 6, `ready` 320 |
| `bench` | `benchmarked` 99, `none` 227 |

Exact-target backend-manifest rows:

| Target | Total | Status counts |
|---|---:|---|
| `apple_cpu` | 364 | `fused` 4, `reference` 360 |
| `apple_gpu` | 199 | `device_verified_abi` 10, `device_verified_jit` 139, `fused` 46, `artifact_only` 2, `reference` 2 |
| `cpu` | 341 | `reference` 341 |
...
| `rocm` | 317 | `device_verified_abi` 3, `device_verified_jit` 314 |
| `x86` | 309 | `device_verified_jit` 267, `fused` 42 |
...
| Native/fused aggregate rows without benchmark inventory | 194 | See generated support table; this is an evidence queue, not proof of a missing kernel. |
