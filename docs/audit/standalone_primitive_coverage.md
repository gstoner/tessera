...
| Measure | Live value |
|---|---:|
| Tracked primitives | 487 |
| Existing public/runtime primitives | 454 |
| Partial registry rows | 462 |
| Planned registry rows | 25 |
| Rows with at least one open contract axis | 388 |
| Graph IR registered | 432 |
...
|---|---:|---:|---:|---:|
| `math_semantics` | 461 | 26 | 0 | 0 |
| `shape_rule` | 455 | 26 | 6 | 0 |
| `dtype_layout_rule` | 461 | 26 | 0 | 0 |
| `vjp` | 292 | 164 | 0 | 31 |
| `jvp` | 284 | 166 | 0 | 37 |
| `batching_rule` | 431 | 56 | 0 | 0 |
| `transpose_rule` | 336 | 151 | 0 | 0 |
| `sharding_rule` | 388 | 56 | 43 | 0 |
| `masking_effect_rule` | 69 | 418 | 0 | 0 |
| `lowering_rule` | 435 | 52 | 0 | 0 |
| `backend_kernel` | 0 | 99 | 379 | 9 |
| `tests` | 487 | 0 | 0 | 0 |
...
The compiler-facing table contains **324** callable/IR rows; the remaining **163** registry entries are host APIs, runtime-only surfaces, or planned domain primitives. The aggregate table chooses the strongest evidence across targets and must not be read as support on every architecture.

| Compiler layer | Live status counts |
|---|---|
| `api` | `public` 324 |
| `frontend` | `public` 324 |
| `graph_ir` | `registered` 292, `not_applicable` 32 |
| `schedule_ir` | `not_applicable` 2, `complete` 322 |
| `tile_ir` | `not_applicable` 25, `partial` 4, `fused` 288, `no_kernel_required` 7 |
| `target_ir` | `not_applicable` 25, `fused` 72, `device_verified_abi` 13, `device_verified_jit` 202, `reference` 4, `no_kernel_required` 8 |
| `runtime` | `fused` 6, `ready` 318 |
| `bench` | `benchmarked` 99, `none` 225 |

Exact-target backend-manifest rows:

| Target | Total | Status counts |
|---|---:|---|
| `apple_cpu` | 362 | `fused` 4, `reference` 358 |
| `apple_gpu` | 199 | `device_verified_abi` 10, `device_verified_jit` 139, `fused` 46, `artifact_only` 2, `reference` 2 |
| `cpu` | 339 | `reference` 339 |
...
| `rocm` | 316 | `device_verified_abi` 3, `device_verified_jit` 313 |
| `x86` | 308 | `device_verified_jit` 266, `fused` 42 |

Current open queues derived from the same sources:

| Queue | Count | Scope |
|---|---:|---|
| Tile IR partial | 4 | `all_gather`, `all_reduce`, `all_to_all`, `reduce_scatter` |
...
| Native/fused aggregate rows without benchmark inventory | 193 | See generated support table; this is an evidence queue, not proof of a missing kernel. |
| VJP / JVP planned | 31 / 37 | Autodiff rules, not forward compiler execution. |
