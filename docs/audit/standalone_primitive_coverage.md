...
| Measure | Live value |
|---|---:|
| Tracked primitives | 495 |
| Existing public/runtime primitives | 462 |
| Partial registry rows | 470 |
| Planned registry rows | 25 |
| Rows with at least one open contract axis | 396 |
| Graph IR registered | 440 |
...
|---|---:|---:|---:|---:|
| `math_semantics` | 469 | 26 | 0 | 0 |
| `shape_rule` | 463 | 26 | 6 | 0 |
| `dtype_layout_rule` | 469 | 26 | 0 | 0 |
| `vjp` | 294 | 166 | 1 | 34 |
| `jvp` | 286 | 168 | 0 | 41 |
| `batching_rule` | 439 | 56 | 0 | 0 |
| `transpose_rule` | 342 | 153 | 0 | 0 |
| `sharding_rule` | 394 | 56 | 45 | 0 |
| `masking_effect_rule` | 74 | 421 | 0 | 0 |
| `lowering_rule` | 443 | 52 | 0 | 0 |
| `backend_kernel` | 0 | 99 | 387 | 9 |
| `tests` | 495 | 0 | 0 | 0 |
...
The compiler-facing table contains **329** callable/IR rows; the remaining **166** registry entries are host APIs, runtime-only surfaces, or planned domain primitives. The aggregate table chooses the strongest evidence across targets and must not be read as support on every architecture.

| Compiler layer | Live status counts |
|---|---|
| `api` | `public` 329 |
| `frontend` | `public` 329 |
| `graph_ir` | `registered` 297, `not_applicable` 32 |
| `schedule_ir` | `not_applicable` 2, `complete` 327 |
| `tile_ir` | `not_applicable` 26, `complete` 4, `partial` 1, `fused` 291, `no_kernel_required` 7 |
| `target_ir` | `not_applicable` 26, `fused` 72, `device_verified_abi` 13, `device_verified_jit` 205, `reference` 5, `no_kernel_required` 8 |
| `runtime` | `fused` 6, `ready` 323 |
| `bench` | `benchmarked` 99, `none` 230 |

Exact-target backend-manifest rows:

| Target | Total | Status counts |
|---|---:|---|
| `apple_cpu` | 370 | `fused` 4, `reference` 366 |
| `apple_gpu` | 199 | `device_verified_abi` 10, `device_verified_jit` 139, `fused` 46, `artifact_only` 2, `reference` 2 |
| `cpu` | 347 | `reference` 347 |
...
| VJP / JVP planned | 34 / 41 | Autodiff rules, not forward compiler execution. |
| Sharding partial | 45 | Multi-device/domain proof. |
