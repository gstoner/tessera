...
|---|---|---|
| `gfx1201` | Radeon RX 9070 series; Radeon AI PRO R9700/R9700S/R9600D | Exact-device RX 9070 XT proof exists for every registered content-addressed family plugin and for the bounded scheduled runtime ABIs. The public per-target capability/execution dashboards have not yet absorbed those packages, so their unjoined rows remain `artifact_only`. |
...
- [Runtime execution matrix](../../audit/generated/runtime_execution_matrix.md)
  is current public operation/executor registry truth. Bounded scheduled
  packages are tracked separately until that registry join is complete.
- [ROCm target map](../../audit/generated/rocm_target_map.md) is the generated
  per-op target view and explicitly calls out that same projection gap.
