...
and Target IR entry points. The production compiler path should replace the
static stubs with verified MLIR generated by the Graph→Schedule→Tile→Target
pipeline while preserving the same command contract.
