...
> with verdict "all <N> gates passed", on at least one Darwin host
...
|---|---|---|
| 1 | `generated_docs_drift` | Any generated audit dashboard is stale vs the registry-managed renderer, including `support_table`, `e2e_op_coverage`, `surface_status`, and target maps. |
| 2 | `tests_manifest_smoke` | A runnable or compile-only row in `tests_manifest.py` no longer smoke-passes. |
| 3 | `canonicals_native_dispatch` | `matmul_softmax_matmul` canonical doesn't actually dispatch its fused MSL kernel on Darwin (regression in driver, runtime shim, or canonical wiring). |
| 4 | `apple_cpu_execution_kind_bench` | matmul-vs-numpy ratio gate breaks (Tessera left the Accelerate fast lane). |
| 5 | `spectral_correctness_proof` | Stockham FFT correctness sentinel disagrees with naive DFT beyond 1e-3 tolerance. |
| 6 | `apple_gpu_hardware_marked_tests` | Any `@pytest.mark.hardware_apple_gpu` test fails on the Darwin runner. |
...
# Expected last line: "[release_gate:apple_gpu] all <N> gates passed"
```
...
| Change | Date | Effect |
|---|---|---|
| Policy created | 2026-05-20 | Apple follow-up #4 — `--target=apple_gpu` becomes the canonical Apple release blocker.  Gate matrix: structure + Apple-specific gates. |
