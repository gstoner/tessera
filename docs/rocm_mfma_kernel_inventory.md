...
- ✅ `mfma_table.inc` C++ refresh to match `_MFMA_VARIANTS` — generated by
  `scripts/generate_mfma_table.py` (`--check` gates drift, guarded by
  `test_phase_ghi_lane2.py`); currently in sync.
- 🔜 `hipcc -S --offload-arch=gfx942` compile-only validation (CDNA)
- 🔜 RCCL 2.22 bindings compile + symbol resolution
- ✅ `tessera-to-linalg` registered so the MLIR `--tessera-emit-rocdl` route works
  (ROCM_AUDIT #8, 2026-06-23): a HIP build links `TesseraIR`/`TesseraPasses`, and
  `--tessera-emit-rocdl` lowers a tessera kernel through
  `tessera-to-linalg → scf.parallel → gpu → ROCDL` to `rocdl.kernel`. The direct
  `rocdl_emit.py` LLVM-IR emitter remains the Stage B production path; this is the
  parallel MLIR-graph route.
