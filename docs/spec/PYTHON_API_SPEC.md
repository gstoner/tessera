...
| `ebm_energy_quadratic(x, y)` / `ebm_langevin_step(y, energy_fn, eta, temperature, rng_key, *, grad_fn=None)` | `(array, array) → array` / `(array, fn, float, float, key) → (array, key)` | `pure` / `random` | Energy-based-model primitives (Graph IR ops `tessera.ebm.energy_quadratic` / `tessera.ebm.langevin_step`). Quadratic reconstruction energy `E_b = ½·‖x_b − y_b‖²` and one Langevin step `y' = y − η·∂E/∂y + √(2ηT)·ξ`. fp32 numpy reference today via `tessera.ebm.*`; the Apple GPU EBM family ships fused MSL kernels (see `docs/status/ga_ebm_milestone.md`). |
