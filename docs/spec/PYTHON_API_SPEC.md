...
| `es_low_rank_correction(x, member_ids, key, *, out_dim, rank, sigma, epoch=0, score="gaussian", antithetic=True, numeric_policy=None)` | `(array, array, key) → array` | `pure` | Content-addressed EGGROLL rank-1 correction. With member IDs and key fixed it is linear in `x`, has a compiler JVP and a proven mathematical transpose; member IDs and key are non-differentiable identities, and native reverse lowering remains open. The member-RNG-v1 identity is SplitMix64→Philox4x32→Box–Muller; fp32 packages execute on gfx1151 and Zen 5 AVX-512. Apple/NVIDIA, rank>1, and integer/VNNI numeric-policy lanes remain fail-closed. |
...
| `dequant_matmul(x, packed_w, *, backend="reference")` | `(array, PackedQuantTensor) → array` | `pure` | Fused dequantize-into-GEMM (model-class roadmap M1): `y = x @ dequant(packed_w)` with an fp32 accumulator, evaluated group-wise over packed INT4/INT8/FP8 codes + a separate per-group scale operand; Graph IR op `tessera.dequant_matmul`. `backend="apple_gpu"` runs the per-group matmuls on the Metal lane (composed; fused dequant MSL kernel pending) |
| `dequant_grouped_gemm(x, packed_experts, group_sizes, *, backend="reference")` | `(array, list[PackedQuantTensor], array) → array` | `pure` | Quantized M-grouped grouped GEMM (M2 MoE expert-FFN core): token block `e` is matmul'd by expert `e`'s packed weight via `dequant_matmul`; Graph IR op `tessera.dequant_grouped_gemm` |
| `quantized_matmul(x, w_packed, scales, biases, *, group_size=64)` | `(array, array, array, array) → array` | `pure` | P3 packed-weight int4 quantized matmul: `y = x @ dequant(w[N,K])^T` where `w` is packed 4-bit codes (uint8, 2 nibbles/byte = 0.5 B/weight, ~8× less weight traffic than `dequant_matmul`'s f32 codes) + per-group affine `scale·code + bias`; Graph IR op `tessera.quantized_matmul`. Apple GPU lane `tessera_apple_gpu_quantized_matmul_i4_{f32,f16,tiled_f32}` (`runtime.apple_gpu_quantized_matmul_i4`); **full `@jit(target="apple_gpu")` artifact dispatch wired** (envelope `quant_matmul` lane + C++ `isAppleGpuRuntimeOp` mirror → `metal_runtime`) |
| `nn.functional.moe_layer(x, W_router, W_gate, W_up, W_down, *, top_k=1, normalize_weights=True, capacity_factor=0.0, kind="contiguous", alignment=None, quant=None)` | `(array, array, array, array, array) → array` | `pure` | Single-device top-k SwiGLU MoE layer: `route_tokens` (top-k gating) → gather tokens into expert order → `moe_swiglu_block` → scatter-add weighted combine. Heavy expert FFN on GPU grouped-GEMM lanes; routing/permute/combine host-side (data-dependent). Last local rung before distributed MegaMoE (no token all-to-all / comm overlap) |
...
| `clifford_hodge_star(a)` | `(array) → array` | `pure` | GA Cl(3,0) Hodge star `⋆ω = reverse(ω)·I` (pointwise, linear); apple_gpu routes to the cl30 kernel; VJP/JVP via the constant transpose; Graph IR op `tessera.clifford_hodge_star` |
| `clifford_ext_deriv(field, spacing=None)` | `(array) → array` | `pure` | GA Cl(3,0) exterior derivative `dω = Σ_i e_i ∧ ∂_i ω` on a sampled field `(*spatial, 8)`; linear finite-difference stencil; VJP = exact discrete adjoint; Graph IR op `tessera.clifford_ext_deriv` |
| `clifford_vec_deriv(field, spacing=None)` | `(array) → array` | `pure` | GA Cl(3,0) geometric gradient `∂F = Σ_i e_i · ∂_i F` on a sampled field; linear stencil; VJP = exact discrete adjoint; Graph IR op `tessera.clifford_vec_deriv` |
| `clifford_codiff(field, spacing=None)` | `(array) → array` | `pure` | GA Cl(3,0) codifferential `δω = (-1)^(n(k+1)+1) ⋆d⋆ω` on a sampled field — the grade-dependent sign is part of the contract (corrected 2026-09-02): without it the operator returns `+div` on a vector field where δ is `-div`, and is not adjoint to `clifford_ext_deriv`. Non-degenerate Euclidean signatures only; `q > 0` and `r > 0` fail closed. Linear; VJP = `hodge*∘ext*∘hodge*` composed with the SAME per-grade sign table (δ = S∘⋆d⋆ with S diagonal and self-adjoint, so δ* = (⋆d⋆)*∘S); Graph IR op `tessera.clifford_codiff` |
...
| `adagrad(params, grads, state=None, lr=1e-2, eps=1e-8, ...)` | `(tree,tree,dict?) → tuple` | `pure` | MSW-3 Adagrad, per `def:determ_adagrad` eq. (1); undecayed running sum of squared gradients, `eps` OUTSIDE the root; Graph IR op `tessera.adagrad` |
| `rmsprop(params, grads, state=None, lr=1e-3, beta=0.9, eps=1e-8, bias_adjusted=False, ...)` | `(tree,tree,dict?) → tuple` | `pure` | MSW-3 RMSprop, per `def:determ_RMSprop` eq. (1)-(2); `bias_adjusted=True` selects `def:determ_RMSprop_bias`, dividing the second moment by `1 - beta**n` before the root; Graph IR op `tessera.rmsprop` |
| `adadelta(params, grads, state=None, beta=0.9, delta=0.9, eps=1e-6, lr=1.0, ...)` | `(tree,tree,dict?) → tuple` | `pure` | MSW-3 Adadelta, per `def:determ_adadelta` eq. (1)-(4); learning-rate free by construction — `lr` is a Tessera extension whose default 1.0 reproduces the definition; Graph IR op `tessera.adadelta` |
| `shampoo(params, grads, state=None, lr=1e-3, eps=1e-4, ...)` | `(tree,tree,dict?) → tuple` | `pure` | MSW-3 Shampoo, per `def:determ_Shampoo` eq. (1)-(4); two-sided full-matrix preconditioning `L^(-1/4) G R^(-1/4)`, rank-1 treated as `d x 1`, rank 0 and rank ≥ 3 refused; state is not param-shaped so `op_catalog` deliberately declares no shape rule; Graph IR op `tessera.shampoo` |
| `midpoint_sgd(params, grad_fn, state=None, lr, ...)` | `(tree,callable,dict?) → tuple` | `pure` | MSW-3 explicit midpoint SGD, per `def:midpointSGD` eq. (1); takes a gradient FUNCTION rather than a gradient, because the method re-evaluates the gradient at a probe point that does not exist until the first is known; stateless; Graph IR op `tessera.midpoint_sgd` |
...
| `score_matching_loss(score, target_score, reduction="mean")` | `(array,array) → scalar/array` | `pure` | S11 score-matching criterion (½·MSE); apple_gpu routes reduction="mean" to the EBM-loss MPSGraph kernel; Graph IR op `tessera.loss.score_matching` |
| `contrastive_divergence_loss(energy_pos, energy_neg, reduction="mean")` | `(array,array) → scalar/array` | `pure` | EBM Contrastive-Divergence loss `mean(E⁺ − E⁻)`; apple_gpu reduction="mean" → EBM-loss kernel; Graph IR op `tessera.loss.contrastive_divergence` |
| `persistent_cd_loss(energy_pos, energy_persistent_neg, reduction="mean")` | `(array,array) → scalar/array` | `pure` | Persistent Contrastive Divergence (same energy-difference form as CD; persistent negative chain maintained by the caller); Graph IR op `tessera.loss.persistent_cd` |
| `implicit_score_matching_loss(score, divergence_score, reduction="mean")` | `(array,array) → scalar/array` | `pure` | Hyvärinen implicit score matching `mean(½‖s‖² + ∇·s)`; apple_gpu reduction="mean" → EBM-loss kernel; Graph IR op `tessera.loss.implicit_score_matching` |
| `denoising_score_matching_loss(score_noisy, y_clean, y_noisy, sigma, reduction="mean")` | `(array,array,array) → scalar/array` | `pure` | Vincent denoising score matching `mean(½‖s + (ỹ−y)/σ²‖²)`; apple_gpu reduction="mean" → EBM-loss kernel; Graph IR op `tessera.loss.denoising_score_matching` |
| `vlb_loss(terms, reduction="mean")` | `(array) → scalar/array` | `pure` | S11 diffusion VLB reducer; Graph IR op `tessera.loss.vlb` |
| `kl_divergence(p_log_probs, q_probs, reduction="mean")` | `(array,array) → scalar/array` | `pure` | S11 KL divergence; Graph IR op `tessera.loss.kl_divergence`; apple_gpu composes it from the exp/log/sub/mul/sum opcode lanes |
| `js_divergence(p_probs, q_probs, reduction="mean")` | `(array,array) → scalar/array` | `pure` | S11 Jensen-Shannon divergence; Graph IR op `tessera.loss.js_divergence`; apple_gpu composes it from the opcode lanes |
...
| `depth_attn(query, sources, eps=1e-6)` | `(array[d], array[m,...,d]) → array[...,d]` | `pure` | Block AttnRes depth-axis attention. The tracer materializes required epsilon and `{storage, softmax, accum}` policy attributes; typed compiler VJP/JVP products are available. Physical backends remain artifact-only pending architecture-owned packages and exact-device proof. |
| `varlen_sdpa(Q, K, V, *, cu_seqlens_q, cu_seqlens_k, causal=False, scale=None)` | `(array,array,array,array,array) → array` | `pure` | Variable-length (packed-sequence) SDPA — the Cosmos-3 "two-way flat attention" contract (`cu_seqlens_q`/`cu_seqlens_k` as first-class operands, independent → rectangular blocks). Graph IR op `tessera.varlen_sdpa` (rank-3 packed streams + rank-1 integer cu_seqlens; verifier pins head axis / head_dim / total_k / per-SM head_dim ceiling). Reference + Apple GPU `metal_runtime` lane in `tessera.nn.varlen` decompose to per-block `flash_attn`; fused FA-3/NATTEN varlen kernels are Phase G/H. |
...
| `power_attn(Q, K, V, *, state, window=None, deg=2, causal=True)` | `(array,array,array) → array` | `state` | LA-4 — Symmetric power attention (linear-cost). Promoted from the retired `archive/examples/advanced/power_retention/` scaffold |
...
| `memory_index_select_ste(indexer_keys, query, *, threshold=0.5, scale=None)` | `(array,array) → array` | `pure` | LSA straight-through selector — hard 0/1 mask forward, sigmoid gradient backward (trains the indexer under hard selection) |
| `lookahead_sparse_attention(Q, K, V, *, window_size, block_size, tau=64, threshold=0.5, causal=True, ...)` | `(array,array,array) → array` | `state` | LSA composite policy — attention over the causal local window ∪ threshold-selected historical blocks (experimental, inference-only) |
...
| `rng_philox_uniform(key, counter, *, shape, dtype="fp32", lo=0.0, hi=1.0)` | `(tensor<2xi64>, tensor<1xi64>) → array` | `pure` | Explicit counter-based Philox Graph op. The key/counter operands make replay identity part of SSA; x86/AVX-512 and gfx1151 execute native distribution transforms. Apple/NVIDIA remain architecture-owned follow-ups. |
| `rng_philox_normal(key, counter, *, shape, dtype="fp32", mean=0.0, std=1.0)` | `(tensor<2xi64>, tensor<1xi64>) → array` | `pure` | Explicit counter-based Philox Graph op with Box–Muller transform. It is constant noise under AD unless an operation explicitly declares a pathwise or score-function estimator; x86/AVX-512 and gfx1151 have native consumers. |
| `fused_epilogue(x, bias=None, activation="linear")` | `(array) → array` | `pure` | Applies bias + activation |
| `game_subset_zeta(v)`, `game_subset_mobius(v)`, `game_superset_zeta(v)`, `game_superset_mobius(v)` | `([..., 2^n]) → [..., 2^n]` | `pure` | Coalition-lattice zeta/Möbius butterflies (GAME_THEORY_PLAN G1; `python/tessera/game/`). Linear, declared transposes (subset ↔ superset), fp64 results by the §6 numerics mandate; trailing axis must be `2^n` (fail closed). |
| `game_coalition_marginal(v)` | `([..., 2^n]) → [..., n, 2^n]` | `pure` | Per-player bit-flip marginals `∂_i v(S) = v(S) − v(S Δ {i})`; linear, self-adjoint per player. |
| `game_semivalue(vhat, weights)` | `([..., 2^n], [n+1]) → [..., n]` | `pure` | Weight-parameterized value in the Möbius basis: `Φ_i = Σ_{T∋i} w(\|T\|)·v̂(T)`. Shapley = `w(t)=1/t`, Banzhaf = `w(t)=2^(1−t)` — one contraction, every probabilistic value a weight vector. |
...
| `spec_accept(draft, target)` | `(array[P×D], array[P×(D+1)]) → array[3]` | `pure` | SD1 speculative-decode acceptance (Graph IR op `tessera.spec_accept`). Greedy (argmax-match) longest-prefix path over a draft tree; returns `[accepted_path_idx, accepted_prefix_length, bonus_token]` where the bonus is `target[path, length]`. Mirrors the `tessera.speculative` greedy reference (`_ref_spec_accept` / `dflash_linear_verify`). Native ROCm/gfx1151 kernel ships (`--generate-rocm-spec-accept-kernel`); the distribution-preserving Leviathan / rejection-sampling form is `spec_accept_sample` (SD1-2). |
| `spec_accept_sample(draft, target_probs, draft_probs, accept_u, resid_u)` | `(array[D], array[(D+1)×V], array[D×V], array[D], array[1]) → array[D+2]` | `pure` | SD1-2 distribution-preserving (Leviathan) rejection-sampling acceptance (Graph IR op `tessera.spec_accept_sample`). Per position accept iff `accept_u[i]·p_draft ≤ p_target`; on first reject draw a corrected token from the residual `normalize(relu(target−draft))` by CDF inversion of `resid_u`; on full accept draw a bonus from `target_probs`'s extra row. **RNG is explicit** (the uniforms are inputs → deterministic, device-bit-exact); the categorical draw is CDF inversion, not numpy `rng.choice`. Returns `[accepted, t0, …, t_D]` (`accepted+1` valid tokens). Native ROCm/gfx1151 kernel ships (`--generate-rocm-spec-accept-sample-kernel`). Mirrors `dflash.py:dflash_speculative_verify`. |
| `target_verify(tokens, logits)` | `(array[S], array[S×V]) → array[S×V]` | `pure` | SD1-4 speculative-decode target-verification I/O contract (Graph IR op `tessera.target_verify`). Given the verified-position context `tokens` (current committed token + `D` draft tokens, `S = D+1` positions) and the (composed, external) target model's raw `logits` at those positions, returns the contract-shaped per-position target **probabilities** `S×V` (a `softmax` over the vocab) — exactly the `target_probs` operand `spec_accept_sample` consumes (its accept ratio and residual read probabilities). Callers wanting log-probs (`spec_accept_tree_sample` / `batch_verify`) take `log` of the result. A **composed model call**, not a fused kernel: the op pins the `(S × V)` batching contract so the target scoring is explicit enough to batch/fuse; a fused target-verify kernel is a later DK-track concern. |
| `cache_commit(cache, accepted_length)` | `(cache, int) → cache` | `state` | SD1-3 speculative-decode commit (Graph IR op `tessera.cache.commit`, `MemoryEffects<[MemWrite]>` → `EffectLattice` `state`). Advances the KV/SSM cache cursor to keep `accepted_length` tokens (the accepted prefix); drops the rejected draft tail. The IR-visible typed-effect form of `speculative.advance_kv` — no hidden Python mutation (CF0). No device kernel (cursor op). |
| `cache_rollback(cache, num_rejected)` | `(cache, int) → cache` | `state` | SD1-3 speculative-decode rollback (Graph IR op `tessera.cache.rollback`, `MemoryEffects<[MemWrite]>` → `state`). Rewinds the cursor over the last `num_rejected` drafts — `SSMStateHandle.rollback` (ring-buffer rewind) or `KVCacheHandle.trim`. CF0's "speculative rollback via cursor/state handles, not implicit Python state" as a typed effect. |
| `spec_accept_tree_sample(target_log_probs, draft_log_probs, accept_u)` | `(array[P×D], array[P×D], array[P×D]) → array[2]` | `pure` | SD1 tree (multi-path) Leviathan rejection acceptance (Graph IR op `tessera.spec_accept_tree_sample`) — the device form of `speculative.batch_verify`. Over `P` draft paths each `D` deep, accept the draft token at `(p,i)` iff `accept_u[p,i] ≤ exp(target_log_probs[p,i] − draft_log_probs[p,i])`; each path's accepted prefix is the leading run of accepts; returns `[accepted_path_idx, accepted_prefix_length]` for the longest-prefix path (first wins ties). **RNG is explicit** (`accept_u`) → deterministic, device-bit-exact. Native ROCm/gfx1151 kernel ships (`--generate-rocm-spec-accept-tree-sample-kernel`). Generalizes the linear `spec_accept_sample` to a tree and the greedy `spec_accept` to the stochastic rule. |
...
| `ebm_energy_quadratic(x, y)` / `ebm_langevin_step(y, energy_fn, eta, temperature, rng_key, *, grad_fn=None)` | `(array, array) → array` / `(array, fn, float, float, key) → (array, key)` | `pure` / `random` | Energy-based-model primitives (Graph IR ops `tessera.ebm.energy_quadratic` / `tessera.ebm.langevin_step`). Quadratic reconstruction energy `E_b = ½·‖x_b − y_b‖²` and one Langevin step `y' = y − η·∂E/∂y + √(2ηT)·ξ`. See the [GA / EBM status card](../status/ga_ebm.md) and generated dashboards for current target execution evidence. |
