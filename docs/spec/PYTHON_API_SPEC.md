...
| `clifford_codiff(field, spacing=None)` | `(array) → array` | `pure` | GA Cl(3,0) codifferential `d*ω = ⋆d⋆ω` on a sampled field; linear; VJP = hodge*∘ext*∘hodge* adjoint composition; Graph IR op `tessera.clifford_codiff` |
