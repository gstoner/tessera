...
| `es_low_rank_correction(x, member_ids, key, *, out_dim, rank, sigma, epoch=0, score="gaussian", antithetic=True, numeric_policy=None)` | `(array, array, key) → array` | `pure`, terminal non-differentiable | Content-addressed EGGROLL rank-1 correction. The member-RNG-v1 identity is SplitMix64→Philox4x32→Box–Muller; fp32 packages execute on gfx1151 and Zen 5 AVX-512. Apple/NVIDIA, rank>1, and integer/VNNI numeric-policy lanes remain fail-closed. |
