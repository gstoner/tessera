...
note: Stage 18 — Apple proved the shape; NVIDIA and ROCm inherit the same value-call contract with backend-specific executor adapters.
---
...
This contract is **proven first on Apple** (CPU via Accelerate/LAPACK and GPU
via narrow value-call executors). Stage 18 promotes that shape to a
backend-neutral compiler contract. NVIDIA and ROCm inherit the same handoff:
...
Only the Target dialect mnemonic, ABI string, framework, and executor adapter
change by backend. The compiler contract does not.
...
### Backend value-call mnemonics (Stage 18)

| Backend | Value kernel call | Package / archive call | Compiler path |
|---------|-------------------|-------------------------|---------------|
| Apple GPU | `tessera_apple.gpu.kernel_call` | `tessera_apple.gpu.package_call` | `apple_value_target_ir` |
| NVIDIA | `tessera_nvidia.kernel_call` | future package/archive op | `nvidia_value_target_ir` |
| ROCm | `tessera_rocm.kernel_call` | future package/archive op | `rocm_value_target_ir` |

Every row uses the same value operands/results and the same required attrs:
`op_kind`, `symbol`, `status`, `abi`, `dtype`, and `framework`. Backend-specific
metadata may be added (`sm`, `gfx`, `num_warps`, descriptor handles, launch
grid, etc.), but those additions must not replace the shared tuple.

Examples:

```mlir
%0 = tessera_apple.gpu.kernel_call %a, %b {
  op_kind = "batched_gemm",
  symbol = "tessera_apple_gpu_bmm_f32",
  status = "executable",
  abi = "mpsgraph_bmm_f32",
  dtype = "f32",
  framework = "MPSGraph"
} : (...) -> (...)

%0 = tessera_nvidia.kernel_call %a, %b {
  op_kind = "batched_gemm",
  symbol = "tessera_nvidia_sm90_bmm_f32",
  status = "planned",
  abi = "cuda_wgmma_bmm_f32",
  dtype = "f32",
  framework = "CUDA"
} : (...) -> (...)

%0 = tessera_rocm.kernel_call %a, %b {
  op_kind = "batched_gemm",
  symbol = "tessera_rocm_gfx942_bmm_f32",
  status = "planned",
  abi = "hip_mfma_bmm_f32",
  dtype = "f32",
  framework = "HIP"
} : (...) -> (...)
```

The NVIDIA/ROCm examples are contract shapes, not execution claims. They remain
`status = "planned"` until an executor adapter and numerical proof exist.
...
## NVIDIA / ROCm inheritance (Stage 18)

NVIDIA and ROCm do **not** inherit Apple runtime behavior; they inherit the
compiler handoff shape. The acceptance ladder is shared:

1. Register the Graph IR op and verifier.
2. Preserve the executable envelope as a registered Tile op.
3. Lower the Tile op to a backend value call:
   `tessera_nvidia.kernel_call` or `tessera_rocm.kernel_call`.
4. Add an executor adapter that reads the value-call tuple and invokes the
   backend ABI named by `symbol`.
5. Flip `status` to `"executable"` only after numerical proof on the real
   backend.

Until steps 4 and 5 are complete, NVIDIA/ROCm value calls are compiler-visible
artifacts with `status = "planned"` or a named pass diagnostic. They must not
share Apple executor rows, Apple runtime probes, or Apple `execution_kind`
labels.
...
1. **Value Target IR first** — define the value ops (operands/results +
   `{op_kind, symbol, status, abi, dtype, framework, ...}`) and a value-mode `-full` lowering that
   `replaceOp`s the Tile op and fails loudly on unsupported ops. Pure IR; no
   hardware needed. (Apple: done; NVIDIA/ROCm: Stage 18 contract shape pinned.)
2. **Executor adapter second** — add a matrix row + an executor that reads the
   ...
   allowlist and `invalid_artifact` for everything outside it. (Apple CPU/GPU:
   narrow lanes done; NVIDIA/ROCm: pending.)
