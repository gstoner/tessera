# Gumiho — Hybrid Speculative Decoding (Apple and ROCm)
...
`matmul / linear_general / rmsnorm / silu_mul / relu / softmax`) or the
fail-closed native ROCm path, then reuses
...
|---|---|---|
| Target model (RMSNorm + MHA + SwiGLU + LM head) | `linear_general`, `matmul`, `rmsnorm`, `silu_mul`, `softmax` | Apple GPU/CPU or ROCm native step |
| Serial head (2-layer Transformer, autoregressive) | same decoder-layer ops | Apple GPU/CPU or ROCm native step |
| Parallel heads (5× `FC → ReLU → FC`) | `linear_general`, `relu` | Apple GPU/CPU or ROCm native step |
| FTA tree-attention verify (1 pass, additive ancestor mask) | batched `matmul` + masked `softmax` | Apple GPU/CPU or ROCm native step |
| Acceptance (Leviathan rule) + KV advance | `tessera.speculative.batch_verify` / `advance_kv` | host |

The Apple example can degrade to NumPy off Apple Silicon. ROCm never silently
does so: a missing GPU, target mismatch, or refused kernel aborts the step.
The demo separately runs a float64 NumPy oracle and cross-checks the result.
...
# Native ROCm dense kernels; select the exact live architecture:
TESSERA_ROCM_CHIP=gfx1201 python3 examples/advanced/gumiho/demo.py --mode step --target rocm
...
The ROCm step uses compiled f32 GEMM, batched GEMM, RMSNorm, softmax,
SwiGLU, ReLU, and binary kernels. Its `native_routes` receipt counts successful
native launches; any missing or refused kernel aborts the run. NumPy runs
separately as a numerical oracle. Token selection, FTA tree construction,
acceptance, KV bookkeeping, and host-array transport remain on the host.
This is a correctness demonstration, not a resident or performance claim;
other modes currently refuse `--target rocm` explicitly.
