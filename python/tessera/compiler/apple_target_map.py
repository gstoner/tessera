"""Apple generic op-dispatch map across apple_cpu + apple_gpu.

This module is the generated source for the **generic op-dispatch** question:
which ordinary compiler route carries an op, and which per-op framework or
reference fallback does it use?  It intentionally does *not* represent strict
Apple Value Target-IR calls or authored ``.mtlpackage`` subgraphs; those use
different execution units and live in ``apple_execution_inventory``. It pulls
from:
...
gives reviewers one table to scan for an answer to:
  ...
  * What does the generic Apple CPU vs Apple GPU dispatch route support today?
...
def render_markdown(rows: Iterable[AppleTargetRow] | None = None) -> str:
    ...
    lines: list[str] = [
        ...
        "",
        "# Apple generic op-dispatch map — apple_cpu vs apple_gpu",
        "",
        "Unified view of every op with generic Apple dispatch coverage today,",
        "pulled from ``capabilities.py``, ``backend_manifest.py``,",
        "and ``compiler/driver.py``.  Use this table as the single",
        "source of truth for generic per-op dispatch. It does **not** cover strict",
        "Apple Value Target-IR calls or authored `.mtlpackage` subgraphs; see",
        "`apple_execution_inventory.md` for those execution units. The fields:",
        "",
        ...
        "and ``docs/backends/apple/`` (Metal 4 implementation-state review):",
        "",
