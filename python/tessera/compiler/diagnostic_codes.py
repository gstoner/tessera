...
REGISTERED_CODES: tuple[DiagnosticCode, ...] = (
    ...
    DiagnosticCode(
        ...
        fix_hint=(
            ...
            "has no FP4 WMMA form, so fp4_e2m1 is refused here -- but read "
            "that as a gap in this route, NOT as MXFP4 being unreachable on "
            "RDNA4. Folding it into the fp8 WMMA is exact (E2M1's sixteen "
            "values are all representable in e4m3, and the E8M0 block scale "
            "is a power of two), which is how the ecosystem runs MXFP4 on "
            "gfx1201; see ROCM-MXFP4-W4A8-1. There is deliberately no "
            "generic gfx-prefix fallback: "
            "guessing a fragment ABI from an architecture prefix is the "
            "miscompile ROCM-5 removed, and it fails silently with wrong "
            "tiles rather than loudly."
        ),
    ...
    DiagnosticCode(
        ...
        severity="error",
        summary="Tile allocation reuse, arena materialization, deallocation, or provenance lacks a proven lifetime/completion path or a safe dynamic launch size.",
    ...
    # APPLE-MATMUL2D-1 — Metal 4 tensor_view / matmul2d[_epilogue] verifiers and lowerings.
    DiagnosticCode(
        code="APPLE_TENSOR_VIEW_LAYOUT", pass_origin="TesseraAppleDialect verifiers",
        severity="error",
        summary="A tensor_view's extents/strides/byte_offset are not a legal MTLTensor layout (innermost stride 1, 8/4-bit inner extent % 32, 128-byte row strides and data-plane offsets, 256-byte FP4 origins).",
        fix_hint="State a view Apple can bind: pad packed operands to the 128-byte stride quantum and place origins on 128-byte (FP4: 256-byte) boundaries.",
        spec="docs/audit/backend/apple/todo.md APPLE-MATMUL2D-1", sprint="APPLE-MATMUL2D-1",
    ),
    DiagnosticCode(
        code="APPLE_TENSOR_VIEW_STORAGE", pass_origin="TesseraAppleDialect verifiers",
        severity="error",
        summary="The view element type differs from the buffer element type; MTLTensor views reinterpret nothing.",
        fix_hint="Bind a view whose element type equals the buffer's, or convert the buffer explicitly before the view.",
        spec="docs/audit/backend/apple/todo.md APPLE-MATMUL2D-1", sprint="APPLE-MATMUL2D-1",
    ),
    DiagnosticCode(
        code="APPLE_TENSOR_VIEW_BOUNDS", pass_origin="TesseraAppleDialect verifiers",
        severity="error",
        summary="The view (origin + outer rows at the row stride + inner extent) reaches past the buffer's elements.",
        fix_hint="Shrink the extents or the origin, or bind a larger buffer.",
        spec="docs/audit/backend/apple/todo.md APPLE-MATMUL2D-1", sprint="APPLE-MATMUL2D-1",
    ),
    DiagnosticCode(
        code="APPLE_MATMUL2D_OPERANDS", pass_origin="TesseraAppleDialect verifiers",
        severity="error",
        summary="A matmul2d operand is not produced by tessera_apple.gpu.tensor_view, so its extents and layout are invisible to the verifier and the lowering.",
        fix_hint="Bind each operand with tessera_apple.gpu.tensor_view first.",
        spec="docs/audit/backend/apple/todo.md APPLE-MATMUL2D-1", sprint="APPLE-MATMUL2D-1",
    ),
    DiagnosticCode(
        code="APPLE_MATMUL2D_PAIR_UNSUPPORTED", pass_origin="TesseraAppleDialect verifiers",
        severity="error",
        summary="The operand storage pair is not one MetalPerformancePrimitives matmul2d accepts (same-type f16/bf16/f8E4M3FN/f8E5M2/f4E2M1FN, or f16 x {f8E4M3FN, f8E5M2, f4E2M1FN}).",
        fix_hint="Convert one operand to an admitted pair explicitly; the lane never converts silently.",
        spec="docs/audit/backend/apple/todo.md APPLE-MATMUL2D-1", sprint="APPLE-MATMUL2D-1",
    ),
    DiagnosticCode(
    ...
    DiagnosticCode(
        code="APPLE_MATMUL2D_SHAPE", pass_origin="TesseraAppleDialect verifiers",
        severity="error",
        summary="The views disagree on K, or the result shape is not [a.outer, b.inner].",
        fix_hint="Bind views whose inner/outer extents contract on K and state the result as tensor<MxNxf32>.",
        spec="docs/audit/backend/apple/todo.md APPLE-MATMUL2D-1", sprint="APPLE-MATMUL2D-1",
    ),
    DiagnosticCode(
        code="APPLE_MATMUL2D_DESCRIPTOR", pass_origin="TesseraAppleDialect verifiers",
        severity="error",
        summary="tile_m/tile_n are not positive multiples of 8 or simdgroups is not 1, 2, 4 or 8.",
        fix_hint="Use a matmul2d descriptor MPP accepts (the lowering emits 64x64 over 4 simdgroups).",
        spec="docs/audit/backend/apple/todo.md APPLE-MATMUL2D-1", sprint="APPLE-MATMUL2D-1",
    ),
    DiagnosticCode(
        code="APPLE_MATMUL2D_EPILOGUE_ACT", pass_origin="TesseraAppleDialect verifiers",
        severity="error",
        summary="The fused epilogue names an activation the runtime has no code for (none, relu, gelu, silu).",
        fix_hint="Use one of the four activations or leave the activation out of the fused op.",
        spec="docs/audit/backend/apple/todo.md APPLE-MATMUL2D-1", sprint="APPLE-MATMUL2D-1",
    ),
    DiagnosticCode(
        code="APPLE_MATMUL2D_EPILOGUE_EMPTY", pass_origin="TesseraAppleDialect verifiers",
        severity="error",
        summary="A matmul2d_epilogue op has neither a bias nor an activation; that is a plain gpu.matmul2d.",
        fix_hint="Emit gpu.matmul2d, or attach a bias operand or an activation.",
        spec="docs/audit/backend/apple/todo.md APPLE-MATMUL2D-1", sprint="APPLE-MATMUL2D-1",
    ),
    DiagnosticCode(
        code="APPLE_MATMUL2D_EPILOGUE_BIAS", pass_origin="TesseraAppleDialect verifiers",
        severity="error",
        summary="The epilogue bias is not tensor<Nxf32> (one fp32 value per output column).",
        fix_hint="Pass a rank-1 f32 bias whose extent equals b's inner extent N.",
        spec="docs/audit/backend/apple/todo.md APPLE-MATMUL2D-1", sprint="APPLE-MATMUL2D-1",
    ),
    DiagnosticCode(
        code="APPLE_MATMUL2D_UNRECOGNIZED", pass_origin="CanonicalGemmToAppleMatmul2dPass",
        severity="error",
        summary="A tessera.matmul carrying tessera.canonical_k_step does not slice two loop-invariant operands, so the nest is not the shared canonical reduction.",
        fix_hint="Run tessera-tiling first, or drop the canonical_k_step marker from a hand-built nest.",
        spec="docs/audit/backend/apple/todo.md APPLE-MATMUL2D-1", sprint="APPLE-MATMUL2D-1",
    ),
    DiagnosticCode(
        code="APPLE_MATMUL2D_SHAPE_UNSUPPORTED", pass_origin="CanonicalGemmToAppleMatmul2dPass",
        severity="error",
        summary="The canonical reduction's operands or result are not static rank-2 tensors, or the operands disagree on K after ragged padding was looked through.",
        fix_hint="Give the GEMM static rank-2 operands; dynamic shapes are not in this slice.",
        spec="docs/audit/backend/apple/todo.md APPLE-MATMUL2D-1", sprint="APPLE-MATMUL2D-1",
    ),
    DiagnosticCode(
    ...
    DiagnosticCode(
        code="APPLE_MATMUL2D_LAYOUT_UNSUPPORTED", pass_origin="CanonicalGemmToAppleMatmul2dPass",
        severity="error",
        summary="An operand cannot be bound as an MTLTensor view: a packed 8/4-bit row stride off the 128-byte quantum, a misaligned sub-block origin, or a 4-bit origin with no byte address.",
        fix_hint="Pad packed operands to the stride quantum and place sub-block origins on 128-byte (FP4: 256-byte) boundaries; the message repeats the verifier's reason.",
        spec="docs/audit/backend/apple/todo.md APPLE-MATMUL2D-1", sprint="APPLE-MATMUL2D-1",
    ),
    DiagnosticCode(
        code="APPLE_MATMUL2D_RAGGED_UNSUPPORTED", pass_origin="CanonicalGemmToAppleMatmul2dPass",
        severity="error",
        summary="The tiling pass's zero-padded product is consumed by something other than the slice back to its true extents, so the padding cannot be looked through.",
        fix_hint="Consume the GEMM result directly; if the padded tensor is needed, keep the padded form (the pass then binds it as-is).",
        spec="docs/audit/backend/apple/todo.md APPLE-MATMUL2D-1", sprint="APPLE-MATMUL2D-1",
    ),
    DiagnosticCode(
        code="APPLE_MATMUL2D_ADMIT", pass_origin="CanonicalGemmToAppleMatmul2dPass",
        severity="error",
        summary="The pass option `admit` is not 'all' or 'lowp'; the admission set is closed.",
        fix_hint="Pass admit=all (standalone) or admit=lowp (the default pipeline's measured admission).",
        spec="docs/audit/backend/apple/todo.md APPLE-MATMUL2D-1", sprint="APPLE-MATMUL2D-1",
    ),
