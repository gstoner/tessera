...
DISTRIBUTED_PRIMITIVE_CONTRACTS: dict[str, DistributedPrimitiveContract] = {
    "named_sharding": DistributedPrimitiveContract(
        "named_sharding", "compile_time_metadata"
    ),
    "partition_spec": DistributedPrimitiveContract(
        "partition_spec", "compile_time_metadata"
    ),
    "shard_map": DistributedPrimitiveContract("shard_map", "compile_time_region"),
    "psum": DistributedPrimitiveContract(
        "psum", "collective_target", "all_reduce", "sum",
        adjoint="broadcast_to_axis",
    ),
    "pmean": DistributedPrimitiveContract(
        "pmean", "collective_target", "all_reduce", "sum", True,
        "broadcast_to_axis_scaled",
    ),
    "pmax": DistributedPrimitiveContract(
        "pmax", "collective_target", "all_reduce", "max",
        adjoint="argmax_routed",
    ),
    "pmin": DistributedPrimitiveContract(
        "pmin", "collective_target", "all_reduce", "min",
        adjoint="argmin_routed",
    ),
    "broadcast_to_axis": DistributedPrimitiveContract(
        "broadcast_to_axis", "collective_target", "all_gather",
        adjoint="psum",
    ),
    "collective_permute": DistributedPrimitiveContract(
        "collective_permute", "point_to_point_target",
...
def collective_record_for_primitive(
    ...
    *,
    ...
    if contract.ownership != "collective_target":
        raise ValueError(
            f"{name} is owned by {contract.ownership}, not collective transport"
        )
