...
* **Tier 3 — hand-tuned** (:attr:`Tier.HAND_TUNED`): the crown-jewel shipped
  kernels (ROCm WMMA GEMM / device_verified_jit FA-2 flash-attn, NVIDIA ``wgmma``, authored
  MSL). **Lead-safety: these stay first-class candidates, displaced only when a
  device_verified_jit kernel measures both faster and in accuracy budget.**
...
# --- D3: arbiter dispatch log (did the device_verified_jit path win or silently degrade?) --
#
...
def arbiter_dispatch_histogram(target: str | None = None, op: str | None = None
    ...
    device_verified_jit path win, silently degrade to the reference, or was no candidate
