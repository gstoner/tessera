...
def _synthesize_linear_attn_variant_bwd_cuda() -> str:
    return ("#include <cuda_runtime.h>\n#include <math.h>\n#define DC 256\n"
        ...
        "extern \"C\" int tessera_nvidia_linear_attn_variant_bwd(const float*hg,const float*hq,const float*hk,const float*hv,const float*hd,float*hdq,float*hdk,float*hdv,long B,int H,long S,int D,int Vd,int f){if(!hg||!hq||!hk||!hv||!hdq||!hdk||!hdv||D>DC)return 2;size_t nq=(size_t)B*H*S*D*4,nv=(size_t)B*H*S*Vd*4,nd=(size_t)B*H*S*4;float*g,*q,*k,*v,*d=0,*dq,*dk,*dv;cudaMalloc(&g,nv);cudaMalloc(&q,nq);cudaMalloc(&k,nq);cudaMalloc(&v,nv);cudaMalloc(&dq,nq);cudaMalloc(&dk,nq);cudaMalloc(&dv,nv);if(hd)cudaMalloc(&d,nd);cudaMemcpy(g,hg,nv,cudaMemcpyHostToDevice);cudaMemcpy(q,hq,nq,cudaMemcpyHostToDevice);cudaMemcpy(k,hk,nq,cudaMemcpyHostToDevice);cudaMemcpy(v,hv,nv,cudaMemcpyHostToDevice);if(hd)cudaMemcpy(d,hd,nd,cudaMemcpyHostToDevice);cudaMemset(dk,0,nq);cudaMemset(dv,0,nv);long r=B*(long)H*S;kb<<<(r+127)/128,128>>>(g,q,k,v,d,dq,dk,dv,B,H,S,D,Vd,f);int ok=cudaDeviceSynchronize()==cudaSuccess?1:3;if(ok==1){cudaMemcpy(hdq,dq,nq,cudaMemcpyDeviceToHost);cudaMemcpy(hdk,dk,nq,cudaMemcpyDeviceToHost);cudaMemcpy(hdv,dv,nv,cudaMemcpyDeviceToHost);}return ok;}\n")
