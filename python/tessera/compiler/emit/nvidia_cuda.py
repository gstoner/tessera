...
#: Update rules ``opt_k`` implements, by code: 0 sgd, 1 momentum, 2 adam,
#: 3 adamw, 4 lion, 5 nesterov. Mirrors ``runtime._OPTIMIZER_OPS``; the kernel
#: entry rejects anything outside this set with rc 2.
_OPTIMIZER_KINDS = frozenset({0, 1, 2, 3, 4, 5})
...
__global__ void tsr_flash_bwd_stats(const float*go,const float*q,const float*k,const float*v,const float*bias,float*sm,float*sz,float*sd,long B,int Hq,int Hkv,long Sq,long Sk,int D,int Dv,float scale,int causal,long wl,long wr,float cap){
 ...
 sm[row]=mx;sz[row]=z;sd[row]=z>0.f?delta_num/z:0.f;}
__global__ void tsr_flash_bwd_dq(const float*go,const float*q,const float*k,const float*v,const float*bias,const float*sm,const float*sz,const float*sd,float*dq,long B,int Hq,int Hkv,long Sq,long Sk,int D,int Dv,float scale,int causal,long wl,long wr,float cap){
 long row=(long)blockIdx.x*blockDim.x+threadIdx.x,total=B*(long)Hq*Sq;if(row>=total)return;long m=row%Sq,tmp=row/Sq;int qh=(int)(tmp%Hq);long b=tmp/Hq;int hk=qh/(Hq/Hkv);
 float mx=sm[row],z=sz[row],delta=sd[row];long goo=(((b*(long)Hq+qh)*Sq+m)*Dv);float aq[TSR_FA_CAP];for(int d=0;d<D;++d)aq[d]=0.f;
 for(long n=0;n<Sk&&z>0.f;++n){if(!tsr_legal(n,m,causal,wl,wr))continue;float raw=tsr_score(q,k,bias,b,qh,hk,m,n,Hq,Hkv,Sq,Sk,D,scale,0.f),s=raw,deriv=1.f;if(cap>0.f){float t=tanhf(s/cap);s=cap*t;deriv=1.f-t*t;}float p=expf(s-mx)/z,dp=0.f;long ko=(((b*(long)Hkv+hk)*Sk+n)*D),vo=(((b*(long)Hkv+hk)*Sk+n)*Dv);for(int j=0;j<Dv;++j)dp+=go[goo+j]*v[vo+j];float ds=p*(dp-delta)*deriv*scale;for(int d=0;d<D;++d)aq[d]+=ds*k[ko+d];}
 for(int d=0;d<D;++d)dq[(((b*(long)Hq+qh)*Sq+m)*D)+d]=aq[d];}
__global__ void tsr_flash_bwd_split(const float*go,const float*q,const float*k,const float*v,const float*bias,const float*sm,const float*sz,const float*sd,float*dk,float*dv,long B,int Hq,int Hkv,long Sq,long Sk,int D,int Dv,float scale,int causal,long wl,long wr,float cap,int split){
...
def _synthesize_linear_attn_variant_cuda() -> str:
    ...
    The decay weight of key ``n`` in output row ``m`` is ``prod(dec[n+1..m])``.
    Materialising it per key is O(S) inside an O(S) key loop, so the kernel
    instead folds it into the accumulator as a Horner recurrence
    (``y = y*dec[n] + s*v``), which yields the same product for every key at
    O(1) each and keeps the summation over ``n`` ascending.  Each key's decay
    factors are still applied in ascending ``u`` order; only the point at which
    ``s*v`` enters the chain moves, so f32 results shift by rounding.
    """
    return (
        "#include <cuda_runtime.h>\n#include <math.h>\n"
...
def _synthesize_linear_attn_variant_bwd_cuda() -> str:
    """VJP of the decayed linear-attention forward.

    ``kb`` walks keys from ``n = m`` down so the decay weight
    ``prod(dec[n+1..m])`` is a running product updated once per key, rather than
    an O(S) inner loop per key.  Unlike the forward, the factor is consumed by
    per-key atomics into ``dv``/``dk`` (distinct addresses), so it cannot be
    folded into an accumulator and the descending walk is the only O(1) form —
    this reverses the ``a[]`` summation order and the product association, so
    f32 results shift by rounding.
    """
...
def _synthesize_softmax_cuda() -> str:
    ...
    return (
        ...
        "    __syncthreads();\n"
        "  }\n"
        "  mx = scratch[0]; __syncthreads();   // every thread must read the max\n"
        "                                     // before any overwrites scratch\n"
        "  float sum = 0.0f;\n"
...
def _synthesize_norm_cuda(dtype: str = "f32") -> str:
    ...
    return (
        ...
        "  float r=s[0]; __syncthreads();  // consecutive tsr_sum calls share `s`:\n"
        "  return r;                       // barrier before the next s[t]=v\n"
        "}\n"
...
def run_optimizer_f32(kind: int, p: Any, g: Any, m: Any, v: Any, n: int,
                      ...
                      b1c: float, b2c: float):
    import numpy as np
    # `kind` selects the update rule, so it is a semantic key: an unrecognised
    # code must be refused here rather than reaching opt_k, whose trailing
    # branch would apply the Adam/AdamW update and report a successful launch
    # (Decision #21a). Checked before any compile so no artifact is built for a
    # request that cannot be served.
    if kind not in _OPTIMIZER_KINDS:raise ValueError(f"NVIDIA optimizer kind {kind!r} is not one of {sorted(_OPTIMIZER_KINDS)}")
...
class NvidiaMmaGemmEmittedCandidate(Candidate):
    ...
    def measure_device_latency(self, region: Any, *inputs: Any, reps: int = 100,
        ...
        The NVIDIA backend carries two block-index conventions: `ptx_emit` and
        the shipped AOT kernel map blockIdx.x -> M, while the Tile lowering
        (`NVIDIALowering.cpp`) and the launch bridge map blockIdx.x -> N. The
        harness used to hardcode the latter, so this lane returned rc=5 and had
