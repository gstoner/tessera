"""Portable-C fused-region candidate for the canonical x86 compiler.

This is deliberately registered as source target ``x86_c``, not ``x86``.
Canonical target ``x86`` is owned by Graph/Tile IR lowering through
``TileToX86Pass`` and the typed native-image package.  The C path remains one
measured fused-region candidate under the x86 arbiter; it is not a second
compiler authority for the canonical target.

Three seams implement that bounded candidate:

* :class:`X86CEmitter` (``register_emitter``) — ``FusedRegion`` → portable C
  source (matmul + prologue/epilogue/residual/reduction), f32.
* :func:`_x86_compile_fn` (``register_compiler``) — ``cc``/``clang -O3
  -march=x86-64-v4 -shared`` → a portable AVX-512 ``.so``.
* :class:`X86CRunner` (``register_runner``, ``default=False``) — ``ctypes``
  launch → ``(out, "x86_c_native")`` when this candidate ran, else the numpy
...
_TARGET = "x86_c"
_CANDIDATE_TARGET = "x86"
_LANG = "c"
_ENTRY = "tessera_x86_fused"
_REAL_TAG = "x86_c_native"
_MARCH = "x86-64-v4"
_X86_64_V4_FLAGS = frozenset({
    "avx512f", "avx512bw", "avx512cd", "avx512dq", "avx512vl",
})


def host_supports_x86_64_v4() -> bool:
    """Whether this host can safely execute an ``x86-64-v4`` artifact."""
...
def _synthesize_fused_c(region: FusedRegion) -> str:
    ...
    return (
        f"/* tessera-x86-c-architecture: {_MARCH} */\n"
        "#include <math.h>\n"
...
class X86CRunner(KernelRunner):
    ...
    def run_fused_region(self, region: Any, A: Any, B: Any, bias: Any = None,
        ...
        import numpy as np
        if not host_supports_x86_64_v4():
...
class X86GenericCCandidate(Candidate):
    ...
    the explicit ``-march=x86-64-v4`` artifact profile). Serves any
    ``FusedRegion`` — the portable x86 floor the AOCL-DLP Tier-3 candidate
    (C1b) is measured against."""
