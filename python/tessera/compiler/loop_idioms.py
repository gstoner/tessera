...
def recognize_attention_loop(fn: Callable) -> LoopIdiomCandidate:
    ...
    # Every bias axis is either the exact attention extent or 1 (broadcast):
    # batch/head since 2026-09-12, query/key since 2026-09-15 (a key-padding
    # mask is [1, 1, 1, K]; a per-query mask is [B, HQ, Q, 1]). The physical
    # shape is what reaches the kernel; nothing is expanded on the host.
    if bias is not None and (bias.dim_names[0] not in ('1', qs[0]) or
                            bias.dim_names[1] not in ('1', qs[1]) or
                            bias.dim_names[2] not in ('1', qs[2]) or
                            bias.dim_names[3] not in ('1', ks[2])):
        raise ValueError("attention bias axes must each be the attention extent or 1 (batch, head, query, key)")
...
'''
    ...
    if bias is not None:
        ...
        bias_index = ", ".join((
            '0' if bias.dim_names[0] == '1' else 'batch',
            '0' if bias.dim_names[1] == '1' else 'head',
            '0' if bias.dim_names[2] == '1' else 'query',
            '0' if bias.dim_names[3] == '1' else ':',
        ))
        template = template.replace("                weights = np.exp", f"                scores = scores + {bias.name}[{bias_index}]\n                weights = np.exp")
