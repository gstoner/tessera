"""Target-independent device/model capacity policy for rematerialization.

Backends supply an exact retained-context capacity envelope. This module
derives a conservative activation budget and stamps the shared Graph IR
attributes consumed by ``tessera-activation-rematerialization``.
"""
...
class RematerializationCapacityPolicy:
...
def resolve_rematerialization_capacity_policy(
    *,
    model_parameter_bytes: int,
...
) -> RematerializationCapacityPolicy:
    """Resolve a shared rematerialization envelope from a device query."""
    if capacity_bytes is None:
        raise ValueError("capacity_bytes must come from a target device query")
    ...
    return RematerializationCapacityPolicy(
...
def apply_rematerialization_capacity_policy(
    ...
    *,
    ...
    optimizer_state_copies: int = 2,
    persistent_bytes: int = 0,
) -> dict[str, RematerializationCapacityPolicy]:
    """Mark model parameters and stamp a target capacity policy on Graph IR.
    ...
    policies: dict[str, RematerializationCapacityPolicy] = {}
    ...
    for function in module.functions:
        ...
        if not has_parameter:
            continue
        policy = resolve_rematerialization_capacity_policy(
...
def _stamp_function_policy(
    function: GraphIRFunction, policy: RematerializationCapacityPolicy
) -> None:
