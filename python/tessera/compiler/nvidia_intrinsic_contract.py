...
SM120_INTRINSIC_CONTRACTS: tuple[SM120IntrinsicContract, ...] = (
    ...
    SM120IntrinsicContract(
        ...
        ("__brev", "__byte_perm", "__clz", "__ffs", "__popc"),
        "bit-exact scalar operations with operation-specific zero-input results",
        "ready", "planned", "planned",
        "__fns base outside 0..31 is undefined",
        ".b32/.b64 or same-width integer registers",
    ),
    SM120IntrinsicContract(
        "funnel_shift_l_wrap", "integer_bits", ("__funnelshift_l",),
        "bit-exact 64-to-32 funnel shift with a modulo-32 shift count",
        "ready", "ready", "ready",
        "wrap and clamp shift-count contracts are not interchangeable",
        ".b32 low/high inputs, .u32 shift, and .b32 result",
    ),
    SM120IntrinsicContract(
    ...
    SM120IntrinsicContract(
        ...
        ("__vadd4", "__vaddss4", "__vabsdiffs4", "__vcmpeq4"),
    ...
    SM120IntrinsicContract(
        "packed_simd_add_u16x2", "packed_simd", ("__vadd2",),
        "two independent unsigned 16-bit wrapping additions",
        "ready", "ready", "ready",
        "lane width, unsignedness, and wrapping behavior are fixed",
        ".b32 containing two .u16 lanes",
    ),
)
