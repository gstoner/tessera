...
REGISTERED_PASSES: tuple[PassMetadata, ...] = (
    ...
    PassMetadata(
        ...
        summary="Lowers isolated static floating/integer bufferized AD or source-bound ANN products to GPU entries, preserving full residual shapes. Optional parallel-ann-rows proves row-independent memory accesses and assigns one row per thread for batches 2 through 64; the default remains serial. Bounded for/if temporaries use dtype-sized slots and are limited to 4096 reserved bytes, with distinct slots per iteration path. The backend option selects NVVM generic or AMDGPU private allocation addressing; replay bounds derived from enclosing induction variables are supported, with SSA-bounded dynamic allocations retaining logical dimensions and capacity-sized slots. Dynamic copies require identical dimension SSA operands. Optional status-buffer appends a guard-v1 i64 status result, propagates nested for/if assertion failures through region exits and later iterations. Optional input-status gates all body effects on an upstream guard-v1 status. Native rank-one public result programs project capacity/shape sidecars from typed result_shape argument attributes and validate returned extents before exposure. Optional public-result-capacity projects multiple rank-one through rank-four AD returns into flat capacity storage and per-axis logical shape sidecars, checking both extents and total volume. Optional public-input-capacity binds dynamic AD inputs through guarded sidecars and contiguous native descriptors; bounded unsigned clamps participate in the allocation range proof. Selects with bounded integer alternatives are supported. Unbounded loaded extents, unknown dynamic aliases and while loops refuse.",
        input_dialects=("func", "arith", "math", "scf", "memref", "cf"),
        output_dialects=("gpu", "llvm", "arith", "math", "scf", "memref", "tile"),
        required_attrs=("tessera.autodiff.product_abi", "tessera.autodiff.product_pair", "tessera.ann.source", "tessera.native_result_program", "tessera.result_shape", "tessera.input_shape"),
        preserved_attrs=("tessera.autodiff.product_abi", "tessera.autodiff.product_pair", "tessera.ann.source", "tessera.autodiff.temporary_bytes", "tessera.autodiff.gpu_status", "tessera.autodiff.input_status", "tessera.native_result_program", "tessera.native_result_abi"),
