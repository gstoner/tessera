    ...
    *,
    ...
    key = hashlib.sha256(
        (
            ...
            f"{tile_q}|{tile_kv}|{staging}|{lds_waves[0]}x{lds_waves[1]}|k{k_unroll}|w{waves_per_eu}|{library_identity}|{depth_cooperative}|{hashlib.sha256(tool.read_bytes()).hexdigest()}"
        ).encode()
    ).hexdigest()
