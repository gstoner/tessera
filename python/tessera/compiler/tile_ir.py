...
ops and tessera_attn.* FA-4 helpers. Async ordering is represented by explicit
SSA-like token lineage; the former ``tessera.queue.*`` compatibility markers
are not part of the compiler-owned Tile contract.
"""
...
class TileIRVerifier:
    ...
    def verify_function(self, fn: TileFunction) -> TileIRVerificationResult:
        diagnostics: list[TileIRDiagnostic] = []
        tokens: set[str] = set()
        waited_tokens: set[str] = set()
        self._verify_ops(fn.body, diagnostics, tokens, waited_tokens)
        return TileIRVerificationResult(tuple(diagnostics))
    ...
    for op in ops:
        ...
        if op.op_name == "schedule.marker":
            ...
            if marker == "tessera.barrier":
                ...
                lowered.append(TileOp("tile.debug_barrier", {"scope": "block", "source": marker, "ordinal": ordinal}))
...
def _elementwise_op(op: ScheduleOp) -> TileOp:
    ...
    if source.startswith("tessera.kv_cache."):
        ...
        return TileOp("tile.group", {"source": source, "ordinal": op.attrs.get("ordinal", 0)}, body=ops)
    ops[0].operands = list(op.operands)
    ops[0].result = op.result
    return ops[0]
...
def _async_token_name(op: ScheduleOp) -> str:
    return f"async_{int(op.attrs.get('ordinal', 0))}"
