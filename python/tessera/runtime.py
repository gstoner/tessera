...
def _build_compiled_flash_attn_hsaco(head_dim: int, dtype: str = "f16",
    ...
    if r.returncode != 0 or "gpu.binary" not in r.stdout:
        raise _RocmCompiledUnavailable(
            "tessera-opt did not serialize the compiled flash_attn in-process "
            f"(rc={r.returncode}): {r.stderr[:400]}")
    hsaco = _extract_hsaco_blob(r.stdout)
    if hsaco[:4] != b"\x7fELF":
        raise _RocmCompiledUnavailable(
            "compiled ROCm flash_attn lane: gpu.binary was not an ELF hsaco")
...
def _execute_rocm_compiled_flash_attn(artifact: RuntimeArtifact, args: Any) -> Any:
    ...
    if head_dim <= 0 or head_dim % 16 != 0:
        raise ValueError(
            f"rocm flash_attn lane needs head_dim a positive multiple of 16; "
            f"got {head_dim}")
    sq = int(q.shape[-2])
    sk = int(k.shape[-2])
    ...
    if q.dtype == np.float16:
        ...
        if bf16 is not None and q.dtype == bf16:
            ...
            raise ValueError(
                "rocm flash_attn lane handles f16/bf16 storage (f32 softmax + "
    ...
    if hip.hipModuleLoadData(ctypes.byref(mod), hsaco) != 0:
        raise _RocmCompiledUnavailable(
            "rocm flash_attn: no usable AMD GPU (module load failed)")
    fn = ctypes.c_void_p()
...
#: op_name -> (bitwise kind index, operand count). Must match avx512_bitwise_i32.
_X86_BITWISE_OPS = {
...
def _execute_x86_compiled_bitwise(artifact: RuntimeArtifact, args: Any) -> Any:
    """The ``target="x86"`` bitwise lane: run the hand-written AVX-512 elementwise
    bitwise kernel (and/or/xor/not over i32 integers, full bit pattern) loaded
    from libtessera_x86_elementwise.so. ``not`` is unary, the rest binary."""
    import numpy as np
    ...
    ops = list(metadata.get("ops") or [])
    op_name = str(ops[0].get("op_name", "")) if len(ops) == 1 else ""
    if len(ops) != 1 or op_name not in _X86_BITWISE_OPS:
        raise ValueError(
            "x86_bitwise_compiled executor handles exactly one of "
            f"{tuple(_X86_BITWISE_OPS)}; got {[o.get('op_name') for o in ops]!r}")
    kind, nin = _X86_BITWISE_OPS[op_name]
    op = ops[0]
    operand_names = [str(n) for n in op.get("operands", [])]
    if len(operand_names) < nin:
        raise ValueError(f"bitwise {op_name} requires {nin} operand(s)")
    values = _bind_launch_args(args, arg_names)
    a = _as_numpy(values[operand_names[0]])
    bnp = None
    if nin == 2:
        ...
        if a.shape != bnp.shape:
            raise ValueError(
                f"x86 bitwise lane requires matching operand shapes; got "
                f"a{a.shape} b{bnp.shape}")
    n = int(np.prod(a.shape)) if a.ndim else 1
    if n <= 0:
    ...
    if lib is None:
        raise _RocmCompiledUnavailable(
            "libtessera_x86_elementwise.so not loadable")
    cv = ctypes.c_void_p
    ac = np.ascontiguousarray(a, dtype=np.int32).reshape(-1)
    # `bc` must stay referenced until the call returns (ctypes holds a raw ptr).
    bc = (np.ascontiguousarray(bnp, dtype=np.int32).reshape(-1)
          if nin == 2 else np.zeros(0, np.int32))
    bptr = bc.ctypes.data_as(cv) if nin == 2 else cv(None)
    out = np.zeros(n, dtype=np.int32)
    lib.tessera_x86_avx512_bitwise_i32(
        ac.ctypes.data_as(cv), bptr, ctypes.c_int64(n),
        out.ctypes.data_as(cv), ctypes.c_int(kind))
    return out.reshape(a.shape)
...
# ─────────────────────────────────────────────────────────────────────────────
# ROCm COMPILED binary-arithmetic lane (2026-06-26) — S2 binary-arithmetic family
# A flat 2-operand per-element kernel applying a pointwise binary arithmetic fn
# (sub/div/pow/maximum/minimum), the binary sibling of the unary-math lane.
# ``compiler_path = "rocm_binary_compiled"``. f32/f16/bf16 storage, f32 compute.
# ONE hsaco per (chip, kind, dtype), cached.
# ─────────────────────────────────────────────────────────────────────────────
_rocm_binary_hsaco_cache: dict[tuple[str, str, str], bytes] = {}

#: op_name → binary kernel kind (the codegen `kind` attr).
_ROCM_BINARY_OPS: dict[str, str] = {
...
def _build_compiled_binary_hsaco(kind: str, dtype: str = "f32") -> bytes:
    chip = _rocm_chip()
    directive = (
        'module {\n'
        f'  "tessera_rocm.binary"() {{name = "b", kind = "{kind}", '
        f'dtype = "{dtype}"}} : () -> ()\n}}\n')
    return _build_rocm_elementwise_hsaco(
        "generate-rocm-binary-kernel", directive, _rocm_binary_hsaco_cache,
        (chip, kind, dtype))


def _execute_rocm_compiled_binary(artifact: RuntimeArtifact, args: Any) -> Any:
    """The ``target="rocm"`` binary-arithmetic lane: run the COMPILER-GENERATED
    flat 2-operand elementwise kernel for a standalone binary arithmetic fn
    (sub/div/pow/maximum/minimum) over any-shape inputs (one thread per
    element). f32/f16/bf16 storage, f32 compute."""
    import numpy as np
    ...
    ops = list(metadata.get("ops") or [])
    op_name = str(ops[0].get("op_name", "")) if len(ops) == 1 else ""
    if len(ops) != 1 or op_name not in _ROCM_BINARY_OPS:
        raise ValueError(
            "rocm_binary_compiled executor handles exactly one of "
            f"{tuple(_ROCM_BINARY_OPS)}; got {[o.get('op_name') for o in ops]!r}")
    kind = _ROCM_BINARY_OPS[op_name]
    op = ops[0]
    operand_names = [str(n) for n in op.get("operands", [])]
    if len(operand_names) < 2:
        raise ValueError("binary requires two operands (a, b)")
    ...
    if a.shape != b.shape:
        raise ValueError(
            f"rocm binary lane requires matching operand shapes; got a{a.shape} "
            f"b{b.shape}")
    n = int(np.prod(a.shape)) if a.ndim else 1
    if n <= 0:
        return np.array(a, copy=True)
    ...
    if a.dtype == np.float32:
        ...
        if bf16 is not None and a.dtype == bf16:
            ...
            raise ValueError(
                f"rocm binary lane handles f32/f16/bf16; got {a.dtype}")

    hsaco = _build_compiled_binary_hsaco(kind, dtype_tag)
    hip = _load_hip_for_launch()
    if hip is None:
        raise _RocmCompiledUnavailable("libamdhip64.so not loadable")
    if hip.hipInit(0) != 0:
    ...
    for dev in (da, db, do):
        if hip.hipMalloc(ctypes.byref(dev), esz * n) != 0:
            raise RuntimeError("rocm binary: hipMalloc failed")
    ...
    if rc != 0:
        for dev in (da, db, do):
            hip.hipFree(dev)
        raise RuntimeError(f"rocm binary: kernel launch failed rc={rc}")
    hip.hipDeviceSynchronize()
    hip.hipMemcpy(o.ctypes.data_as(ctypes.c_void_p), do, esz * n, 2)
    for dev in (da, db, do):
        hip.hipFree(dev)
...
# ─────────────────────────────────────────────────────────────────────────────
# ROCm COMPILED logical lane (2026-06-26) — S2 logical family
# A flat elementwise kernel applying a pointwise logical op (and/or/xor/not) over
# any-shape i8 bool inputs, producing a bool result. ``compiler_path =
# "rocm_logical_compiled"``. Inputs normalized to bool via != 0 (numpy
# semantics). ``not`` is unary; and/or/xor binary. ONE hsaco per (chip, kind).
# ─────────────────────────────────────────────────────────────────────────────
_rocm_logical_hsaco_cache: dict[tuple[str, str], bytes] = {}

#: op_name → (logical kernel kind, operand count).
_ROCM_LOGICAL_OPS: dict[str, tuple[str, int]] = {
    "tessera.logical_and": ("and", 2), "tessera.logical_or": ("or", 2),
    "tessera.logical_xor": ("xor", 2), "tessera.logical_not": ("not", 1),
}
...
def _build_compiled_logical_hsaco(kind: str) -> bytes:
    ...
    directive = (
        'module {\n'
        f'  "tessera_rocm.logical"() {{name = "l", kind = "{kind}"}} '
        ': () -> ()\n}\n')
    return _build_rocm_elementwise_hsaco(
        "generate-rocm-logical-kernel", directive, _rocm_logical_hsaco_cache,
        (chip, kind))


def _execute_rocm_compiled_logical(artifact: RuntimeArtifact, args: Any) -> Any:
    """The ``target="rocm"`` logical lane: run the COMPILER-GENERATED flat
    elementwise logical kernel (and/or/xor/not) over any-shape inputs, producing
    a numpy ``bool_`` result. Inputs are normalized to bool (``!= 0``); ``not``
    is unary, the rest binary."""
    import numpy as np
    ...
    ops = list(metadata.get("ops") or [])
    op_name = str(ops[0].get("op_name", "")) if len(ops) == 1 else ""
    if len(ops) != 1 or op_name not in _ROCM_LOGICAL_OPS:
        raise ValueError(
            "rocm_logical_compiled executor handles exactly one of "
            f"{tuple(_ROCM_LOGICAL_OPS)}; got {[o.get('op_name') for o in ops]!r}")
    kind, nin = _ROCM_LOGICAL_OPS[op_name]
    op = ops[0]
    operand_names = [str(n) for n in op.get("operands", [])]
    if len(operand_names) < nin:
        raise ValueError(f"logical {kind} requires {nin} operand(s)")
    values = _bind_launch_args(args, arg_names)
    a = _as_numpy(values[operand_names[0]])
    inputs = [a]
    if nin == 2:
        ...
        if a.shape != bb.shape:
            raise ValueError(
                f"rocm logical lane requires matching operand shapes; got "
    ...
    hsaco = _build_compiled_logical_hsaco(kind)
    hip = _load_hip_for_launch()
    if hip is None:
        raise _RocmCompiledUnavailable("libamdhip64.so not loadable")
    if hip.hipInit(0) != 0:
        raise _RocmCompiledUnavailable("rocm logical: hipInit failed")
    mod = ctypes.c_void_p()
    if hip.hipModuleLoadData(ctypes.byref(mod), hsaco) != 0:
        raise _RocmCompiledUnavailable("rocm logical: no usable AMD GPU")
    fn = ctypes.c_void_p()
    if hip.hipModuleGetFunction(ctypes.byref(fn), mod, b"l") != 0:
        raise RuntimeError("rocm logical: kernel symbol 'l' not found")

    # i8 bool storage (1 byte/element); normalize numpy inputs to 0/1 bytes.
    devs: list[Any] = []
    ...
    for arr_in in inputs:
        host = (np.ascontiguousarray(arr_in).reshape(-1) != 0).astype(np.uint8)
        dev = ctypes.c_void_p()
        if hip.hipMalloc(ctypes.byref(dev), n) != 0:
            for d in devs:
                hip.hipFree(d)
            raise RuntimeError("rocm logical: hipMalloc failed")
        hip.hipMemcpy(dev, host.ctypes.data_as(ctypes.c_void_p), n, 1)
        devs.append(dev)
        launch_args += _mr(dev, n)

    o = np.zeros(n, dtype=np.uint8)
    do = ctypes.c_void_p()
    if hip.hipMalloc(ctypes.byref(do), n) != 0:
        for d in devs:
            hip.hipFree(d)
        raise RuntimeError("rocm logical: hipMalloc failed")
    devs.append(do)
    ...
    if rc != 0:
        for d in devs:
            hip.hipFree(d)
        raise RuntimeError(f"rocm logical: kernel launch failed rc={rc}")
    hip.hipDeviceSynchronize()
    hip.hipMemcpy(o.ctypes.data_as(ctypes.c_void_p), do, n, 2)
    for d in devs:
        hip.hipFree(d)
