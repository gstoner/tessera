...
// selected-block decode kernels. The tiled variant maps one workgroup to one
// (B,Hq,Sq) query row and one thread to one value lane, which is the first
// cooperative sparse-flash promotion beyond the scalar-per-output proof.
//===----------------------------------------------------------------------===//
...
struct GenerateROCMBlockSparseAttnKernelPass
  ...
  void runOnOperation() override {
    ...
    for (Operation *op : directives) {
      ...
      emitBody(body, loc, gpuFunc, rowTiled);
      op->erase();
    }
  }
};
