...
// RUN: %trop --allow-unregistered-dialect --pass-pipeline='builtin.module(lower-tile-to-rocm{arch=gfx1201},generate-rocm-depth-attention-kernel)' %s | FileCheck %s --check-prefix=GENERATED
// RUN: %trop --allow-unregistered-dialect --pass-pipeline='builtin.module(tessera-rocm-executable{family=depth_attention input=tile output=target arch=gfx1201})' %s | FileCheck %s --check-prefix=PIPELINE-TARGET
...
module {
  llvm.func @block_attnres_depth_attention(
    ...
    tile.depth_attention_kernel %query, %sources, %output,
      ...
      accum = "f32", arch = "gfx1201", eps = 1.000000e-06 : f64,
