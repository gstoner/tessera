...
//
// The rows are PADDED (ROCM-LDS-BANKPAD-1, default `lds-pad-dwords=4`), so the
// stride is 24 f16 rather than 16: an unpadded 16-element row is 8 dwords and
// `gcd(8, 32) = 8`, which puts sixteen lanes on four banks -- a 4-way conflict
// on every fragment read.
//
// The default is 4 rather than the conflict-free 1 because it MEASURES faster
// (2048^3 f16: 40.3 against 39.5, and 35.0 unpadded), even though a 24-f16
// stride is 12 dwords and `gcd(12, 32) = 4` reinstates a 4-way conflict. The
// conflict model explains the +13% step from unpadded to padded and does not
// explain the 2% above it -- see wmma-fragment-layout.md section 10k. Pinning
// the number here is what keeps that an open question rather than a silent
// regression.
//
// So 64*24 + 128*24 halves = 9216 bytes, not 64*16 + 128*16 = 6144. If these
// go back to 1024/2048/6144 the padding has been lost, which is silent at
// runtime and shows only as lower throughput.
// GEN: gpu.func @gemm
// GEN-SAME: workgroup(%{{.*}}: memref<1536xf16, #gpu.address_space<workgroup>>, %{{.*}}: memref<3072xf16, #gpu.address_space<workgroup>>)
// GEN-SAME: known_block_size = array<i32: 128, 1, 1>
// GEN-DAG: tessera.rocm.lds_bytes = 9216
// GEN-DAG: tessera.rocm.lds_pad_dwords = 4
