...
func.func @empty_callee(%a: f32) -> f32 {
  ...
  %r = tessera_nvidia.kernel_call %a
      {callee = "", arch = "sm_120", binding = "c_abi",
       provenance = "vendor_library", accuracy = "reference_exact", determinism = "deterministic", covers = "whole_region"}
      : (f32) -> f32
  return %r : f32
}
...
func.func @bounded_without_a_bound(%a: f32) -> f32 {
  ...
  %r = tessera_nvidia.kernel_call %a
      {callee = "cublasLtMatmul", arch = "sm_120", binding = "c_abi",
       provenance = "vendor_library", accuracy = "tolerance_bounded", determinism = "deterministic", covers = "whole_region"}
      : (f32) -> f32
  return %r : f32
}
...
func.func @exact_claim_carrying_a_tolerance(%a: f32) -> f32 {
  ...
  %r = tessera_nvidia.kernel_call %a
      {callee = "tessera_nvidia_flash", arch = "sm_120",
       ...
       accuracy = "reference_exact", determinism = "deterministic", covers = "whole_region", tolerance = 1.000000e-03 : f64}
      : (f32) -> f32
  return %r : f32
}
...
func.func @unknown_binding(%a: f32) -> f32 {
  ...
  %r = tessera_nvidia.kernel_call %a
      ...
      {callee = "tessera_nvidia_flash", arch = "sm_120",
       ...
       accuracy = "reference_exact", determinism = "deterministic", covers = "whole_region"}
      : (f32) -> f32
  return %r : f32
}
