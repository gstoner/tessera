  ...
  func.func @call(%a: f32, %b: f32) -> f32 {
    %r = tessera_nvidia.kernel_call %a, %b
        {callee = "tessera_nvidia_flash", arch = "sm_120",
         ...
         accuracy = "reference_exact", determinism = "deterministic", covers = "whole_region"} : (f32, f32) -> f32
    %p = tessera_nvidia.inline_ptx %r
        {ptx = "mul.f32 $0, $1, $1;", constraints = "=f,f",
         arch = "sm_120", accuracy = "tolerance_bounded", determinism = "deterministic", covers = "whole_region",
