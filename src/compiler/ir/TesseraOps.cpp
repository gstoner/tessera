...
LogicalResult TargetVerifyOp::verify() {
  ...
  auto outT = dyn_cast<RankedTensorType>(getTargetProbs().getType());
  if (!tokT || !logT || !outT)
    return emitOpError("tokens, logits, and target_probs must be ranked "
                       "tensors");
  ...
  // logits / target_probs: S x V f32 — one distribution per position.
  ...
  if (outT.getRank() != 2 || !outT.getElementType().isF32() ||
    ...
    return emitOpError("target_probs must match logits shape (S x V f32)");
  return success();
}
