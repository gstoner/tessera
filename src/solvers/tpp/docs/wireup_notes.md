...
1) Unzip `tpp_package_v0_1.zip` into `src/solvers/tpp/`.
2) Apply the CMake additions from `CMakeLists_additions.txt`:
   - `add_subdirectory(src/solvers/tpp)`
...
4) Build & run smoke tests:
   - `ninja tessera-opt`
   - `tessera-opt src/solvers/tpp/test/TPP/pipeline_alias.mlir -tpp-space-time`
