...
func.func @stencil_r2(%x: tensor<64x64xf32>, %k: tensor<5x5xf32>) -> tensor<64x64xf32> {
  %y = "tpp.stencil.apply"(%x, %k) {scheme = "central", order = 2 : i64, spacing = [1.0 : f64, 1.0 : f64]} : (tensor<64x64xf32>, tensor<5x5xf32>) -> tensor<64x64xf32>
...
func.func @grad_o4_dirx(%x: tensor<48x48xf32>) -> tensor<48x48xf32> {
  %y = "tpp.grad"(%x) {scheme = "central", order = 4 : i64, axis = 1 : i64, spacing = [1.0 : f64, 0.5 : f64]} : (tensor<48x48xf32>) -> tensor<48x48xf32>
  // CHECK: tpp.grad
