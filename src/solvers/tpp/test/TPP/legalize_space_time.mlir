...
// pipeline: a stencil preserves explicit scheme/order/spacing and receives
// normalized/validated markers; a time.step gets its scheme validated and
// expanded into stages/order/dt.

func.func @grad_defaults(%x: tensor<32x32xf32>) -> tensor<32x32xf32> {
  %y = "tpp.grad"(%x) {scheme = "central", order = 2 : i64, spacing = [0.5 : f64, 0.25 : f64]} : (tensor<32x32xf32>) -> tensor<32x32xf32>
  // CHECK: tpp.grad
  // CHECK-SAME: order = 2
  // CHECK-SAME: scheme = "central"
  // CHECK-SAME: tpp.scheme.normalized
  // CHECK-SAME: tpp.spacing.validated
  return %y : tensor<32x32xf32>
}

func.func @rk4_step(%s: tensor<8x8xf32>, %h: tensor<8x8xf32>) -> tensor<8x8xf32> {
  %r = "tpp.time.step"(%s) ({
    %g = "tpp.grad"(%h) {scheme = "central", order = 2 : i64, spacing = [1.0 : f64, 1.0 : f64]} : (tensor<8x8xf32>) -> tensor<8x8xf32>
