...
func.func @grad(%x: tensor<128x64xf32>) -> tensor<128x64xf32> {
  %y = "tpp.grad"(%x) {scheme = "central", order = 2 : i64, spacing = [1.0 : f64, 1.0 : f64]} : (tensor<128x64xf32>) -> tensor<128x64xf32>
  // CHECK: tpp.grad
...
func.func @grad_short(%x: tensor<16x5xf32>) -> tensor<16x5xf32> {
  %y = "tpp.grad"(%x) {scheme = "central", order = 2 : i64, spacing = [1.0 : f64, 1.0 : f64]} : (tensor<16x5xf32>) -> tensor<16x5xf32>
  // CHECK: tpp.grad
