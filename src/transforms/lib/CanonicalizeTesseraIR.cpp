...
struct FuseMatmulBiasGELU : public RewritePattern {
  FuseMatmulBiasGELU(MLIRContext *ctx)
  ...
  LogicalResult matchAndRewrite(Operation *gelu, PatternRewriter &rewriter) const override {
    ...
    st.addAttributes({
      rewriter.getNamedAttr("epilogue", rewriter.getStringAttr("gelu")),
      rewriter.getNamedAttr("has_bias", rewriter.getBoolAttr(true))
    });
    Operation *fused = rewriter.create(st);
    rewriter.replaceOp(gelu, fused->getResults());
    return success();
  }
};

struct FuseConvRelu : public RewritePattern {
  FuseConvRelu(MLIRContext *ctx)
      : RewritePattern("tessera.relu", /*benefit=*/2, ctx) {}
  ...
  LogicalResult matchAndRewrite(Operation *relu, PatternRewriter &rewriter) const override {
    ...
    st.addAttribute("epilogue", rewriter.getStringAttr("relu"));
    Operation *newConv = rewriter.create(st);
    rewriter.replaceOp(relu, newConv->getResults());
    return success();
  }
};

struct DropoutZeroSimplify : public RewritePattern {
  DropoutZeroSimplify(MLIRContext *ctx)
  ...
  LogicalResult matchAndRewrite(Operation *attn, PatternRewriter &rewriter) const override {
    ...
    if (!pAttr) return failure();
    if (pAttr.getValueAsDouble() != 0.0) return failure();
    ...
    for (auto &na : attn->getAttrs()) {
      if (na.getName() == "dropout_p") continue;
      st.addAttribute(na.getName(), na.getValue());
    }
    st.addAttribute("has_dropout", rewriter.getBoolAttr(false));
    Operation *newAttn = rewriter.create(st);
    rewriter.replaceOp(attn, newAttn->getResults());
    return success();
  }
};

struct TransposeIntoMatmul : public RewritePattern {
  TransposeIntoMatmul(MLIRContext *ctx)
  ...
  LogicalResult matchAndRewrite(Operation *mm, PatternRewriter &rewriter) const override {
    ...
    Value a = mm->getOperand(0);
    Value b = mm->getOperand(1);
    ...
    auto getFlag = [&](StringRef n){ 
      if (auto b = mm->getAttrOfType<BoolAttr>(n)) return b.getValue(); 
      return false; 
    };
    ...
    Operation *newMM = rewriter.create(st);
    rewriter.replaceOp(mm, newMM->getResults());
    return success();
  }
};

struct RedundantCastElim : public RewritePattern {
  RedundantCastElim(MLIRContext *ctx)
      : RewritePattern("tessera.cast", /*benefit=*/1, ctx) {}

  LogicalResult matchAndRewrite(Operation *cast, PatternRewriter &rewriter) const override {
    if (cast->getNumOperands() != 1 || cast->getNumResults() != 1) return failure();
...
struct CanonicalizeTesseraIRPass
    : public PassWrapper<CanonicalizeTesseraIRPass, OperationPass<ModuleOp>> {
  MLIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(CanonicalizeTesseraIRPass)
  void runOnOperation() override {
    MLIRContext *ctx = &getContext();
    RewritePatternSet patterns(ctx);
    patterns.add<FuseMatmulBiasGELU>(ctx);
    patterns.add<FuseConvRelu>(ctx);
    patterns.add<DropoutZeroSimplify>(ctx);
    patterns.add<TransposeIntoMatmul>(ctx);
    patterns.add<RedundantCastElim>(ctx);
...
namespace tessera {
std::unique_ptr<Pass> createCanonicalizeTesseraIRPass() {
  return std::make_unique<CanonicalizeTesseraIRPass>();
}
} // namespace tessera
