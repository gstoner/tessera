...
| Work item | State at 2026-08-05 | Test deliverables | Required failure proof |
| --- | --- | --- | --- |
| E2E-REAL-0 — lineage truth | implemented 2026-08-05 | `tests/unit/test_compiler_artifact_lineage.py` plus orchestration/runtime serialization coverage now gate producer/version metadata, parent/output digest adjacency, stage-presence independence, and metadata round trips. | Substituted Tile text and serialized-text tampering are detected; missing legacy provenance yields `lineage_complete = false`. Native Graph-owned package forks remain visible rather than being promoted by stage presence. |
| E2E-REAL-1 — real Schedule IR | bounded static-matmul slice implemented 2026-08-05 | Production full and lean `tessera-opt` register the generated dialect. `schedule.matmul` is an SSA edge over the retained Graph result; its SHA-256 decision identity agrees with the Graph marker and durable `schedule.artifact`. | Invalid ODS fields fail verification. Recomputed digest mismatch, altered schedule fields, or missing/duplicate durable artifacts stop Schedule→Tile rather than accepting metadata by presence. |
| E2E-REAL-2 — scheduled matmul to launch Tile | implemented 2026-08-05; backend consumption planned | `e2e_matmul_graph_schedule_tile.mlir` covers x86-f32 and ROCm-f16/f32 from Graph through Schedule to one six-operand launch Tile op. It checks selected M/N/K, storage/accumulation, layouts, pipeline depth, raster policy, and digest retention. | Graph/Schedule survivors, unsupported target dtype, dynamic shapes, transpose, forged hashes, altered knobs, and absent/duplicate artifacts fail closed. Physical package consumption and exact-device numerical evidence remain E2E-REAL-3. |
