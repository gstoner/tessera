...
| E2E-REAL-1 — real Schedule IR | registration slice implemented; SSA/content-addressed lowering planned | Production full and lean `tessera-opt` compile/link/register the generated Schedule dialect. Positive and invalid ODS fixtures run without `--allow-unregistered-dialect`; `--show-dialects` reports `schedule`. Python Schedule tests remain oracle coverage. | Invalid tile dimensions fail in the registered op verifier. Unknown/missing mixed-level SSA contracts, content-address validation, duplicate registration, and replacement of the annotation skeleton remain gates for the E2E-REAL-2 PR. |
