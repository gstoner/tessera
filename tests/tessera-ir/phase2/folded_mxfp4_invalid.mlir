...
module attributes {tessera.target = "rocm", tessera.arch = "gfx1201"} {
  func.func @wrong_reference_shape(
    ...
    // The unpacked folded contract takes a rank-1 [N] row reference; [2,48] is
    // neither that nor the packed contract's K32+reference plane.
    // CHECK: folded prefill requires fp32 A scale [M] and versioned E8M0 row reference or K32+reference plane
