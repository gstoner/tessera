// The CUDA bridge applies a proven scalar affine map, then feeds the existing
// physical tile.view linear-base contract.  It does not choose a new fragment
// layout or schedule.
// REQUIRES: tessera-nvidia-backend
// RUN: tessera-opt --lower-tile-to-nvidia='sm=120' %s | FileCheck %s

#lay = #tile.layout<shard = [16, 16] : [16, 1] on ["tlane", "reg"], replica = [] : [] on [], offset = 0>
#mem = #tile.memory_layout<space = "gmem", order = "row_major", leading_dim = 64>

func.func @nvidia_composed_layout_proof(%base: !llvm.ptr, %r: i64, %c: i64) -> !tile.tile {
...
func.func @nvidia_dynamic_nested_composed_layout_proof(
...
func.func @nvidia_tuple_basis_composed_layout_proof(%c: i64) -> i64 {
  %linear = "tile.materialize_composed_layout"(%c) {layout = #tile.composed_layout<[8], [1], [[[2, 4], [1, 2]]], [0]>} : (i64) -> i64
  return %linear : i64
}

func.func @nvidia_tuple_codomain_composed_layout_proof(%c: i64) -> (i64, i64) {
