// RUN: not %tessera_strict_opt --split-input-file --tessera-nvwgmma-lowering='sm=90' %s 2>&1 \
// RUN:   | FileCheck %s
//
// W1.1 step 2b (guard) — NVWGMMALoweringPass refuses an mma it would silently
// mis-compile.
//
...
func.func @legacy_k_step_accumulator(%a: tensor<64x64xbf16>,
  ...
  // CHECK: NVWGMMA_ACCUMULATOR_DROPPED
...
// The TYPED form, which W1.1 step 2 made expressible. Same refusal, same
// reason — the guard does not care which form the accumulator arrived in.
...
// tile control types before counting (the #491 rule). This still refuses,
// because the accumulator is real — the point is that it refuses for the
// accumulator and not for the two tokens.
//
