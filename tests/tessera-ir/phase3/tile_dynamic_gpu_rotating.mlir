...
module {
  gpu.module @dynamic {
    gpu.func @scratch(%input: !llvm.ptr<1>, %output: !llvm.ptr<1>, %n: index, %rounds: index) kernel {
      ...
      %last:2 = scf.for %i = %zero to %rounds step %one iter_args(%token = %initial_group, %acc = %initial) -> (!nvgpu.device.async.token, f32) {
        ...
        %refill = nvgpu.device_async_copy %source[%refill_position], %a[%tid], 1 : memref<?xf32, 1> to memref<?xf32, 3>
        %new_group = nvgpu.device_async_create_group %refill
        %total = arith.addf %acc, %read : f32
        scf.yield %new_group, %total : !nvgpu.device.async.token, f32
      }
