...
@pytest.mark.hardware_apple_gpu
def test_canonical_compile_default_consumes_bmm_descriptor_on_metal():
    """Level C is the default canonical route, not an opt-in test-only path."""
    ...
    result = canonical_compile(_module(), target="apple_gpu")
    assert result.bundle.native_image is not None
    assert result.bundle.launch_descriptor is not None
    launched = launch(result.to_runtime_artifact(), {"a": a, "b": b, "out": np.empty((2, 4, 16), np.float32)})
