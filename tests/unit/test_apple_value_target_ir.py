...
def test_gpu_full_ppo_policy_loss_emits_value_symbol():
    p = _run("tessera-lower-to-apple_gpu-full", _GRAPH["ppo_policy_loss"])
    assert p.returncode == 0, p.stderr
    assert "tessera_apple.gpu.kernel_call" in p.stdout
    assert 'op_kind = "ppo_policy_loss"' in p.stdout
    assert 'symbol = "tessera_apple_gpu_ppo_policy_loss_f32"' in p.stdout
    assert 'framework = "MPSGraph"' in p.stdout
    ...
    assert c["symbol"] == "tessera_apple_gpu_ppo_policy_loss_f32"
    assert c["abi"] == "mpsgraph_ppo_policy_loss_f32"
...
def test_gpu_value_executor_allowlist_exact():
    ...
    expected = frozenset({
        ...
        "tessera_apple_gpu_mtl4_matmul2d_view",           # APPLE-MATMUL2D-1: one view
        "tessera_apple_gpu_mtl4_matmul2d_view_epilogue",  # symbol per op, every pair
