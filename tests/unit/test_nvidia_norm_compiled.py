...
def _artifact(op_name: str, eps: float | None = None):
    ...
    return rt.RuntimeArtifact(metadata={
        ...
        "output_name": "o", "ops": [{"op_name": op_name, "result": "o",
        "operands": ["x"], "kwargs": {} if eps is None else {"eps": eps}}],
    })
...
def test_live_nvidia_norm_matches_numpy(op, eps, shape, dtype):
    ...
    expected = _ref(x, op, eps).astype(dtype)
    np.testing.assert_allclose(result["output"], expected,
                               atol=3e-5 if dtype == np.float32 else 5e-3, rtol=0)
...
def test_nvidia_norm_rejects_non_float_without_gpu():
    ...
    with pytest.raises(ValueError, match="f32/f16"):
        rt._execute_nvidia_compiled_norm(_artifact("tessera.rmsnorm"),
