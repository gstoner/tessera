"""kv_cache state_update lane on AMD ROCm gfx1151 (P13 of
S_SERIES_GAP_CLOSURE_PLAN) — the paged scatter-copy into a KV buffer: append
(the COMPILER-GENERATED gfx1151 scatter kernel, set mode), read (the gather
kernel), prune (host page compaction). Reachable via
`compiler_path="rocm_kv_cache_compiled"`. Validated vs a numpy paged-buffer
...
def test_append_writes_rows():
    rt = _rocm_or_skip()
    buf = _RNG.standard_normal((8, 4, 16)).astype(np.float32)
    pos = np.array([5, 0, 3], np.int64)               # unique → no store race
