...
rocm_stat_reduce_compiled (var/std/count_nonzero) / rocm_stable_reduce_compiled.
...
def test_prod(axis):
    ...
    res = rt.launch(_art(rt, "tessera.prod", "rocm_reduce_compiled",
                         {"axis": axis}), (x,))
...
def test_var_std(op, ref, axis):
    ...
    res = rt.launch(_art(rt, op, "rocm_stat_reduce_compiled", {"axis": axis}),
                    (x,))
    assert res["ok"] is True, res.get("reason")
    np.testing.assert_allclose(np.asarray(res["output"]).astype(np.float32),
...
def test_count_nonzero():
    ...
    res = rt.launch(_art(rt, "tessera.count_nonzero", "rocm_stat_reduce_compiled",
                         {"axis": -1}), (x,))
    assert res["ok"] is True, res.get("reason")
...
def test_logsumexp(axis):
    ...
    res = rt.launch(_art(rt, "tessera.logsumexp", "rocm_stable_reduce_compiled",
                         {"axis": axis}), (x,))
    assert res["ok"] is True, res.get("reason")
...
def test_log_softmax_and_safe():
    ...
    rl = rt.launch(_art(rt, "tessera.log_softmax", "rocm_stable_reduce_compiled",
                        {"axis": -1}), (x,))
    ...
    rs = rt.launch(_art(rt, "tessera.softmax_safe", "rocm_stable_reduce_compiled",
                        {"axis": -1}), (x,))
    ...
    np.testing.assert_allclose(np.asarray(rs["output"]).astype(np.float32),
                               ...
                               atol=2e-4, rtol=2e-4)
    rg = rt.launch(_art(rt, "tessera.sigmoid_safe", "rocm_stable_reduce_compiled",
                        {}), (x,))
