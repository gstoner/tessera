...
def _artifact(rt, op_name, operands, kwargs=None):
    return rt.RuntimeArtifact(metadata={
        "target": "x86",
        "compiler_path": "x86_metric_loss_compiled",
        "executable": True,
...
def test_x86_metric_losses_match_reference():
    rt = _x86_or_skip()
    ...
    for op_name, names, args, kwargs, ref in cases:
        ...
        assert res["compiler_path"] == "x86_metric_loss_compiled"
        np.testing.assert_allclose(res["output"], ref, atol=2e-4, rtol=2e-4)


def test_x86_info_nce_nt_xent_and_seq2seq_match_reference():
    rt = _x86_or_skip()
