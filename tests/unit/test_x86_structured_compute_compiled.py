"""x86 structured-compute runtime lane."""
...
def _artifact(op_name, operands, kwargs=None):
    return rt.RuntimeArtifact(metadata={
        "target": "x86",
        "compiler_path": "x86_structured_compute_compiled",
        "executable": True,
...
def _launch(op_name, names, args, kwargs=None):
    ...
    assert res["compiler_path"] == "x86_structured_compute_compiled"
    return res["output"]


def test_x86_structured_loss_and_vision_layout_match_reference():
    ...
    out = _launch("tessera.ctc_loss", ("lp", "targets", "ilen", "tlen"),
...
def test_x86_structured_model_recurrent_and_stencil_match_reference():
    rng = np.random.default_rng(71)
    x = rng.standard_normal((2, 3, 7)).astype(np.float32)
    w = rng.standard_normal((4, 3, 3)).astype(np.float32)
    b = rng.standard_normal((4,)).astype(np.float32)
    np.testing.assert_allclose(
        _launch("tessera.conv1d", ("x", "w", "b"), (x, w, b), {"padding": 1}),
    ...
    np.testing.assert_allclose(
        _launch("tessera.simple_rnn_cell", ("x", "h", "Wih", "Whh"),
                (xt, h, Wih, Whh)),
        F.simple_rnn_cell(xt, h, Wih, Whh),
        atol=1e-6,
    )
    ...
    np.testing.assert_allclose(
        _launch("tessera.gru_cell", ("x", "h", "Wih", "Whh"),
                (xt, h, Wih3, Whh3)),
        F.gru_cell(xt, h, Wih3, Whh3),
        atol=1e-6,
    )

    # Canonical tessera.ops.lstm_cell: (4H,In)/(4H,H) weights, packed output.
    ...
    np.testing.assert_allclose(
        _launch("tessera.lstm_cell", ("x", "h", "c", "Wih", "Whh"),
...
def test_x86_structured_attention_and_scan_match_reference():
    ...
    np.testing.assert_allclose(
        _launch("tessera.perceiver_resampler", ("latents", "x"), (latents, x)),
    ...
    k4 = rng.standard_normal((1, 2, 3, 8)).astype(np.float32)
    v4 = rng.standard_normal((1, 2, 3, 8)).astype(np.float32)
    np.testing.assert_allclose(
        _launch("tessera.attn_compressed_blocks", ("q", "k", "v"), (q4, k4, v4)),
        ops.attn_compressed_blocks(q4, k4, v4),
        atol=1e-6,
    )
    np.testing.assert_allclose(
        _launch("tessera.linear_attn_state", ("q", "k", "v"), (q4, q4, q4),
                {"feature_map": "identity", "causal": True}),
        ops.linear_attn_state(q4, q4, q4, feature_map="identity", causal=True),
        atol=1e-6,
    )

    q5 = rng.standard_normal((1, 1, 2, 2, 4)).astype(np.float32)
    k5 = rng.standard_normal((1, 1, 2, 2, 4)).astype(np.float32)
    v5 = rng.standard_normal((1, 1, 2, 2, 4)).astype(np.float32)
    np.testing.assert_allclose(
        _launch("tessera.attn_local_window_2d", ("q", "k", "v"), (q5, k5, v5),
                {"window": (1, 1)}),
        ops.attn_local_window_2d(q5, k5, v5, window=(1, 1)),
        atol=1e-6,
    )
...
def test_x86_structured_layout_tail_matches_reference():
    ...
    np.testing.assert_allclose(
        _launch("tessera.cast", ("x",), (x,), {"dtype": "fp16"}),
        ops.cast(x, "fp16"),
    )
    np.testing.assert_allclose(
    ...
    np.testing.assert_allclose(
        _launch("tessera.transpose", ("x",), (x,), {"axes": (0, 2, 1)}),
    ...
    depth = _launch("tessera.mor_router", ("x", "w"), (hidden, w_router), {"max_depth": 3})
    np.testing.assert_array_equal(depth, ops.mor_router(hidden, w_router, max_depth=3))
    part = _launch("tessera.mor_partition", ("x", "depth"), (hidden, depth), {"step": 2})
    np.testing.assert_array_equal(part, ops.mor_partition(hidden, depth, step=2))
    updated = hidden + 1.0
    np.testing.assert_allclose(
        _launch("tessera.mor_scatter", ("full", "updated", "mask"), (hidden, updated, part)),
        ops.mor_scatter(hidden, updated, part),
    )


def test_x86_structured_helper_tail_matches_reference():
    ...
    np.testing.assert_allclose(
        _launch("tessera.masked_scatter", ("x", "mask", "source"), (x, mask, source)),
    ...
    got_state = _launch("tessera.online_softmax_state", ("x",), (logits,), {"axis": -1})
