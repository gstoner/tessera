#!/usr/bin/env python3
import json, argparse, math, collections, time, os
...
def select_peaks(peaks_map, arch, env_arch):
    if arch and arch in peaks_map:
        p = peaks_map[arch]; return p.get("peak_flops"), p.get("hbm_gbs")
    if env_arch and env_arch in peaks_map:
        p = peaks_map[env_arch]; return p.get("peak_flops"), p.get("hbm_gbs")
    for _, p in peaks_map.items():
        return p.get("peak_flops"), p.get("hbm_gbs")
    return None, None
...
def main():
    ...
    ap.add_argument("--peaks", type=str, default=None, help="YAML with device peaks")
    ap.add_argument("--arch", type=str, default=None, help="Architecture key to pick from YAML (e.g., sm90)")
    args = ap.parse_args()
    ...
    pf = args.peak_flops
    hb = args.hbm_gbs
    if args.peaks and (pf is None or hb is None):
        peaks_map = load_peaks(args.peaks)
        env_arch = os.environ.get("TPROF_ARCH")
        # select using helper from v3 logic
        if args.arch and args.arch in peaks_map:
            sel = peaks_map[args.arch]
            pf = pf if pf is not None else sel.get("peak_flops")
            hb = hb if hb is not None else sel.get("hbm_gbs")
        elif env_arch and env_arch in peaks_map:
            sel = peaks_map[env_arch]
            pf = pf if pf is not None else sel.get("peak_flops")
            hb = hb if hb is not None else sel.get("hbm_gbs")
        else:
            for _, p in peaks_map.items():
                pf = pf if pf is not None else p.get("peak_flops")
                hb = hb if hb is not None else p.get("hbm_gbs")
                break

    make_html(totals, args.out, peak_flops=pf, hbm_gbs=hb)
